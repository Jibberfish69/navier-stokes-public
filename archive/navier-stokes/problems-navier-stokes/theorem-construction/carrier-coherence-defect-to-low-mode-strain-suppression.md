...
- an isolated coherent block `K_D`;
...
1. **Frequency-aligned readout.**
   There is a carrier block `K_D(j_0)` whose coherent projector exports to the
   classical low-frequency projector:
   ```math
   \rho(\mathbb H_D(j_0)X)=P_{\le j_0}u.
   ```
2. **Coherent block control.**
   The coherent transport operator on `K_D(j_0)` has operator norm bounded by
   the viscous scale:
   ```math
   \|B_i(X)|_{K_D(j_0)}\|_{\mathrm{op}}
   \le
   c\,\nu\,2^{2j_0}.
   ```
3. **Leakage export.**
...
- coherence stays on the carrier block;
- leakage is lower order by spectral gap plus commutator control;
- only the coherent block contributes at principal order to the low-frequency
  classical strain.
...
- a readout theorem aligning coherent carrier blocks with classical dyadic low
  modes;
