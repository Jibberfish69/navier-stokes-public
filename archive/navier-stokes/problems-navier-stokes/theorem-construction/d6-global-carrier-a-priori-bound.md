...
Assume the carrier block calculus has a spectral gap on each active
...
be the carrier-block expansion on `K_D\oplus \mathbb H_D`. Define the block
weights
...
\quad\text{for every active carrier block }Q_i e_\beta,
}
```
