...
incompressible flow of finite energy and a fixed positive viscosity. The
momentum equation has four pieces:
\[
...
\]
The central problem lies in the competition between nonlinear transport
and viscosity.

A vortex can concentrate velocity
gradients while it stretches.  In the concentrating case, surrounding axial
strain lengthens the tube, and incompressibility contracts its fluid across
the width: a longer parcel cannot keep its old cross-section without gaining
volume.  Mean circulating speed is circulation magnitude divided by
circumference, so the shrinking circuit speeds the rotation up if viscous
change in circulation does not offset that contraction.  Faster rotation
across less width steepens the velocity gradients, while viscosity spreads
concentrated vorticity outward.  Both effects can grow stronger as the core
gets smaller.
...
In order for a singularity to happen, velocity
...
opens into every product allowed by the product rule.  At order \(r\), those
products occur with multiplicity \(\binom ra\), giving the full pattern
\[
 \partial_t^{r+1}u
 +\sum_{a=0}^{r}\binom ra
...
\[
 ...
 Q_r:=\partial_t^rp.
\]
With \(p\) fixed by Riesz normalization, incompressibility determines \(Q_r\)
through the Riesz transforms \(R_i\):
\[
 Q_r
 =R_iR_j\sum_{a=0}^{r}\binom ra
   V_{a,i}V_{r-a,j}.
\]
The pressure row stays with the nonlinear row at every order.
...
\(T_*<\infty\).  For the pressure-complete mixed jet generated by \(u_0\)
at this fixed viscosity, Theorem~\ref{thm:datum-rectangles} supplies the
whole-terminal adjacent rows: for every finite \(n,M\),
\[
 V_n\in L^2(0,T_*;H^{M+1}),
 \qquad
 V_{n+1}\in L^2(0,T_*;H^{M-1}).
\]
Each entry is an actual derivative of this velocity--pressure history, with
integrability on the full approach to \(T_*\).  Restricting a row to
\((0,T)\), where \(T<T_*\), can only decrease its norm.  For a fixed finite
pair \((N,M)\), finite summation gives
\[
\begin{gathered}
...
\end{gathered}
\]
These finite construction bounds assemble the coordinates of the intersection.
The familiar space-time and continuation estimates are later consequences of
that completed object, not assumptions used to obtain the rows.

For \(0\le n\le N\), \(V_{n+1}=\partial_tV_n\), so the Hilbert triple
\[
