...
\[
 ...
 \qquad \nabla\!\cdot u=0.
\]
The initial velocity is \(u_0\in H^\infty_\sigma(\mathbb R^3)\), and
\(\nu>0\) stays fixed throughout. A classical solution exists for a while;
the question is whether its maximal lifespan \([0,T_*)\) can end at a finite
...
circulates faster around a smaller core. The velocity then changes more
sharply across the core, and the stretching can intensify the very gradients
that viscosity is smoothing. Viscosity diffuses momentum between neighboring
regions of fluid, reducing velocity differences. Whether it can smooth
those differences fast enough is the
difficulty.
...
\]
Infinitely many stages could then fit before a finite terminal time. That
...
from \(u_0\); the differentiated identities reveal it, while the construction
