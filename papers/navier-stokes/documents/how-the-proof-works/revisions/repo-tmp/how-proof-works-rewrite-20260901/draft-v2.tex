...
The vortex picture has now reached a finite endpoint.  The derivative tower
begins by asking what an unbounded change in velocity would have to do in the
time left.  For every finite temporal order \(j\),
\[
\partial_t^ju(t)-\partial_t^ju(0)
=\int_0^t\partial_s^{j+1}u(s)\,ds.
\]
If \(\partial_t^{j+1}u\) stayed bounded on a finite interval, then
\(\partial_t^ju\) could not become unbounded there.  Apply that observation
from velocity to each response that follows it.
