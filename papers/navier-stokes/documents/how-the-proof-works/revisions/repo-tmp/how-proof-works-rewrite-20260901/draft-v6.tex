...
\]
grows as the radius \(\ell\) shrinks.  The velocity changes more sharply across
less distance.  This is nonlinear sharpening in the fluid itself.  Viscosity
resists it by diffusing velocity
...
stretching term \(S\omega\) strengthens it.  This is how axial stretching can
intensify rotation inside the narrowing core.  Viscosity acts against that
...
singularity to form, velocity has to blow up.  For that to happen, its
acceleration also has to blow up, which means its jerk has to blow up, and so
on, and so on:
\[
...
The two finite-time integrals now repeat at every finite order \(j\):
\[
...
\]
The smooth datum makes every finite initial response finite.  If one higher
response stayed uniformly bounded on the finite interval, the response below
it would stay bounded as well.  Beginning with the required escape of \(u\)
and repeating this implication upward gives, for every fixed \(j\),
\[
...
=\mathcal P_{r,s}-2\nu E_{r,s+1}.
\]
The differentiated identities have now exposed the coordinates to be paid for;
they have not made any coordinate finite by rearrangement.  Fix a finite
...
The adjacent velocity pair then gives the middle trace:
\[
V_r\in C([0,T_*];H^M).
\]
