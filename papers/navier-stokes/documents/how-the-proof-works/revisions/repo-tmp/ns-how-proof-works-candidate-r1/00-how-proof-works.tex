...
The control comes from the whole-terminal adjacent rows.  Let
\([0,T_*)\) be the maximal classical lifespan and suppose \(T_*<\infty\).
For every fixed finite pair \((N,M)\), the datum-generated rectangle theorem
gives, for \(0\le n\le N\),
\[
 V_n\in L^2(0,T_*;H^{M+1}),
 \qquad
 V_{n+1}\in L^2(0,T_*;H^{M-1}).
\]
These are bounds on the full approach to \(T_*\), not bounds on one cutoff
interval at a time.  Since \(V_{n+1}=\partial_tV_n\), the adjacent pair sits in
the Hilbert triple
\[
...
\[
 V_n\in C([0,T_*];H^M).
\]
The pressure formula supplies \(Q_n\) and \(\nabla Q_n\) at the matching
heights, so the finitely many rows form one pressure-complete rectangle
\[
...
Their projective intersection is
\[
 \mathcal I[u_0]
 :=\varprojlim_{N,M}\mathcal R_{N,M}[u_0;T_*],
 \qquad
