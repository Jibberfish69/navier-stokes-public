\section{Compatible intersection, endpoint jet, and restart}

\begin{definition}[Compatible intersection]
...
\begin{theorem}[Compatible assembly and whole-terminal mixed regularity]
...
\end{equation}
and all of its finite coordinates are the restrictions of the same
pressure-complete history.  Its coordinates \((V_n,Q_n)_{n\in\Nzero}\)
satisfy
\begin{align}
...
\end{align}
and every finite restriction is the corresponding rectangle
\(\cR_{N,M}[u_0;T_*]\).  Its velocity realization satisfies
\begin{equation}
...
\begin{proof}
Theorem~\ref{thm:datum-rectangles} places every velocity coordinate of
\(\cR_{N,M}[u_0;T_*]\) in its whole-terminal adjacent spaces.
Proposition~\ref{prop:rectangle-pressure-completion} places every retained
pressure coordinate in its whole-terminal pressure space.
Proposition~\ref{prop:rectangle-compatibility} gives
\[
 \rho_{N',M'}^{N,M}\cR_{N,M}[u_0;T_*]
 =\cR_{N',M'}[u_0;T_*]
 \qquad(N'\le N,\ M'\le M),
\]
so \eqref{eq:ier-compatible-family} belongs to \(\mathscr X_\infty\).

Fix \(n\).  Every rectangle with \(N\ge n\) contains the same distributions
\(\partial_t^nu\) and \(\partial_t^np\).  Increasing \(M\), using the
canonical Sobolev embeddings, gives
\eqref{eq:ier-inverse-velocity-coordinate} and
\eqref{eq:ier-inverse-pressure-coordinate}.  Thus each finite rectangle reads
the same distributional coordinate in its own Sobolev topology.

Fix finite \(r,m\) and choose \(N\ge r\), \(M\ge m\).  The upper rectangle
edges give
\[
 \partial_t^nu\in L^2(0,T_*;H^{M+1})
 \hookrightarrow L^2(0,T_*;H^m)
 \qquad(0\le n\le r).
\]
They are the distributional time derivatives of the same \(u\), so
\(u\in H^r(0,T_*;H^m)\).  Since \((r,m)\) was arbitrary,
\eqref{eq:mixed-sobolev-intersection} follows.  Summing the retained
coordinates and using Sobolev nesting gives
\eqref{eq:mixed-sobolev-finite-bound}.
\end{proof}

\begin{proposition}[Complete spatial row and common endpoint]
\label{prop:ier-complete-spatial-endpoint}
Under the hypotheses of
Theorem~\ref{thm:compatible-inverse-intersection}, for every
\(m\in\Nzero\),
\begin{equation}
...
to \(L^1(0,T_*;H^m)\).  The same spatial row and \(T_*<\infty\) give
...
the same preterminal function after the embedding \(H^k\hookrightarrow
...
Under the hypotheses of Theorem~\ref{thm:compatible-inverse-intersection},
...
\begin{proof}
Equation \eqref{eq:ier-whole-terminal-bochner} and
...
\eqref{eq:ier-finite-mixed-bound}.  Apply the same multiplier bound to the
...
\begin{proof}
For \(0\le n\le N\), Theorem~\ref{thm:datum-rectangles} gives
\[
 V_n\in L^2(0,T_*;H^{m+1}),
 \qquad
 \partial_tV_n=V_{n+1}\in L^2(0,T_*;H^{m-1}).
\]
Lemma~\ref{lem:adjacent-trace} gives the first membership in
...
\end{align*}
This proves the bound in \eqref{eq:ier-row-trace}.  Apply the same lemma to
...
representatives equal the same preterminal distribution.  After applying
...
\eqref{eq:ier-terminal-pressure-modulus}.  The Fourier multiplier inequality
\[
 \norm{\partial_x^\alpha f}_{H^m}
 \le\norm f_{H^{m+|\alpha|}}
\]
applied to \eqref{eq:ier-terminal-pressure-size} and
\eqref{eq:ier-terminal-pressure-modulus} proves
...
which proves \eqref{eq:ier-terminal-velocity-row}.
...
\[
 ...
 \delta_0\le\delta_\nu(u_*),
\]
with \(\delta_\nu\) as in Theorem~\ref{thm:local-restart}.
Theorem~\ref{thm:local-restart} gives both pairs on the stated intervals.
For each \(b<T_*\), Lemma~\ref{lem:shifted-mild-restriction} places
\((u,p)|_{[t_0,b]}\) and
...
\eqref{eq:ier-same-history-uniqueness-class} has the same datum \(u_*\) on
the right of \(T_*\) and belongs to the same shifted mild class as
...
\begin{proof}
If \(T_*<\infty\), Theorem~\ref{thm:datum-rectangles} supplies the
whole-terminal rectangles.  The chain from
...
\(t\ge0\); \eqref{eq:ier-global-energy-bound} follows by discarding the
