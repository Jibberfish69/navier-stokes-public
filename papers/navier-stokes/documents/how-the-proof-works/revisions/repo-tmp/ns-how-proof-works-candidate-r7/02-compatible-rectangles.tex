...
The theorem is displayed in rectangle form, but its proof order is
\[
 u_0
 \longrightarrow \text{pressure-complete mixed jet}
 \longrightarrow \text{whole-terminal adjacent rows}
 \longrightarrow \text{finite rectangles}.
\]
The middle arrow is the fixed datum-side payment: the rows in
\eqref{eq:datum-adjacent-rows} are the actual derivatives of the one maximal
history and have the stated whole-terminal integrability.  The last arrow is
finite summation.  The next proposition writes out that final step, proves the
converse by monotone exhaustion, and realizes the cutoff norms on \((0,T_*)\).
...
\end{equation}
For every \(m\in\Nzero\), the same row yields the spatial-column projection
\begin{equation}
...
\begin{proof}
All retained entries are the same distributions
