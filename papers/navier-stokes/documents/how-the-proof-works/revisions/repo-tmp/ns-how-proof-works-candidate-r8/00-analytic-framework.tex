...
\]
The same integral defines \(\widehat f\) for \(f\in L^1(\R^3)\).  The
...
\[
 ...
 \le\mathsf C_m^{\rm F}\norm f_{H^m}\norm g_{H^m}.
\]
The chosen maximum proves both asserted estimates with the same constant.
\end{proof}
...
\(v_R\).  Applied to \(S_Rv-S_Sv\), the same estimate shows that
...
\end{samepage}
\begin{samepage}
For \(\ell=1\), the same formula follows from the direct
...
endpoint.  To continue their common limit as the same solution, we now fix a
...
\end{equation}
If \((u,p)\) is a pressure-complete classical history and
...
velocities have the same left trace, their shifted Duhamel identities and the
...
and commutes with the heat flow, \(\Pp\), and spatial derivatives.  The same
estimates that gave \(\Phi\) its contraction factor at most \(1/2\) apply to
...
the same distributional trace as the already constructed \(C H^3\) solution.
