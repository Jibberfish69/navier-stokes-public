...
u(\cdot,0)=u_0.
\]
A vortex tube inside this flow can be caught in a strain that pulls along its
axis. Because \(\nabla\!\cdot u=0\), the volume of each fluid parcel is
preserved. The tube lengthens, its cross-section contracts, and the rotating
core narrows.

The tube carries its rotation in the vorticity
\(\omega=\nabla\times u\), which evolves by
\[
\partial_t\omega+(u\!\cdot\nabla)\omega
=(\omega\!\cdot\nabla)u+\nu\Delta\omega.
\]
The term \((\omega\!\cdot\nabla)u\) is the stretching of that rotation. When
the strain is aligned with the vortex,
\(\omega\cdot((\omega\!\cdot\nabla)u)>0\), so this contribution drives
\(|\omega|\) upward inside the narrowing core. Because \(\omega\) is already
one spatial derivative of \(u\), the velocity now changes more sharply across
the core: its gradient rises.

Viscosity enters through \(\nu\Delta\omega\), whose strength also depends on
the width of the core. For \(\lambda>1\), shrink space and change velocity,
pressure, and time with it:
\[
u_\lambda(x,t)=\lambda u(\lambda x,\lambda^2t),
\qquad
p_\lambda(x,t)=\lambda^2p(\lambda x,\lambda^2t).
\]
A core of width \(r\), velocity scale \(U\), and duration \(\tau\) becomes a
core of width \(r/\lambda\), velocity scale \(\lambda U\), and duration
\(\tau/\lambda^2\). Applied to the equation, this change gives
\[
\begin{aligned}
&\partial_tu_\lambda
+(u_\lambda\!\cdot\nabla)u_\lambda
...
\end{aligned}
\]
Every term has acquired the factor \(\lambda^3\), so nonlinear sharpening and
viscous diffusion have intensified in equal proportion: narrowing the core has
not given viscosity an advantage.

The velocity scale grew by \(\lambda\), but the occupied volume fell by
\(\lambda^{-3}\). Hence
\[
\|u_\lambda(\cdot,t)\|_2^2
=\lambda^{-1}\|u(\cdot,\lambda^2t)\|_2^2.
\]
The gradients can grow while the kinetic energy falls.

Across the homogeneous Sobolev scale, this contraction gives
\[
...
\]
At \(s=\tfrac12\), the exponent vanishes, so the width of the core changes
without changing its \(\dot H^{1/2}\) size. Below that height the norm
decreases; above it the norm increases. This makes \(s=\tfrac12\) the critical
height.

Now assume one evolving vortex repeats the full scale change, reducing its
width by a fixed factor \(0<\rho<1\) at each turn. Then
\[
\begin{gathered}
r_j=\rho^jr_0,
\qquad U_j=\rho^{-j}U_0,
...
\end{gathered}
\]
Under this hypothesis, infinitely many turns fit before finite time while
\(U_j\to\infty\): a Zeno cascade compatible with finite duration and falling
kinetic energy.
