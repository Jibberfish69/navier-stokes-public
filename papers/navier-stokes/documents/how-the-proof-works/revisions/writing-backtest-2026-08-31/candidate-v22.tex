...
In three-dimensional Navier--Stokes, nonlinear transport can sharpen a flow while viscosity smooths it:
\[
\partial_tu+(u\cdot\nabla)u+\nabla p=\nu\Delta u,
\qquad \nabla\cdot u=0,
\qquad u(\cdot,0)=u_0.
\]
The regularity problem is whether that sharpening can keep concentrating a smooth velocity field until it becomes singular in finite time.

A vortex tube gives the physical picture. The surrounding strain pulls it along its axis. Because the fluid is incompressible, the tube cannot gain volume as it lengthens; its cross-section contracts and the rotating core narrows. The strain acts on the vorticity \(\omega=\nabla\times u\) through
\[
...
\]
the stretching part raises \(\lvert\omega\rvert\) while the core narrows. Vorticity is one spatial derivative of velocity, so the velocity changes across less distance and its gradients steepen. The nonlinear motion is sharpening the core at the point where viscosity is trying to diffuse it.

Scaling asks how this contest changes when the entire pattern is compressed. For \(\lambda>1\), form
\[
u_\lambda(x,t)=\lambda u(\lambda x,\lambda^2t),
\qquad
p_\lambda(x,t)=\lambda^2p(\lambda x,\lambda^2t).
\]
A structure of width \(r\), velocity scale \(U\), and duration \(\tau\) becomes one of width \(r/\lambda\), velocity scale \(\lambda U\), and duration \(\tau/\lambda^2\). The characteristic sizes of the nonlinear and viscous terms change as
\[
\frac{U^2}{r}
\longmapsto
\lambda^3\frac{U^2}{r},
...
\lambda^3\nu\frac{U}{r^2}.
\]
Direct substitution gives the factor \(\lambda^3\) in every term of the equation. As the core shrinks, nonlinear sharpening and viscous diffusion strengthen in exact proportion. Scale alone gives neither one an advantage.
...
\]
Its gradients can grow while its contribution to the energy falls. Across the homogeneous Sobolev scale,
\[
\|u_\lambda(\cdot,t)\|_{\dot H^s}
=\lambda^{s-\frac12}
\|u(\cdot,\lambda^2t)\|_{\dot H^s}.
\]
Concentration makes the norm smaller below \(s=\tfrac12\) and larger above it. At \(s=\tfrac12\), it does neither. This is the critical height: the scale at which narrowing the flow gives the measurement no automatic gain and no automatic loss. Write
\[
...
\]
Infinitely many narrower and faster stages fit inside a finite interval. This is the Zeno cascade. It does not construct a singular solution. It isolates the difficulty: finite energy does not prevent concentration, and shrinking the scale does not make viscosity overpower the nonlinear term.
...
\]
A singularity would need the temporal ladder to run toward spatial roughness. If every finite coupled row can be kept on the whole interval, the ladder runs into \(H^\infty\) instead.
...
\]
and its full norm satisfies
\[
\sup_{0<T<T_*}\mathcal R_{N,m}(u;T)<\infty.
\]
Viscosity spans the two spatial levels in the adjacent pair; the nonlinear products and pressure remain inside the joined row. The constant may depend on \(N\) and \(m\). The proof never asks one constant to control the infinite tower.

Now repeat this finite argument for every \(N\) and \(m\). Every block comes from one velocity history generated by \(u_0\), so entries shared by two blocks already agree. The blocks fit into
\[
u_0
\longrightarrow
...
\]
This is where the finite estimates pay off: compatibility turns them into one all-orders intersection without taking an infinite-order limit of their constants.
...
-\nabla Q_N.
\]
Every \(V_N\) and \(Q_N\) lies in \(H^\infty\) at \(T_*\). Smooth local existence restarts the solution from \(u_*\), and uniqueness glues that restart to the original velocity history. The solution continues past \(T_*\), contradicting maximality. The finite terminal time cannot occur.
