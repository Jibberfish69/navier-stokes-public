...
A vortex tube lengthens when the surrounding strain pulls it along its axis. Incompressibility prevents that lengthening from creating volume, so the cross-section contracts and the rotating core narrows. The rotation is now confined to a smaller region.

The velocity field carrying the tube evolves according to the three-dimensional Navier--Stokes equation:
\[
\partial_tu+(u\cdot\nabla)u+\nabla p=\nu\Delta u,
\qquad \nabla\cdot u=0,
\qquad u(\cdot,0)=u_0.
...
\]
the stretching part raises \(\lvert\omega\rvert\) while the core narrows. Since vorticity is one spatial derivative of velocity, the velocity changes across less distance and its gradients steepen. This is the sharpening: nonlinear motion confines velocity variation to a narrower core, while viscosity diffuses it. The global regularity problem asks whether a smooth velocity field can concentrate far enough to become singular in finite time.
