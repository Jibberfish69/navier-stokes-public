...
\qquad u(\cdot,0)=u_0.
\]
To see how the first effect can arise, follow a vortex tube. Surrounding strain pulls it along its axis. Incompressibility prevents that lengthening from creating volume, so the cross-section contracts and the rotating core narrows. The rotation is now confined to a smaller region.
