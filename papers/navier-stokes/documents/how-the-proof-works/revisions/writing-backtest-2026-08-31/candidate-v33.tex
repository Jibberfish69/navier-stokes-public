...
The three-dimensional Navier--Stokes regularity problem asks whether a smooth velocity field can become singular in finite time. The equation carries two opposed effects: nonlinear motion can concentrate velocity variation into smaller scales, while viscosity diffuses it.
\[
\partial_tu+(u\cdot\nabla)u+\nabla p=\nu\Delta u,
\qquad \nabla\cdot u=0,
\qquad u(\cdot,0)=u_0.
\]
To see how the first effect can arise, follow a vortex tube. Surrounding strain pulls it along its axis. Incompressibility prevents that lengthening from creating volume, so the cross-section contracts and the rotating core narrows.

Curling the equation shows what the strain does to the vorticity \(\omega=\nabla\times u\):
\[
...
\]
the stretching part raises \(\lvert\omega\rvert\) while the core narrows. Since vorticity is one spatial derivative of velocity, the velocity changes across less distance and its gradients steepen. The stretching term pushes the gradients upward as the core narrows; viscosity diffuses them across the core.
...
A finite kinetic-energy bound does not exclude this concentration. The compressed core occupies \(\lambda^{-3}\) of the volume while the square of its velocity contributes \(\lambda^2\), so
\[
\|u_\lambda(\cdot,t)\|_2^2
=\lambda^{-1}\|u(\cdot,\lambda^2t)\|_2^2.
\]
As the core narrows, its gradients can grow while its contribution to kinetic energy falls. Across the homogeneous Sobolev scale,
\[
...
\]
The \(n\)-th derivative of the critical energy \(H\) reaches the spatial energy at height \(n+\tfrac12\), while every differentiated nonlinear and pressure transfer remains in the identity. Together with the finite-energy level \(L^2\), those half-integer heights fill every Sobolev order:
\[
L^2(\mathbb R^3)
...
Take one finite temporal order \(N\) and one finite spatial order \(m\). Differentiate only to those orders and keep the resulting velocity, nonlinear, pressure, and viscous terms in one block. The pressure-complete recurrence constructs its entries. Every entry is a derivative of one velocity history generated by \(u_0\), so any coordinate shared with another block is already fixed. The recurrence constructs this block; the whole-terminal rectangle theorem bounds it. For this fixed \(N,m\), the theorem gives
\[
V_N\in L^2(0,T_*;H^{m+1}),
\qquad
V_{N+1}\in L^2(0,T_*;H^{m-1}),
\]
with a bound that remains finite as \(T\uparrow T_*\). Viscosity spans the two spatial levels in the adjacent pair; the nonlinear products and pressure remain inside the joined row. The constant may depend on \(N\) and \(m\). No common constant over the infinite tower is needed.

Repeat the finite argument for every \(N\) and \(m\). Since the shared entries already agree, the blocks fit into
\[
u_0
\longrightarrow
