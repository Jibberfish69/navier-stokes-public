...
H(t):=E_{1/2}(t).
\]
The derivatives \(H',H'',H''',\ldots\) are taken along
\(V_0,V_1,V_2,V_3,\ldots\). Pairing the \(V_0\)-equation at critical height
makes the first spatial step visible, and each further temporal derivative
brings out the next top viscous term:
\[
\begin{aligned}
H'   &=(\text{transport terms})-2\nu E_{3/2},\\
H''  &=(\text{differentiated transport terms})+(2\nu)^2E_{5/2},\\
H''' &=(\text{differentiated transport terms})-(2\nu)^3E_{7/2},\\
H^{(n)}
     &=(\text{terms below the new top spatial height})
       +(-2\nu)^nE_{n+1/2}.
\end{aligned}
\]
In its shortest form, the temporal sequence reads
\[
H,\ H',\ H'',\ H''',\ldots
\qquad\text{exposes}\qquad
E_{1/2},\ E_{3/2},\ E_{5/2},\ E_{7/2},\ldots .
\]
This sequence is the critical-height reading of the \(V_n\)-recurrence: while
\(V_n=\partial_t^{\,n}u\) climbs in temporal order, viscosity exposes successively
higher spatial energies of \(u\).
...
that, you get higher order Sobolev embeddings. Finite energy supplies the low
frequencies, and \(E_{n+1/2}\) supplies the homogeneous height, so for any
finite \(k\), choosing \(n>k+1\) gives
\[
u\in H^{n+1/2}(\mathbb R^3)
\hookrightarrow C_b^k(\mathbb R^3).
\]
Carrying every finite exposed height to one endpoint gives the whole insight at
once:
\[
...
\]
The temporal sequence that should have exposed increasing spatial roughness has
instead opened every finite spatial height and, through their common endpoint,
smoothness. This is one realization and one whole insight.
