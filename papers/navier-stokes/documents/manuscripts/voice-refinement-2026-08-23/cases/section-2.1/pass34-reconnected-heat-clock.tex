...
The \(\lambda^{-2}\) contraction can be read at width \(r\) from heat flow near frequency \(r^{-1}\). Each Fourier mode evolves by
\[
...
\[
  ...
  \frac{r^2}{\nu}.
\]
Here \(r\) records a concentration near frequency \(r^{-1}\), not yet the material radius of the vortex. If one solution develops concentration at that width, viscosity compares it on the time \(r^2/\nu\); identifying this spectral width with a material radius still requires a localization estimate. Halving the comparison width quarters the clock:
\[
  \tau_\nu(\lambda^{-1}r)
  =
  \lambda^{-2}\tau_\nu(r).
...
Repeatedly halving the spectral concentration width gives the comparison sequence
\[
  r_n:=2^{-n}r_0,
  \qquad
  \tau_n:=\frac{r_n^2}{\nu},
...
\[
  ...
  \infty.
\]
That calculation alone creates no singularity. It becomes relevant to the vortex only if one \NavierStokes{} solution develops concentration at the widths
\[
