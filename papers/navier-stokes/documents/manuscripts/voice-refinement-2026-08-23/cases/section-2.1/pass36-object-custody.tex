...
For concentration at width \(r\), viscosity makes an order-one change over the time \(r^2/\nu\). Let \(v_t=\nu\Delta v\) be the linear heat comparison. Near frequency \(r^{-1}\), each Fourier mode evolves by
\[
...
After \(n\) halvings, the width and its heat clock are
\[
  r_n:=2^{-n}r_0,
  \qquad
  \tau_n:=\frac{r_n^2}{\nu},
...
\[
  ...
  \infty.
\]
That finite sum alone creates no singularity. It applies to the narrowing vortex only if one \NavierStokes{} solution develops concentration at the widths
\[
...
\[
  ...
  \frac{4r_n^2}{3\nu}.
\]
The same velocity field would have to develop the next finer concentration, and every later one, within a remaining interval of order \(r_n^2/\nu\), which shrinks to zero with the width.
