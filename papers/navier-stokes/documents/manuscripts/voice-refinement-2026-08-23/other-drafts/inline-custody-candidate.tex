...
\]
A scale-localized velocity structure of width \(r\) occupies frequencies that are not \(|\xi|\simeq r^{-1}\), and the multiplier changes by order one when
\[
...
\[
  ...
  \infty.
\]
The finite sum belongs to the clocks. For it to describe a candidate singular history, one Navier--Stokes velocity field would have to exhibit scale-localized structures at the widths
\[
...
The shrinking intervals leave a question about the same velocity field at a fixed spatial point: how does its first time response connect to its successive time responses?
