...
Only a small part of the intersection is needed to contradict a finite endpoint.
The lowest adjacent pair reaches the critical norm that would have to blow up.
The remaining coordinates explain what becomes of the critical height, the shrinking vortex, the derivative Tower, and the terminal history once that contradiction is closed.
...
\begin{equation}\label{eq:EndpointCriterionFromLowRectangle}
  \sup_{0\leq t\leq T_*}\norm{u(t)}_3<\infty.
\end{equation}
This is where the finite maximal-time criterion introduced in Section~\ref{sub:ThePerfectlyStretchingVortex} is met by the first rectangle.
...
The lowest critical rectangle already puts the singularity and the estimate on the
same velocity, in the same norm, over the same terminal interval.

The base row has already closed the finite endpoint.  The remaining coordinates show
what the intersection carries with it once that endpoint is closed.  Fix
...
The contradiction already occurs in the base row: the continuation criterion requires
\(V_0=u\) to escape in \(L^3\), while the coordinate \((0,1)\) places that same
velocity continuously in \(H^1\hookrightarrow L^3\).
The temporal and spatial directions have met inside the row that carries the original
singularity question, and they give opposite conclusions.
...
\Cref{thm:BodyWholeTerminalRectangleTheorem} bounds every finite
pressure-complete rectangle of the mixed jet uniformly through \(I_*\).
The restriction identity \eqref{eq:BodyRectangleRestrictionCompatibility}
keeps their shared coordinates identical, so
\Cref{prop:CompatibleProjectiveAssembly} gives
\[
  u\in\bigcap_{r,m<\infty}H^r(I_*;H^m(\R^3)).
\]

Fix \(m\).
Taking \(r=1\) places \(u\) in \(H^1(I_*;H^m)\), and \Cref{lem:CompleteSpatialRowEndpoint} gives a limit
\(u_*^{(m)}\in H^m\) as \(t\uparrow T_*\).  If \(k>m\), the limits found in
\(H^k\) and \(H^m\) agree after the embedding \(H^k\hookrightarrow H^m\),
since both are limits of the same \(u(t)\).  The compatible limits define one
divergence-free endpoint
...
\end{equation}
This is the common endpoint constructed in
\Cref{lem:CompleteSpatialRowEndpoint}.

The same argument reaches every temporal row.  For finite \(N,m\), take
\(r=N+1\) in the intersection.  Then
\(V_N=\partial_t^Nu\in H^1(I_*;H^m)\), so it has a terminal trace
\(V_N^*\in H^m\); compatibility in \(m\) places \(V_N^*\) in \(\Hinfty\).
Passing to the limit in the pressure and velocity recurrences gives
\begin{align*}
  Q_N^*
  ...
  -\nabla Q_N^*.
\end{align*}
Thus the left-hand history reaches \(T_*\) with the actual normalized velocity
and pressure jet generated from \(u_*\), as proved in
\Cref{prop:PressureCompleteEndpointJet}.

Apply \Cref{lem:SmoothLocalRestart} at \(T_*\) with datum \(u_*\).  It gives a
smooth solution on \([T_*,T_*+\delta]\).  Its right-hand jet is generated from
the same \(u_*\) by the same normalized recurrences, so every derivative agrees
at the joining time.  The two pieces form one classical solution beyond
\(T_*\), and strong uniqueness identifies it with the history generated by
\(u_0\).  This contradicts maximality.

By \Cref{thm:CompatibleIntersectionClosure}, \(T_*=\infty\) and the normalized pair \((u,p)\) is smooth on every finite interval.
...
The energy identity proved in \Cref{prop:ClassicalKineticEnergyIdentity} and restated in \eqref{eq:BodyEnergyIdentity} holds on every finite
...
The finite-time contradiction is complete, but the bounded rectangles carry more information than continuation alone consumes.
They show how the same global history distributes vorticity across scales, where the classical criteria sit inside the grid, what becomes of the local singularity picture, and exactly why the argument stops when viscosity is removed.

\subsection{A finite sum across infinitely many scales}\label{sub:AFiniteSumAcrossScales}

The finite-time contradiction is already complete at the \((N,M)=(0,1)\) rectangle, where the \(L^3\) escape is blocked.
...
The finite rectangles exclude both descriptions on the same global history.
...
These are not extra hypotheses placed on the solution after global smoothness is known.
They are finite projections of the same compatible rectangles that closed the endpoint.
On every finite interval the solution has every finite mixed velocity and pressure norm requested above, while \Cref{prop:ClassicalKineticEnergyIdentity} keeps kinetic energy uniform for all time.
...
For fixed positive viscosity, the rectangle theorem bounds the finite coordinates exposed by that ascent on any putative terminal interval.
This rules out a finite last time.
On the resulting global history, every finite mixed coordinate is bounded on each finite interval, and \Cref{prop:ClassicalKineticEnergyIdentity} keeps kinetic energy uniformly bounded for all time.
