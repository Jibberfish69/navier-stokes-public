...
\]
A narrower comparison field can carry less kinetic energy and a larger first-derivative norm without changing \(H\). Finite energy is too low to decide the concentration question, because the scale that stays fixed is exactly the one that can still register the sharpening. That unchanged level is the critical height, and the next question belongs to one actual solution: does this height rise or fall as time advances?

\divider{Nonlinear Production versus Viscous Dissipation}

Its evolution is where the original competition reappears in exact form. At that level the nonlinear term contributes the signed production
\[
  \mathcal P_{1/2}[u](t)
  :=
  -\operatorname{Re}
