...
At a time \(t_0<T_*\), let \(\ell:=r_{\mathrm{eff}}(t_0)\). The exact \NavierStokes{} rescaling to the smaller width \(\lambda^{-1}\ell\) is
\[
  u_\lambda(x,t)
  :=
...
\end{aligned}
\]
All four terms gain the same factor, so scale alone gives viscosity no relative advantage.

\divider{Critical Height}

The equation keeps that balance while energy and gradient measurements move in opposite directions. Let \(\Lambda:=(-\Delta)^{1/2}\). Since
\[
...
\]
Energy falls, the gradient rises, and the half-derivative height stays fixed; finite energy still leaves concentration open.

\divider{Nonlinear Production versus Viscous Dissipation}

Scale-invariance still does not decide whether \(H\) can grow along one solution. At critical height, the nonlinear term contributes the signed production
\[
  \mathcal P_{1/2}[u](t)
  :=
  -\operatorname{Re}
...
\[
  ...
  \mathcal P_{1/2}[u](t).
\]
The original hope is now the sign of production minus dissipation. Under the same rescaling,
\[
...
\]
Their common \(\lambda^2\) factor keeps that balance fixed while time contracts by \(t\mapsto\lambda^{-2}t\).

\divider{The Heat Clock}

The heat equation pays for that contraction at width \(r\):
\[
...
\[
  ...
  \lambda^{-2}\tau_\nu(r).
\]
The viscous comparison time shrinks by the same square law that scaled production and dissipation together.

\divider{The Zeno Cascade}

Keep narrowing the same way. For
\[
  r_n:=2^{-n}r_0,
  \qquad
  \tau_n:=\frac{r_n^2}{\nu},
...
\[
  ...
  \infty.
\]
This finite sum is only an arithmetic schedule. For it to describe a candidate singularity, one \NavierStokes{} solution would have to pass through widths
\[
...
\[
  ...
  \frac{4r_n^2}{3\nu}.
\]
At the \(n\)-th width, both the next transition and the total time left are of order \(r_n^2/\nu\). The burden is now on that one history: each change of scale must occur on an interval of order \(r_n^2/\nu\), and those intervals must collapse to zero along the same solution as \(t\uparrow T_*\).
