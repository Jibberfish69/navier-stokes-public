...
The same rescaling changes Sobolev size by powers that separate energy from gradients.
...
That fixed height still has to evolve along one solution.
...
\[
  ...
  \mathcal P_{1/2}[u](t).
\]
The original hope is now the sign of production minus dissipation.
...
\]
Their common \(\lambda^2\) factor keeps the balance fixed while time contracts by \(t\mapsto\lambda^{-2}t\).
...
\[
  ...
  \lambda^{-2}\tau_\nu(r).
\]
Each finer width comes with a shorter viscous comparison time by the same square law.

\divider{The Zeno Cascade}

Applying that law repeatedly to
\[
  r_n:=2^{-n}r_0,
  \qquad
  \tau_n:=\frac{r_n^2}{\nu},
...
\[
  ...
  \infty.
\]
This finite sum is only an arithmetic schedule.
For it to describe a candidate singularity, one \NavierStokes{} solution would have to pass through widths
\[
...
What remains is the response of that one velocity history: each change of scale must occur on an interval of order \(r_n^2/\nu\), and those intervals collapse to zero along the same solution as \(t\uparrow T_*\).
