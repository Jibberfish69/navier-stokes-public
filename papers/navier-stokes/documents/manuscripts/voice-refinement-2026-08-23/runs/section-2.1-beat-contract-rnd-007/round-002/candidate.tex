...
\end{aligned}
\]
The narrower width has changed the scale of every term and left their balance where it was. What moves under this map is not the equation but the way norms register the field.
...
\]
Energy drops by one power of \(\lambda\), the first-derivative norm gains one, and between them one level does not move. Set
\[
  H(t)
  :=
...
\]
The narrowed comparison solution can carry less kinetic energy and a larger first-derivative norm while \(H\) stays unchanged. Finite energy does not close the concentration question at this width.

\divider{Nonlinear Production versus Viscous Dissipation}

That unchanged level has to be followed in time. At critical height, the nonlinear term contributes the signed production
\[
  \mathcal P_{1/2}[u](t)
  :=
  -\operatorname{Re}
...
\[
  ...
  \mathcal P_{1/2}[u](t).
\]
So \(H\) can rise only when nonlinear production exceeds viscous dissipation. Under the same rescaling,
\[
...
\]
The competition survives the narrowing again in the common factor \(\lambda^2\), so the width now comes with a shorter time on the same scale.

\divider{The Heat Clock}

For the viscous term, that time is read from the heat equation \(v_t=\nu\Delta v\):
\[
...
\[
  ...
  \frac{r^2}{\nu}.
\]
Shrinking the width to \(\lambda^{-1}r\) multiplies that clock by \(\lambda^{-2}\):
\[
  \tau_\nu(\lambda^{-1}r)
  =
  \lambda^{-2}\tau_\nu(r).
\]
The same contraction that accompanied the matched \(\lambda^2\) rates now appears as the viscous response time attached to the width.

\divider{The Zeno Cascade}

Repeat that narrowing dyadically. For
\[
  r_n:=2^{-n}r_0,
  \qquad
  \tau_n:=\frac{r_n^2}{\nu},
...
\[
  ...
  \infty.
\]
That finite sum is only the arithmetic. For these clocks to describe a candidate singularity, one \NavierStokes{} solution would have to pass through widths
\[
...
\[
  ...
  \frac{4r_n^2}{3\nu}.
\]
At the \(n\)-th width, both the next transition and the total time left are of order \(r_n^2/\nu\). One actual velocity history would have to keep traversing those shrinking widths on intervals of that size while the intervals themselves collapse to zero as \(t\uparrow T_*\).
