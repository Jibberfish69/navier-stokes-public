...
\end{aligned}
\]
The four transformed terms carry the same cubic factor, so viscosity gains no automatic advantage. The rescaled velocity field \(u_\lambda\) has Sobolev norms whose powers now separate.
...
The localization-width comparison does not select an active frequency range. Nontrivial activity near frequencies comparable to \(r_n^{-1}\) remains a further hypothesis, separate from ancestry and amplitude. The evolution times are separate as well. The flow must reach the sampled states at times
\[
  t_0<t_1<t_2<\cdots\uparrow T_*<\infty,
  \qquad
  t_{n+1}-t_n\asymp\tau_n,
\]
with comparison bounds independent of \(n\). The active-frequency hypothesis and these imposed gaps jointly make the linear heat scale relevant to this conditional history. The gaps also give
\[
  T_*-t_n
  \asymp
  \sum_{k=n}^{\infty}\tau_k
