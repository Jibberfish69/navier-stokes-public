...
At sampled times \(t_n\), the parcel radius \(r_{\mathrm{eff}}(t_n)\) and the rotation's transverse localization width must each lie between a positive lower multiple and a finite upper multiple of \(r_n\), with both comparisons uniform in \(n\). Under the parcel-radius comparison, the axial strain accumulated along the tracked trajectory satisfies \(\int_{t_0}^{t_n}\sigma(s)\,ds=2n\log 2\) up to an additive error bounded independently of \(n\). This cumulative relation states the amount of strain required for the parcel contraction without asserting exact halving on each interval.
