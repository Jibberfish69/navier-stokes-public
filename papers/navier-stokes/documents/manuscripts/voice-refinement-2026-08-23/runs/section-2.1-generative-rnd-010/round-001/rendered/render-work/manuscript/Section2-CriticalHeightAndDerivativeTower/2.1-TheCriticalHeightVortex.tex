...
The Zeno sum stops exactly there.
It leaves room inside finite time, but it does not put a singularity in that room.
Suppose the same solution nevertheless loses continuation at \(T_*\) through blow-up in one fixed continuation-controlling Banach space \(B\).
Before \(T_*\) the solution is still classical, so choose any \(t_0<T_*\).
Now imagine that some finite temporal response \(\partial_t^N u\), with \(N\geq1\), remains uniformly bounded in \(B\) on the rest of the interval.
Because \(T_*-t_0\) is finite, one integration keeps \(\partial_t^{N-1}u\) bounded.
Another keeps the next response down bounded.
Continuing in this way carries the bound through every lower response and finally to the same velocity \(u\), contradicting its blow-up.
A genuine singular history must therefore lose uniform control in \(B\) at every finite temporal order.

The \NavierStokes{} equation supplies the responses that this conclusion has made unavoidable.
