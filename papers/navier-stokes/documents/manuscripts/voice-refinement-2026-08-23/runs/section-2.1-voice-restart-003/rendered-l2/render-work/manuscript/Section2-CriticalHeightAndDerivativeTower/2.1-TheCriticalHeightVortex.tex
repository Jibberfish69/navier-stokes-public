...
\end{aligned}
\]
The common \(\lambda^3\) preserves the competition intact: at the finer width, viscosity has gained nothing on nonlinear sharpening.
...
\]
Concentration drives the neighboring levels apart while leaving \(H\) unchanged.
No subsequent motion of \(H\) can come from the shrinking ruler; it must come from the equation itself.
...
\]
Their common \(\lambda^2\) preserves the spatial balance and exposes the inverse contraction in the time coordinate \(t\mapsto\lambda^{-2}t\).
The balance has survived, but its clock has shortened.
