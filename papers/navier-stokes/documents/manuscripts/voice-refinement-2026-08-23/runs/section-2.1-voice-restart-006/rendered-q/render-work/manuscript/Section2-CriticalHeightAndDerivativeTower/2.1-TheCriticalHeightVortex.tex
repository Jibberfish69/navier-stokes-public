...
The effective cross-sectional area \(A(t):=\mathcal V/L(t)\) and effective radius \(r_{\mathrm{eff}}(t):=\sqrt{A(t)/\pi}\) therefore satisfy
\[
  \nabla\cdot u=0,
  \qquad
  \frac{d}{dt}(A L)=0,
...
\]
Stretching contributes the positive term \(\sigma(t)|\omega|^2\).
The viscous term has no fixed sign at an individual point; if viscosity is to stop the amplification there, it must offset that positive term.
When their sum remains positive along the material trajectory, \(|\omega|\) grows while \(r_{\mathrm{eff}}\) falls.
The kinetic-energy identity from \Cref{prop:ClassicalKineticEnergyIdentity} can keep the global energy finite at the same time:
\[
  \norm{u(t)}_2
  \leq
  \norm{u_0}_2
...
The rising rotational gradient is tied to a shrinking radius, so the same event must now be read at its scale.
