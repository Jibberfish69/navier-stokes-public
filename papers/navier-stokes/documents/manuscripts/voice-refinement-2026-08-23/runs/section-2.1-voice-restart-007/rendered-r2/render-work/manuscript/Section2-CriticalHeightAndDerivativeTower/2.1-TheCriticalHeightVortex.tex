...
On a short material segment with spatially uniform strain, let \(e(t)\) be its unit tangent in a positive strain direction:
\[
...
With \(A(t):=\mathcal V/L(t)\) and \(r_{\mathrm{eff}}(t):=\sqrt{A(t)/\pi}\), incompressibility gives
\[
  \nabla\cdot u=0,
  \qquad
  \frac{d}{dt}(A L)=0,
  ...
  -\frac{\sigma(t)}{2}.
\]
As the length grows, the effective cross-section falls and the vortex becomes narrower.
...
\[
  ...
  \partial_t+u\cdot\nabla.
\]
Along the tracked trajectory, the alignment \(\omega=|\omega|e(t)\) gives
\[
  S\omega
  =
  \sigma(t)\omega,
...
\end{aligned}
\]
Every term carries the same \(\lambda^3\), so the finer width gives viscosity no relative advantage over nonlinear sharpening.
Scale alone cannot decide the contest.
To see whether the original vortex is actually gaining strength, we need a measure the comparison itself cannot raise.
...
\[
  ...
  \mathcal P_{1/2}[u](t).
\]
The height rises only when nonlinear production exceeds viscous dissipation.
...
\[
  ...
  \infty.
\]
The clocks are summable, but comparison arithmetic is not one fluid history.
A candidate singularity would require one \NavierStokes{} solution to pass through widths
\[
