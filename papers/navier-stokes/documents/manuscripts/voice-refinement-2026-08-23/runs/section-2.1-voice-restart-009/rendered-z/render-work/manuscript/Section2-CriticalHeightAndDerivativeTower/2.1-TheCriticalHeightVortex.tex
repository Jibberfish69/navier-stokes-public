...
The surrounding flow can pull a vortex along its axis.
A material segment of that vortex has an axial direction \(e\) and length \(L(t)\).
The symmetric strain
\[
  S:=\frac12\bigl(\nabla u+(\nabla u)^{\mathsf T}\bigr)
\]
has an aligned positive direction when
\[
  Se=\sigma(t)e,
  \qquad
  \sigma(t)>0,
...
Incompressibility forces the cross-section to pay for that lengthening.
Let \(\mathcal V\) be the fixed material volume of the segment, set \(A(t):=\mathcal V/L(t)\), and write \(r_{\mathrm{eff}}(t):=\sqrt{A(t)/\pi}\).
Then
\[
  \nabla\cdot u=0,
  \qquad
...
The axial strain also acts on the rotation carried by that segment.
With \(\omega:=\nabla\times u\),
\[
  \frac{D\omega}{Dt}
  =
...
Viscosity is already in the same balance; it has to overcome that positive stretching term at the place where the radius is falling.
When it does not, the segment lengthens, the radius falls, and the rotation strengthens.
\[
  \frac{|\omega|}{\sqrt2}
  =
...
The falling radius is now the scale in the problem.
...
This is another solution at a different scale, not a later state of the original vortex.
...
The equation keeps the contest balanced under rescaling, but the usual sizes of the velocity do not behave the same way.
...
At that fixed height, actual growth has to come from the solution itself.
...
Viscosity has a natural time at each width.
...
The clock shrinks with the scale; it does not give viscosity a new advantage over the sharpening.
...
\[
  ...
  \frac{4r_n^2}{3\nu}.
\]
Under that schedule, each new width is formed inside an interval tending to zero at order \(\tau_n\).
