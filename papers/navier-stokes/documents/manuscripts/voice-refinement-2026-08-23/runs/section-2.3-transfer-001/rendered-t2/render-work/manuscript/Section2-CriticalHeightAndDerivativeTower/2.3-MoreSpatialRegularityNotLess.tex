...
Different rectangles already agree wherever they overlap: every shared coordinate is the same derivative of the same \(u\).
What is missing is not compatibility but reach.
For each fixed rectangle, one bound must survive every strict cutoff \(T<T_*\); its size may depend on \(N\) and \(M\), but not on \(T\).
Section~\ref{sec:TheIntersectionInsideTheTower} isolates exactly that whole-terminal burden.
If it can be paid, the compatible rectangles reach the terminal interval together, and their intersection contains every finite time--space coordinate.
