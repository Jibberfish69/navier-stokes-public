...
At a single endpoint it blocks classical compatibility, but changing one point does not change a spacetime distribution and cannot by itself prove \(\operatorname{Exit}(Q)\) under the distributional membership test.
...
At every regular time before \(T_*\), the complete velocity field at that time can be used as new initial data for the same forward evolution.
