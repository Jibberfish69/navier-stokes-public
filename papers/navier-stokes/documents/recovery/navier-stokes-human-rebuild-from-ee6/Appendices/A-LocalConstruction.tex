...
The nonlinearity and the heat equation first force three adjacent rows.  The
construction must absorb one derivative of \(w\otimes w\).  In three
dimensions,
\[
...
Those three rows dictate the linear estimate.  On
\(J_\delta=[t_0,t_0+\delta]\), \(0<\delta\leq1\), consider
\[
  z_t-\nu\Delta z=F,
  \qquad
  z(t_0)=a,
...
With the linear estimate aligned to the nonlinear force, set
\[
  X_\delta
...
The fixed point has so far been written through the Leray projection.  To
restore the pressure to this same velocity, set
\begin{equation}
  q:=R_iR_j(w_iw_j).
...
\(L^2\) function with zero gradient and must coincide.  The uniqueness
already proved is consequently uniqueness of the pair \((w,q)\).

The same clock must now carry every higher derivative already present in the
datum.  Fix an integer \(m\geq3\) and temporarily take \(a\) smooth.  Differentiate the
...
The local pieces can now be joined without changing the solution.  Begin at
\(t_0\) and extend through every interval allowed by the construction.
