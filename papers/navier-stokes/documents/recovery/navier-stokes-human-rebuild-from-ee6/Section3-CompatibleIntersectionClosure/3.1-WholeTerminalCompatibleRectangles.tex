...
The preceding lemma permits exactly that growth.  The whole-terminal condition
in \Cref{ass:BodyWholeTerminalRectangleCondition} is the additional input: after
a finite \((N,M)\) is chosen, its bound is independent of the cutoff.  For
...
Assume \Cref{ass:BodyWholeTerminalRectangleCondition}.  For every finite
\(N,M\), the pressure-complete history satisfies
...
\]
Each strict integral is one nonnegative term in the assumed cutoff-independent
bound.  Its limit is finite, and
\[
  V_n\in L^2(I_*;H^{M+1}),
  \qquad
...
Compatibility concerns this identity; terminal boundedness is the separate
quantifier supplied by \Cref{ass:BodyWholeTerminalRectangleCondition}.
...
Under \Cref{ass:BodyWholeTerminalRectangleCondition}, every finite \(n,m\)
has
