...
% ============================================================
% 1.2 Earlier Attempts and the Present Frontier
% ============================================================
\subsection{Earlier Attempts and the Present Frontier}\label{sub:HistoricalEstimates}
...
\[
  \nu\int_\Omega \Delta u\cdot u\,dx
  =
  -\nu\lVert\nabla u\rVert_{L^2}^{2}.
\]
...
The energy identity therefore leaves room for derivative concentration near a finite time.
...
\]
holds for divergence-free fields with the decay or periodicity assumed here.
...
\]
with the usual decay normalization on \(\mathbb R^3\) or zero-mean normalization on \(\mathbb T^3\).
...
At a singular point, every sufficiently small cylinder must therefore retain a definite amount of scale-invariant concentration.
...
The global energy ceiling therefore gives no scale-independent ceiling on the critical norm.
...
The present theory has located the possible failure with remarkable precision.
It says what must diverge, where concentration must persist, and which continuation bounds would exclude it.
What remains unavailable is an estimate derived from arbitrary smooth data, fixed viscosity, and the equations that keeps one continuation-grade norm finite through every candidate endpoint.
