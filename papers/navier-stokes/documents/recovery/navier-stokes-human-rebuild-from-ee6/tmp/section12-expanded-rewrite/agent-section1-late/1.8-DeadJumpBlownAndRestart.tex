...
\]
At the base rung, this is simply the statement that material acceleration answers the pressure and viscous field:
\[
  D_tu=-\nabla p+\nu\Delta u.
\]
...
This sequence is a kinematic picture and does not construct a Navier--Stokes solution.
...
The three names organise visible failures of lawful response, compatible limiting value, and finite size without forcing every singular geometry into one exclusive box.
...
\[
  ...
  v(0)=u(t_0).
\]
The entire time slice \(u(t_0,\cdot)\) has become the new initial datum.
A single spatial point cannot serve that role.
