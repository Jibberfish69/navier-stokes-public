...
The energies \(E_s\) are homogeneous high-frequency measurements.
For every \(s>0\), the inhomogeneous Sobolev norm used for continuation satisfies
\[
  \norm{u}_{H^s}^2
  \simeq
  \norm{u}_2^2
  ...
  \norm{\Lambda^su}_2^2,
\]
so the Leray energy controls the low-frequency term and \(E_s\) supplies the corresponding high-frequency height.
