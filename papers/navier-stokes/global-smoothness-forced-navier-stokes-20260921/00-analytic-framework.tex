...
\begin{equation}
 \norm{(-\Delta)^{-1}\nabla\cdot h}_{H^m}
 \le C_m^{\rm fp}\bigl(\norm h_{L^1}+\norm h_{H^m}\bigr).
 \label{eq:force-pressure-potential-bound}
\end{equation}
...
\eqref{eq:force-pressure-potential-bound}.  Lemma~\ref{lem:sobolev-product} gives
...
source \(r\mapsto f(s+r)\).  It is therefore enough to write the proof for
\(s=0\).
Let \(0<\delta\le1\), let
\[
 z_t-\nu\Delta z=F,
...
\eqref{eq:restart-quadratic-bilinear} places the quadratic momentum term in
...
\begin{equation}
 ...
 \label{eq:restart-truncation-uniform-H3}
\end{equation}
...
\eqref{eq:restart-truncation-uniform-H3},
\[
...
{}+2\nu\int_0^t\norm{\nabla w^{(k)}(s)}_{H^m}^2\dd s
 ...
 \label{eq:restart-truncation-uniform-Hm}
\end{equation}
Since
\(
 \norm{w^{(k)}}_{H^{m+1}}^2
...
\(
 q=R_iR_j(w_iw_j)-(-\Delta)^{-1}\nabla\cdot f
\)
and Lemma~\ref{lem:mixed-leibniz} give
