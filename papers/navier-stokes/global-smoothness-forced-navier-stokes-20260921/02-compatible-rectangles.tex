...
jet.  The first identifies compatible coordinates of one forced history.  The
second uses the viscous tether in the complete equation to estimate every
selected coordinate.  We now derive the rectangle identities, including the
nonlinear, pressure, and force faces, and carry their finite estimates to the
whole terminal interval.
...
Definition~\ref{def:finite-rectangle}.  Its finiteness is the output of the
complete-equation estimate in Theorem~\ref{thm:datum-rectangles}.  The next
proposition identifies its exact value by monotone exhaustion and records the
equivalent coordinate memberships.
