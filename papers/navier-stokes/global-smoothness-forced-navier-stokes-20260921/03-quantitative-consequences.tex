...
one prescribed force throughout. The datum-generated whole-terminal
intersection and same-force continuation transfer, after positive-time
smoothing, to the strong \(H^1\) class for that force.
...
\end{samepage}
\begin{samepage}
The integral inequality in \eqref{eq:qc-H1-ode}, with the force contribution
...
regularity iteration in Lemma~5.5 of \cite{Tao2013}, after viscosity
rescaling with interior time \(h/2\), applies to the inhomogeneous mild
formula as well: each additional source term is bounded directly by the
displayed smooth-force norms.  The same heat-semigroup/product iteration
therefore gives, for every \(m\in\N\), with
\[
 \Gamma_{m,S}(f):=
...
\(H^\infty_\sigma\).  Theorem~\ref{thm:datum-intersection} applies to that
smoothed datum and the same shifted prescribed force.  Therefore
Theorem~\ref{thm:ier-global-smoothness}, with time origin \(t_0\),
supplies a global smooth history from that datum with the
same absolute-time force \(f(t)\).  The
...
In the terminal alternative \(T=T_*<\infty\), the whole-terminal representatives
are those of
...
Definition~\ref{def:qc-producer-constants} is fixed for \((u_0,\nu,f)\).
Thus
...
\begin{align}
 ...
 &:=\sup_{0\le t\le T}\norm{F_n(t)}_{H^m},
 \label{eq:qc-force-row-majorants}
\end{align}
...
\begin{align}
 ...
 \label{eq:qc-force-pressure-majorants}
\end{align}
Every one of these coordinates is finite by the prescribed force class and
\eqref{eq:force-pressure-potential-bound}.  They belong to the same forced
...
The force-row identity in \eqref{eq:qc-row-viscosity-scaling} makes the
force term in \eqref{eq:qc-Drec} homogeneous of the same degree as
\(V_{n+1}\).  Equation \eqref{eq:qc-initial-jet-viscosity-scaling} therefore
follows by induction in \eqref{eq:qc-D0}--\eqref{eq:qc-Drec}.  The amplitude relation
\(u(t)=\nu v(\nu t)\) proves
...
of the fixed datum-generated history.  The whole-terminal intersection and
the rectangle theorem first establish the finite rows.  Equations
\eqref{eq:qc-height-spatial-bounds} and
\eqref{eq:qc-B-by-height-reconstruction} then express that same finite
information in rectangle and height coordinates, respectively.
\end{proposition}
...
with \(\Pp F_n\) retained and \eqref{eq:qc-product-constant} give the next velocity row through
...
finite rectangle-matrix suprema in \eqref{eq:qc-B-matrix-identity} and the
...
\end{equation}
Equation~\eqref{eq:qc-product-constant} at order \(m+1\), with
\(\Pp F_n\) retained and evaluated at
\(t=0\), proves \eqref{eq:qc-D-bound} by induction through
\eqref{eq:qc-Drec}.  This estimate, taken pointwise on \([0,T]\),
...
\begin{equation}
 ...
 \label{eq:qc-Y-bound}
\end{equation}
The base case \(m=0\) is the forced estimate \eqref{eq:energy-Linf}.  For \(m\ge1\), both
...
special forced-energy cases follow from \eqref{eq:energy-Linf} and
...
\eqref{eq:qc-mixed-low-modulus}.  Applying this argument to the rectangle
...
follows from \eqref{eq:qc-pressure-sup} and Sobolev embedding, including the
...
rectangle matrix and the three scalar rectangle coordinates by
\begin{align*}
 \mathbf C_{N,M}(T_*)&:=
...
\begin{align}
 ...
 \label{eq:qc-closed-force-column}
\end{align}
\end{proposition}
...
the third column through \eqref{eq:qc-Q1}.  The fourth column is the
prescribed-force budget itself, bounded by
\eqref{eq:qc-force-row-majorants}--\eqref{eq:qc-force-pressure-majorants}.
...
the forced pressure bound \eqref{eq:qc-pressure-L1-sharp} at order
\(s+r_1+|\alpha|\) gives
...
\eqref{eq:qc-critical-current}.  Finally,
\[
...
\eqref{eq:qc-vorticity-action}.  The forced \(H^m\) energy identity and
...
\end{equation}
Define the same-force local coordinate
\[
 \Gamma_T:=\norm{\Pp f}_{L^2(T,T+1;H^2)},
\]
and define
\begin{equation}
 \delta_{\rm qc}[u_0,\nu,f;T]
 :=\min\left\{
 1,
 c_\nu\bigl(1+\mathfrak Z_{0,3}[u_0,\nu,f;T]+\Gamma_T\bigr)^{-2}
 \right\}.
 \label{eq:qc-restart-lifespan}
\end{equation}
The class \(\mathscr M_{\nu,f}^3(T,T+\delta_{\rm qc};u_T)\) contains exactly one
pair \((w,q)\), driven by the same absolute-time force \(f(t)\).  It is the original history's continuation constructed in
...
\begin{equation}
 ...
 \le\left(
 \mathfrak Z_{0,m}[u_0,\nu,f;T]
 +\norm{\Pp f}_{L^1(T,T+\delta_{\rm qc};H^m)}
 \right)^2
...
\[
 \delta_{\rm qc}[u_0,\nu,f;T]
 \le\min\{1,c_\nu(1+\norm{u_T}_{H^3}+\Gamma_T)^{-2}\}.
\]
Theorem~\ref{thm:local-restart} supplies the unique pair
\((w,q)\in\mathscr M_{\nu,f}^3(T,T+\delta_{\rm qc};u_T)\), and the fixed-point
ball in its proof gives \eqref{eq:qc-restart-base-size}.  Hence
\[
 ...
 \le\mathfrak A_T.
\]
The forced \(H^m\) energy estimate in
Proposition~\ref{prop:qc-continuation-norms}, now on the restart interval,
with \(\norm{u_T}_{H^m}\le\mathfrak Z_{0,m}\) and the same source
\(f(t)\), proves
...
\begin{align}
 ...
 \label{eq:qc-force-work-row-majorants}\\
 \mathfrak C_{j,\ell}^{[1]}
...
\begin{align}
 ...
 \label{eq:qc-sharp-force-work-row-family}\\
...
the rectangle coordinates, this is a re-expression of the finite information
already carried by the original history.
\begin{samepage}
This is the exact coordinate-level catalogue for general data: every
downstream quantity is expressed through the finite rectangle selected by its
displayed indices.\par
\end{samepage}
...
\eqref{eq:qc-sharp-pressure-endpoint}, with the corresponding
...
\eqref{eq:qc-sharp-energy-row-family}.  Leibniz' rule applied to
\(\mathcal W_{j+1/2}\) gives the majorants
\eqref{eq:qc-force-work-row-majorants}.  Differentiating the forced identity
\[
...
Lemma~\ref{lem:transport-commutator}, with the prescribed force retained,
...
\[
 ...
 +\norm{\Pp f}_{H^m}.
\]
The inhomogeneous Gronwall inequality and \eqref{eq:qc-sharp-lipschitz-action} prove
