...
\begin{equation}
 ...
 \label{eq:app-small-force-class}
\end{equation}
Set
\[
 a_{\rm c}:=\norm{\Lambda^{1/2}a}_2,
...
\begin{align}
 ...
 \label{eq:app-small-higher-force-data}
\end{align}
Then
\begin{align}
...
\([0,T_*)\).  Integration and the definition of \(\delta_f\) prove
...
\eqref{eq:app-critical-action-identity}, use
\(\norm{\mathcal B(u)-g}^2\le
(1+\eta)\norm{\mathcal B(u)}^2
...
Lemma~\ref{lem:app-critical-commutator}, and Young's inequality yield
\[
 \frac{\dd}{\dd t}\norm{\Lambda^mu}_2^2
...
\[
 \delta_{\rm crit}
 :=\min\left\{1,
 c_\nu\left(1+M_3+\norm g_{L^2(0,\infty;H^2)}\right)^{-2}
 \right\}>0.
\]
...
\(t_0<T_*\) with \(T_*-t_0<\delta_{\rm crit}/2\).  Both restrictions belong
 to \(\mathscr M_{\nu,f}^3(t_0,b;u(t_0))\) for every \(b<T_*\), so uniqueness on
...
the force norms in \eqref{eq:app-small-force-class} and
\eqref{eq:app-small-higher-force-data}, and the finite family
\[
...
\eqref{eq:app-small-time-row} for \(m\ge1\); the forced energy estimate
...
\eqref{eq:app-small-pressure-rectangle}, with the force-pressure potential
...
Theorem~\ref{thm:ier-global-smoothness} from \((u_0,\nu,f)\), and let
\(0<T<\infty\).  Then
\begin{align}
...
Theorem~\ref{thm:ier-global-smoothness} for \((u_0,\nu,f)\).  This is a post-regularity
...
\eqref{eq:app-vorticity-dissipation}.  Since
...
upper bound in \eqref{eq:app-flow-bilipschitz}.  Solving this equation
...
forced Leray--Hopf solution in Definition~\ref{def:app-leray-hopf}, with
...
global strong history from Theorem~\ref{thm:ier-global-smoothness} for
...
\begin{proof}
Combine \eqref{eq:app-leray-energy}, the forced strong energy equality
\eqref{eq:ier-global-energy}, and \eqref{eq:app-cross-energy}.  The two
...
\]
The derivation of \eqref{eq:app-relative-energy-action} uses only the smooth
...
from Theorem~\ref{thm:app-small-critical-global} gives the stated explicit
...
Theorem~\ref{thm:ier-global-smoothness}.  Thus \(v\) has
the smooth
representative
\[
