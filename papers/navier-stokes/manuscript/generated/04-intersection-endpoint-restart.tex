...
\FrontMatterProofNotice{Theorem~\ref{thm:compatible-inverse-intersection}}
...
\end{equation}
and all of its finite coordinates are the restrictions of the original pressure-complete history.  Its coordinates \((V_n,Q_n)_{n\in\Nzero}\)
satisfy
\begin{align}
...
Fix \(n\).  Every rectangle with \(N\ge n\) contains the fixed distributions
...
one distributional coordinate in its own Sobolev topology.
...
\[
 ...
 \qquad(0\le n\le r).
\]
They are the distributional time derivatives of the fixed \(u\), so
