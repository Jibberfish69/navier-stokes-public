...
## Verified claim ladder

The direct proof is complete:

u_0 -> datum-generated whole-terminal compatible rectangles -> projective all-orders
intersection -> common H-infinity endpoint -> pressure-explicit endpoint jet ->
same-history restart -> T_*=infinity.

Every arrow is established for the one fixed-viscosity, pressure-complete
history. The first signed critical prefix is a downstream equivalent readout,
not a second gap. The detailed proof and claim ownership are in
[PROOF-PROGRAM.md](PROOF-PROGRAM.md).

## Silver classification

Silver retains the one-line VPI-complement contrapositive:
...
\[
\operatorname{VPIParticipation}(Q)\Rightarrow
\operatorname{Member}(Q)\Rightarrow
\operatorname{Smooth}(Q).
\]

Dead, Blown, Jump, Part, Field, and any conditionally selected other PDE
classify participation loss. The line does not prove that full VPI
participation persists at a candidate terminal time, so it does not close
global regularity.
...
heat-rate scale associated algebraically with its global outer half-height
pair-separation scale, while terminal doubling intervals shrink on the
corresponding parabolic scale. That scale is not a localized ripple width or
material collar. The scale product is the already-installed fixed action
quantum, so the faster rate supplies no stronger cumulative budget. Exact
globally smooth triangular data have an
...
visible crest can decrease while global critical height rises. Crest count or
height and phase efficiency therefore cannot by themselves close Gold; this
does not prove or refute a general spatial maximum-spikiness theorem. Gold
...
\(J_{\mathrm{pair}}\). Cycle 037 proposed a parent-announced selected
shape-action submeasure with shape-null routing, proved same-event ancestry,
bounded fibres, and the separate WLF.60 datum-finite parent bound. Cycle 038
below withdraws that proposed strict reduction. Silver is unchanged.

Eighth live gate result, 2026-07-12: Cycle 038 installs ontology F6.17. For a
one-shell state \(\Lambda v_0=\lambda v_0\), the present shape and
\(J_{\mathrm{pair}}\) vanish, yet

\[
\partial_t\!\left[\Lambda^{1/2}(\Lambda-m_H)v\right]_{t=0}
=-\Lambda^{1/2}(\Lambda-\lambda)B(v_0).
\]

The exact datum \(v_0=(\sin z,\sin x,\sin y)\) sends its pressure-completed
source from shell \(1\) to shell \(\sqrt2\). The later positive critical
current is linear in time and \(J_{\mathrm{pair}}\) is quadratic, so a
backward WLF detector reads order \(\tau\) while
\(\int_0^\tau J_{\mathrm{pair}}dt=O(\tau^3)\). More generally, the linear
pair current has density \(|\eta_H|d\sigma_Bdt\) while the pair action has
density \(\eta_H^2d\sigma_Bdt\); deleting only \(\eta_H=0\) leaves the
unbounded near-null ratio \(1/|\eta_H|\). Instantaneous shape-nullity is not a
historical null class. A direct datum bound on \(\int J_{\mathrm{pair}}dt\)
remains Gold-strength. The WLF route must retain full signed first-admission
source work and prove its propagation to later unpaid shape reads, the
selected linear near-null bound, causal bounded fibres, and WLF.60 parent
...
| 5 | The pass branch remains the lawful smooth continuation branch, while the fail branch has no legal in-class nonsmooth continuation status | `submission-bundle/submission-synopsis.md`, `submission-verdict.yaml` | historical submission-route classification | blocks the stale positive-no-exit or periodic-opening rewrite |
...
- The current direct Gold edges are
  ...
  Gold-strength routes, not the sole current Gold burden. They do not affect the
  accepted Silver VPI-complement proof stated at the top of this file.
...
- Navier--Stokes global regularity is proved on the Silver lane by the exhaustive participation-complement implication and its direct contrapositive. Part, Field, Dead, Blown, Jump, and any conditional other-PDE destination locate the negative-side loss; they are not extra closure theorems. Gold remains a parallel direct research lane and is not a Silver premise.
...
- Silver mechanism: on the one smooth-data, fixed-positive-viscosity, no-reset original history, exhaust `not Smooth(Q) => not Member(Q) => not VPIParticipation(Q)` and contrapose directly to `VPIParticipation(Q) => Member(Q) => Smooth(Q)`.
- Cross-lane braid, periodic, and source-history imports may sharpen negative-side classification or independent Gold research, but they do not replace the closed Silver implication. Part and Field only localize participation loss; Pack remains outside CM.

## Two-Lane Route

- `Gold`: direct regularity route
  - same-datum same-interval Navier-Stokes controlled evolution for the shared object
- `Silver`: VPI-complement contrapositive, closed
  - same smooth-data, fixed-positive-viscosity, no-reset original Navier--Stokes history under full VPI participation
  - exhaustive negative side `not Smooth(Q) => not Member(Q) => not VPIParticipation(Q)` and direct contrapositive `VPIParticipation(Q) => Member(Q) => Smooth(Q)`
  - Dead, Blown, Jump, Part, Field, and a defect-free conditional other-PDE identification only present participation loss; Pack is Field-window evidence outside CM
...
- Sidecar route material is support only until it lands in the independent Gold proof or classifies full VPI nonparticipation on Silver's negative side. Part and Field only localize that loss; Pack and packet-window material remain Field-window evidence outside CM.
- The projected-flow and periodic packages do not set the manuscript opening by themselves.
- The live paper surface starts from the original Navier--Stokes history, states closed Silver directly, and keeps Gold as independent nonblocking research.

## First Unsafe Overclaim

- Replacing the closed Silver VPI-complement contrapositive with a terminal-witness tree, a positive no-exit theorem, a Gold prerequisite, or the stale torus-first / `Q(t)` / periodic-opening story.
