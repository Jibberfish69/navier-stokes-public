...
- The current Gold burden is the signed-height owner-and-charge law: before selected readout, the same signed-height record work must have one parent-known owner map and one original-history charge measure. After pre-return address admission/lift exists, the owner map is formal; the proof-sized burden is one finite pre-readout boundary measure \(\Gamma_{\rm pre}\) from the original packet, with address/boundary and selected-critical root-payment faces. The address/boundary face now reads as stopped address admission/lift plus same-event first-boundary charge. Return first-address, finite return multiplicity, and selected-return no-duplication are downstream of that same owner process. This is a strict reduction toward `SignedCriticalHeightSignPersistenceOrPeakBound.A`, not a proof of it from original data.
