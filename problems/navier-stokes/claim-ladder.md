...
- The current Gold burden is the signed-height owner-and-charge law: before selected readout, the same signed-height record work must have one parent-known owner map and one original-history charge measure. After pre-return address admission/lift exists, the owner map is formal; the proof-sized burden is one finite pre-readout boundary measure \(\Gamma_{\rm pre}\) from the original packet, with address/boundary and selected-critical root-payment faces tied to the same event key. The address/boundary face reads as stopped address admission/lift plus same-event first-boundary charge, with the parent-currency off-face entry reduced to simultaneous same-parent active-critical/full-exchange/active-transit storage with spectral radius below one, or an independent original-data proof of one storage face. The checked unpaid row in that storage wall is WLF.60: finite parent-weighted selected first-admission pulse measure with strict viscosity feedback, now reduced to one same selected native carrier for the lower fresh source and upper selected positive queue, with selected overrun paid there. Stopped potential-average control and positive second-order scale exactness are analytic faces of that same carrier key. Return first-address, finite return multiplicity, and selected-return no-duplication are downstream of that same owner process. This is a strict reduction toward `SignedCriticalHeightSignPersistenceOrPeakBound.A`, not a proof of it from original data.
