...
The natural Euler carrier is:

```math
\Xi_N^E[u](t) := \sum_{j\ge N}\|P_j u(t)\|_{L^2_x}^2,
```
...
\int_0^t \sum_{j\ge N}
\left|
\left\langle P_j((u\cdot\nabla)u), P_j u \right\rangle
\right| ds.
```
...
```math
\boxed{
\text{control }\Phi_N^E[u]\text{ on Euler terms alone, without absorbing it
into viscous dissipation.}
}
```
