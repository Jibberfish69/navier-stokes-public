...
More sharply, on the current MPP object-law refoundation, the ontic side now
supports the following reduction:

```text
no downstream branch introduces a new primitive failure family beyond
class-exit from shared packing / participation / one-field coherence,
```

and the exact remaining exhaustiveness wall is not "find a fifth family,"
but:

```text
collapse the two currently visible terminal survivor families to one common
terminal survivor class.
```

That reduction is recorded explicitly in
