...
| tower / frozen family to source ledger | packet source terms, pressure/strain/cutoff ledgers | `FFSRC.A`, `FCI.5f`, with exact pressure wall `FPCR.C` kept visible and packet-local `FFPB.A0` solved on the stronger branch | translate tower source control into the fixed-family ledger without spending receiver data | useful source-side bridge | source closure cannot be asserted independently of collar data |
...
| --- | --- | --- | --- | --- |
| Route-native `OFP` loop | executable live-edge closure | `FFSRC.A` or exact `FCI.5f` plus `LCI.A` / receiver closure -> `CSP.A` -> `OFP.A` -> `CFI.A` | solved packet-local `FFPB.A0` or exact pressure wall path; `RCF.A` / `LCI.A`; thickened collar consumer; widened receiver return | highest |
