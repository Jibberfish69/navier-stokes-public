...
> **Frozen detailed snapshot (2026-07-14):** this preserves the complete
> pre-consolidation ontology through Cycle 154. It is read-only evidence and
> detail authority; future derivations belong in `theorem-construction/`, and
> future promoted ontology cards belong in `../ontology.md`.
...
Write the complete radial marginal of the signed chord law in conservative
variables
...
Here \(m,g\ge0\), while positive \(J\) is inward pressure-completed chord
current.  Every scale--time adjoint satisfies

\[
\boxed{
\iint(-\psi_r)J
=\left[\int\psi m\right]
-\iint\psi_t m
+2\nu\iint\psi g.
}
\]
...
\boxed{
\begin{aligned}
\int_{t_0}^{t_1}\int_0^{L(t)}\frac{J}{r^4}\,drdt
={}&\left[\frac13\int_0^L(r^{-3}-L^{-3})m\,dr\right]_{t_0}^{t_1}\\
&-\int_{t_0}^{t_1}\frac{L'}{L^4}C_L\,dt
+\frac{2\nu}{3}\int_{t_0}^{t_1}\int_0^{L(t)}
(r^{-3}-L^{-3})g\,drdt,
\end{aligned}
}
\]

where \(C_L=\int_0^Lm\,dr\).  The selector term is unavoidable: annular
adjoints move it to shell boundaries, and their sum reconstructs this unique
critical tent.  The energy-scale ceiling \(C_L\le C\mathcal E_0L^3\) gives
only

\[
-\int\frac{L'}{L^4}C_Ldt
\le C\mathcal E_0\int-d\log L,
\]

which is logarithmically unbounded across infinitely many scales.

The same conclusion is visible in every moving profile.  For
\(T_\Phi=\int\Phi(r/L)m\), its scale derivative \(Q_\Phi\), dissipation
\(G_\Phi\), and joined current \(J_\Phi\) obey

\[
\boxed{
T_\Phi'+2\nu G_\Phi+left(-\frac{L'}L\right)Q_\Phi=J_\Phi.
}
\]

The sharp normalized stock \(E(t,L)=C_L/L^3\) therefore satisfies
...
=\frac d{dt}E(t,L(t))
+2\nu\frac{G_L}{L^3}
-\frac{L'}L\partial_{\log L}E(t,L).
}
\]

The scale-shape term has no sign.  At fixed dyadic radii, the strongest
signed telescope lands on
...
the critical incidence and Fisher objects themselves, rather than on a
kinetic-energy reserve.  Taking positive record parts before telescoping
destroys the shell cancellation because different radii contact their records
at different times.

This loss is realized exactly within the radial conservation--dissipation
class.  Let \(\lambda(t)\downarrow0\), choose smooth \(h\ge0\) with
\(h(z)=z^4\varphi(z)\) near zero, and set \(z=r/\lambda(t)\):

\[
\boxed{
\begin{aligned}
m&=c\mathcal E_0\lambda^2h(z),\\
J&=-c\mathcal E_0\dot\lambda\lambda^2zh(z),\\
g&=-\frac{3c\mathcal E_0}{2\nu}\lambda\dot\lambda h(z).
\end{aligned}
}
\]

Then \(J,g\ge0\) and \(m_t-J_r=-2\nu g\) exactly.  It has regular positive
zero-chord trace, vanishing radial boundary flux, the energy-scale stock
envelope, and finite total viscous loss.  Nevertheless, for every sufficiently
small crossed radius,

\[
C_R\le C\mathcal E_0R^3,
\qquad
\mu_R\ge c_h\mathcal E_0R^3,
\]
...
\[
\boxed{
\sum_nR_n^{-3}\mu_{R_n}=\infty,
\qquad
\int_0\mu_RR^{-4}dR=\infty.
}
\]

This counterflow is not asserted to be a signed coboundary or a
Navier--Stokes realization.  It identifies the exact information erased by
radial scalarization: whether all those scale currents can be generated by
one center-dependent, direction-dependent incompressible coboundary
\(V_{xy}+V_{yz}=V_{xz}\) with its slaved pressure.  Any history payment that
distinguishes the fluid from this counterflow must therefore use that retained
