...
- Gold affine/current absorption: the affine Schur/current audit removes the fake repeated-perpendicular channel. First parent-current entry is Bessel novelty, and retained same-coordinate production is Schur work only after admission. The live reduction for `(ODP.160)` is therefore predictable active weight plus lifted martingale/Bessel square-packing for the stopped affine transition pile before positive readout; if the branch reaches terminal fixed-core concentration, terminal no-atom/no-jump reduces to root-fixed terminal-uniform \(L^1\) full-packet service storage. Both are faces of the same WLF/ODP early-row payment, not new sibling frontiers.
