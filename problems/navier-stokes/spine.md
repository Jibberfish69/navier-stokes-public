...
| 068 | Leave the passive parallel-column null class and ask, in the same fixed-viscosity participant, which successive physical events turn moving nonlocal orientation into actual critical-height rise. | Exact strict hierarchy: Cycle 067 frame rotation with \(r=P_H=0\); ABC local \(r>0\) with global \(P_H=0\); pure-2D triad with \(P_H>0\) but \(P_H\le\nu D\); and the same triad above \(a_*=\nu(303+137\sqrt5)/2\) with \(P_H>\nu D\) and \(\mathcal H_{1/2}'>0\). Velocity reversal preserves \(\mathsf T_u,\tau,\Pi\) and quadratic state sizes at the compared snapshots while reversing \(S,r,P_H\). | Active work begins only when the simultaneous strain sees the nonlocal all-chord orientation in overlapping tensor directions. Its local positive read must survive cancellation by the surrounding field and then exceed the viscosity acting on the same relative-velocity structure. The exact rising witness is globally regular and two-dimensional: pressure remains the forced nonlocal same-field response, viscosity acts simultaneously, and \(S\omega=0\) throughout. | The theorem constructs no packet, annulus, core, carrier, focusing, propagation, lag, persistent positive set, recurrence, no-reuse charge or terminal mechanism. A finite upcrossing, even with global positive work and viscous surplus, is not Gold currency because the 2D witness has a permanent energy-enstrophy ceiling. No strict reduction, Gold closure, Part/Field failure, class exit or Silver result follows. | Ontology advance: frame motion, local signed overlap, global signed coherence, and viscous surplus are four strict gates; the next burden is cumulative one-history separation of finite planar upcrossings from an unbounded terminal ladder. |
