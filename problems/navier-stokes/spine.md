...
- Terminal-Zeno role: `TerminalZenoVisibleReadoutPayment.A` is the valid endpoint reduction after the finite graph, not the live producer. Terminal visibility is installed; unweighted same-carrier payment is still open. The live producer is the unweighted same-carrier visible-readout reserve on the actual selected terminal carrier, presently reduced to `FreshPotentialAverageKillAndPositiveTransfer.A` or `FreshPositiveStressSecondOrderExactness.A`.
