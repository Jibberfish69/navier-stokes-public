...
| 037 | Compare a homogeneous heat-age surface, an \(\mathbb R^3\) varying-data event, and a terminal-fitted endpoint window. | No universal positive normalized heat-age floor, late reconstruction theorem, and residual recombination. | Normalized heat age and endpoint reconstruction are different questions; a physical dwell theorem would additionally require one retained carrier. | No generic ancestry, WLF first admission, or four independent debts. | Countertest and route correction. |
...
| 039 | Follow signed mean-free \(\mathbb R^3/\mathbb T^3\) spectral shape through one full history and endpoint probe. | Exact shape equation, covector-qualified Duhamel attribution, positive-variation counterexample, and periodic example. | Spectral shape is a signed same-history state; a chosen endpoint detector sees only the source-generated component with which it pairs nontrivially. | No material ancestry, positive common measure, or bounded descendant fibres. | Ontology fact and route obstruction. |
| 040 | Factor critical height on \(\mathbb R^3/\mathbb T^3\) into energy and global mean Fourier radius; test relative width on periodic varying data. | Exact centroid-rate identity and periodic Pell relative-bandwidth counterfamily. | Rise means the remaining energy distribution's mean Fourier radius increases faster than viscosity depletes the weighted moment. | Centroid lift is not literal spectral travel, monotone cascade, spatial spikiness, localization, phase coherence, or material carriage; the Pell family varies the datum and is instantaneous. | Ontology coordinate and no-go. |
