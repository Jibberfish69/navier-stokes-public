...
| 003 | One incompressible local affine pose has extension and compression at the same instant. | Small-pair affine quadrupole and spin-robust positive angular sector. | Incompressibility forces instantaneous anisotropic angular structure. | No persistence, thickness, carrier, or all-scale passage. | Local ontology fact. |
| 004 | One exact viscous cellular history revisits pair-distance geometry while energy decays. | Periodic cellular solution and return count. | Each datum has finitely many returns; arbitrarily many occur only across data. | No fixed-datum Zeno recurrence or non-dissipative return. | Exact countertest. |
...
| 006 | The same velocity tangent is read through critical transfer, vortex stretching, and enstrophy. | Exact projection identities and separating examples on the declared surfaces. | These are distinct relational projections; viscosity changes later geometry while dissipating global balances. | No pointwise monotone drain or interchangeable production sign. | Ontology distinction. |
| 007 | Assume one datum-anchored history has unbounded critical height. | \(H^2\le E_0Z\) and necessary divergent positive stretching action. | Terminal ascent must create unbounded first-derivative strength and stretching above simultaneous viscous loss. | No interval synchronization, separate payment, or material carrier. | Conditional terminal anatomy. |
| 008 | Follow the vorticity direction and its aligned strain rate along one material path. | Exact material VPI equation for \(\alpha=\xi\cdot S\xi\). | Aligned stretching has one local joined evolution. | No residence duration, persistent maximizer, or switching-supremum control. | Ontology fact and open target. |
...
| 010 | Test an alleged singular \(\mathbb R^3\) history against proved vorticity-direction regularity criteria. | Contrapositive of the coherence criteria at their exact integrability surfaces. | A singular history cannot retain a hypothesis already known to regularize it. | Criterion failure is not cause, bulk incoherence, material mechanism, or Silver exit. | Conditional obstruction. |
| 011 | Existing vorticity-direction variation enters the same vector-diffusion tangent. | Polar-vorticity identities and \(\rho\)- and \(\rho^2\)-weighted budgets. | Direction gradients supply exact negative magnitude terms. | Viscosity need not monotonically reduce direction disorder; it may turn direction. | Ontology fact and route obstruction. |
| 012 | One field's local strain meets all of its relative-velocity chords at once. | \(P_H=-\int S:\mathsf T_u\) and the critical doubling balance. | Critical production is a global anisotropic all-chord contraction. | \(\mathsf T_u\) is not local energy or material mass; the identity is not a finite reserve. | Ontology fact. |
| 013 | In an invariant shear, pure diffusion reweights the already present Fourier directions. | Exact scale--direction covariance law on the pure-diffusion/invariant-shear surface. | Heat selects existing radii and can change normalized spectral direction with either sign. | No full-NS monotone direction law, Fourier migration, spatial narrowing, or energy travel. | Exact model countertest. |
| 014 | Differentiate the complete critical all-chord contraction along one fixed-\(\nu\) history. | Full induced equation for \(C=-S:\mathsf T_u\), including all pressure and viscous rows. | Only the complete joined derivative has causal status. | \(C\) is not a transported density and no component row acts alone. | Ontology fact. |
| 015 | The sign-selected part of the same nonlocal contraction changes through its full VPI source. | Positive-part/Kato identity, ABC residence test, and necessary source action. | The sign interface is Eulerian and the heat scale is only a comparison clock. | No persistent material positive set or hard memory horizon. | Ontology fact plus countertest. |
| 016 | At one instant, compare pressure source, vorticity compression, and rotational contribution in the same field. | Exact pointwise three-case algebraic partition. | The cases classify simultaneous anisotropic poses. | They are not causal mechanisms, dwell histories, or independent causes. | Algebraic classification. |
| 017 | Resolve the same pressure response into its source, trace-free curvature, and pair-increment appearances. | Exact two-slot pressure decomposition, component norms, and signed-action counterfamilies. | All pressure slots recombine in one VPI tangent. | Finite component norms do not control pressure action; no pressure-only route or datum-finite action budget. | Component anatomy and route obstruction. |
| 018 | One exact participant carries pressure, convection, and viscosity in a cancellation-sensitive tangent. | Complete derivative through \(\mathscr X_\nu\) and exact torus cancellation family. | Large separated pressure action can cancel with convection while viscosity supplies the surviving tangent. | Component control is not participant control. | Participation counterexample. |
| 019 | One history loses energy while its global Fourier-weighted critical height rises or falls. | Critical-height occupancy and uninterrupted-rise duration bounds on the global spectral surface. | The same energy can acquire a higher energy-weighted mean Fourier radius. | No shorter-spatial-scale claim, localization, renewal obstruction, or carrier. | Global readout theorem. |
| 020 | Normalize an assumed bounded critical terminal record while the original fluid retains fixed viscosity. | \(\mathbb R^3\) normalization and transformed-coefficient limit. | The transformed viscous coefficient tends to zero in the chosen normalization. | No Euler limit without compactness, pressure, material-passage, and defect theorems. | Conditional route boundary. |
| 021 | Assume a monotone terminal critical-height tail in one history. | Falling energy, rising mean radius, derivative action, and global all-pair compression identities. | The terminal tail has exact global anatomy. | No packet, vortex, annulus, or material mechanism. | Conditional terminal anatomy. |
...
| 054 | Start from the actual threshold population supplied by Cycle 053 and ask how that same population can reside, be rebuilt, or be removed under the joined VPI evolution. | Exact fixed-threshold convex-truncation/interface laws, coarea--isoperimetric interface bound, heat-time residence-or-response alternative, and fragmented pure-heat scalar countertest on decaying \(\mathbb R^3\). | Direct heat drains threshold excess through its full interface while the complete trace-free source supplies or removes that same excess. Backward from a late slice, either it already contributes a scale-independent Gold-action quantum or the joined source rebuilds it against heat with net action \(\gtrsim\nu^2/s\) and dual action \(\gtrsim\nu^3/s\). Fragmentation shortens component heat clocks and increases interface burden. | The selector \(s\) is not a component diffusion radius; raw superlevel volume has no heat sign. No datum source bound, interval non-overlap, recurrence law, packet, annulus, material carrier, Gold closure, or Silver exit follows; the scalar fragmented family is not an NS history. | Ontology advance, exact Gold anatomy, and target refinement without strict reduction. |
...
\(h=s^2/\nu\), either this excess stays above \(e_*/2\) for a fixed fraction
of \(h\), giving

\[
\int I^2dt\ge c(R_0,\eta)\nu^3,
\]
