...
- Gold current strict reduction: construct, before selected readout, one parent-known owner-and-charge law for the same signed-height record event. After pre-return address admission/lift exists, the owner map is formal; the proof-sized burden is one finite original-history pre-readout boundary measure, \(\Gamma_{\rm pre}\), with address/boundary and selected-critical root-payment faces. This is the current reduced route to `SignedCriticalHeightSignPersistenceOrPeakBound.A`.
- Gold open proof-sized object: derive \(\Gamma_{\rm pre}\) from the original pressure-viscosity-incompressibility-velocity history of the same packet. The address/boundary face now has a two-part lower reading: stopped address admission/lift plus finite same-event first-boundary charge. Current sources have not proved this construction from arbitrary original data.
