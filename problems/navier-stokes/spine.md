...
- Gold current strict reduction: construct, before selected readout, one parent-known owner-and-charge law for the same signed-height record event. The owner map is the active-state row, the selected-critical charge is the root-currency row, and the same-record charge inequality is the record-admitted storage row. After the pre-return address cells and address-change charge exist, the owner map is formal; the proof-sized burden is the original-history charge measure. This is the current reduced route to `SignedCriticalHeightSignPersistenceOrPeakBound.A`.
- Gold open proof-sized object: derive the original-history charge measure from the original pressure-viscosity-incompressibility-velocity history of the same packet, with enough pre-return address control to define the owner map before readout. Current sources have not proved this construction from arbitrary original data.
- Absorbed or demoted Gold surfaces: positive clipping, selected `L^1` counting, freestanding active capacity, detached root payment, storage after choosing a payer, raw capacity, raw atomization, scalar BV, chamber-level return, fixed-chart formulas, primitive material channelization, raw active-state first address, stopped common-selector admission, pre-return face-address ownership, same-face return budget, return first-face address, finite return multiplicity, and selected-return no-duplication are support, strict reductions, or checked obstructions unless they attach to the same pre-readout signed-height record packet.
- Primitive descent status: the lower labels now test one event-ownership and charge problem, not sibling frontiers. A primitive signed-channel event must have same-material carrier support, a stopped common-selector chamber, chart/projector and detector/channel ownership, a pre-return face trace, and paid address changes before same-face return or selected-return no-duplication is allowed to count.
