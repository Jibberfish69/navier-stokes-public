...
This manuscript develops the class-membership contrapositive route.
...
This is the silver-standard route: it does not identify one positive estimate that controls every obstruction; it proves that every breakdown-capable obstruction leaves the class object whose members are the legal Clay continuation branches.
\end{abstract}
...
\]
Thus the proof does not need to know in advance which positive estimate will save the pass side.
It needs the fail side to leave the legal same-solution class.
...
Zeno is a presentation of terminal behaviour, not a fourth primitive service.
...
Local \(L^3\) and Duhamel translators are useful when a public-critical-looking terminal mass must be brought back to the same witness ledger.
...
They can illuminate which mechanisms use viscosity and which do not.
...
The working Clay class object \(\Owork\) is the class of same-solution Navier--Stokes continuation packets generated by the original smooth datum and read on one terminal witness ledger.
...
\begin{definition}[Witness ledger]
A witness ledger is the common record
\[
  \WitnessLedger=(u_0,\nu,u,p,Q,N,r,\Phi,T_*),
\]
...
The same-ledger condition is important.
A carrier from one branch, a participation tower from another branch, and a readout from a third branch do not synthesize into one continuation packet.
The services must belong to the same terminal object.

\begin{definition}[The three services]
On a witness ledger \(\WitnessLedger\), the services are:
\[
...
\begin{proof}
Membership means that the terminal packet retains the services needed for a continuation readout on the same ledger.
