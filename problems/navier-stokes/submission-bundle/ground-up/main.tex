...
It means admissible for the test.
...
The previous theorem explains the short proof.
...
The class-law part of the proof closes the in-class branch question: the legal same-solution continuation candidates are exactly the branches represented inside the working class object.
...
\begin{theorem}[Positive obstruction no-third-branch class law]
There is no obstruction branch which both witnesses finite same-surface Clay nonsmoothness and remains a legal in-class continuation candidate:
\[
  \neg\Bigl(
    \Legal_{\NS}(Q)
    \wedge
    \text{finite nonsmoothness witness on }Q
  \Bigr).
\]
\end{theorem}

\begin{proof}
Assume a branch is legal as a same-solution Clay continuation candidate and also witnesses finite nonsmoothness.
By the legal branch law, the legal continuation side gives
\[
  \Legal_{\NS}(Q)\Longrightarrow\Member(Q;\Owork).
\]
By the finite failure witness theorem, the nonsmoothness side gives
\[
  \Exit(Q;\Owork):=\neg\Member(Q;\Owork).
\]
This is a contradiction.
Therefore a branch cannot be both a legal in-class continuation candidate and a finite nonsmoothness witness.
\end{proof}
...
The referee-facing Clay hinge is sharper than this class-law statement.
A finite Clay counterexample may present itself exactly as finite class exit, so classifying the fail branch as \(\Exit(Q;\Owork)\) is not by itself a contradiction of the standard Clay breakdown alternative.
The final Clay-level theorem is:
\[
...
\]
for every finite \(T_*\).
Equivalently, the maximal smooth solution generated by smooth data cannot realize a genuine terminal class exit.
...
This is the mathematical promise.
