...
This manuscript uses that forward field to build a class-membership contrapositive proof.
The direct estimates first show what can go wrong.
The class object is introduced only after that pressure is visible.
...
\]
This gives the paper its order.
First the forward obstructions are displayed.
Then the class object is defined as the object those obstructions must either preserve or leave.
After that the faces, branch families, and closing law have a single job: carry each terminal witness to its first failed service or to smooth continuation.

\section{The Forward Obstructions}

The forward-positive program is the right way to find the hard objects.
It asks each obstruction to be controlled directly.
The present proof reads those same objects as terminal tests.
The reader should see the pressure before the class machinery appears: the machinery is introduced because these families all ask for a same-branch decision.
...
The services are now defined, so the forward families can be read without becoming a catalogue.
Each family has two jobs in this manuscript.
It explains why the direct estimate route reaches a hard obstruction, and it says how that obstruction is read by Pack, Part, or Field.
The class proof stays readable when those two jobs remain together: the family supplies the pressure, and the face decision supplies the proof role.

\subsection{Source Reserve And Donor Refill}

Source reserve is the main Pack-side pressure point.
...
The current source-reserve chain is therefore not a side discussion.
It is the Pack-side form of the final closure burden.
...
This is the right place for pressure-tail Hardy controls, transported boundary tightness, pure-pressure sustain reductions, time-face anti-atom arguments, and production into rigid subclasses.
Each one is valuable because it narrows the residue.
...
This is why height-flux and frame-variation arguments belong in the manuscript.
They do not merely decorate the proof.
They are the mechanism that keeps a retained-amplitude branch from floating between faces.
...
Those alternatives are Pack/Part/Field outcomes or support outcomes, not a separate terminal world.
...
At this point the proof law is in place, so the large surface field can be read without becoming the argument.
