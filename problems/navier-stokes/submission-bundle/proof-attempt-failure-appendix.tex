...
survived into the Pack/Part/Field test.
...
payment the route tried to make.  Then it shows the calculation that made the
route worth trying.  Then it names the mathematical stopping point.  Finally it
states what survived into the class-membership argument.

\subsection{How to Read the Failure Record}

The reader has already seen the class test.  Pack, Part, and Field are the
...
a Pack/Part/Field face, or an auxiliary theorem that is still waiting for one of
...
entered the CM test and a first Pack, Part, or Field face failure has been
...
produces a theorem or a face landing for a named object.
...
context until a same-surface drain theorem or a terminal face landing is
supplied.
...
zero-radius Pack/Part face evidence after CM-test admission.
...
pressure Field-failure context until a same-witness face is derived.
...
Sobolev interpolation bounds the source term by the local-theory inequality
\[
  Y'(t)\le C_\nu Y(t)^3 .
\]
...
The \(Q(t)\) body so has to do more than repeat enstrophy control; it
...
close this body.
The fourth payment is so a same-surface derivative recovery theorem.
...
\(B_j(t)\le\varepsilon\) needed to absorb growth.
...
lands the exact same Navier--Stokes packet in Pack, Part, or Field.
...
stopped there, and how CM tests it as zero-radius Pack/Part face evidence after
terminal admission.
...
The face conversion so begins after boundary bookkeeping.
...
&\Longrightarrow\ \text{derive the first Pack/Part/Field face failure}\\
...
The Euler boundary comparison is admitted only in the clean selected-packet case:
the fixed-viscosity packet has already lost Part or Field participation while
the base inviscid flux-momentum reading still holds.
...
the scale statement.
The proposed proof was so circular in the intended good sense: every
...
\[
  ...
  \int_{\mathbb T^3}(u\cdot\nabla)u\cdot\Delta u\,dx .
\]
Holder, Sobolev interpolation, and Young absorption give
\[
...
In the exact model \(T(k,t)=C_Tk^{-\alpha}\) and
\(E(k,t)=C_Ek^{-\beta}\), the balance equation gives
\[
  k_{\mathrm{crit}}
  =
...
\[
  ...
  B_j(t)=\frac{A_j(t)}{\nu 2^{2j}} .
\]
The hoped-for statement was that, for a chosen small \(\varepsilon>0\), the
active shell sees
\[
  B_j(t)\le\varepsilon
\]
