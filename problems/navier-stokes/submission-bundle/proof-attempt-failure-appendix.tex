...
This appendix records the positive proof attempts that led into the
...
Every later branch below is a way of trying to make one of those payments.

The purpose here is historical and mathematical.
The appendix first says what each route tried to control.
It then shows the calculation that made the route worth trying.
It then marks the exact point where the route stopped.
Only after that does it say what survived into the later class-membership
argument.

\subsection{The Page-24 Failure-Record Contract}

From this point onward the paper has a stricter job than ordinary historical
commentary.
The reader has already seen the class test: Pack, Part, and Field are the three
places where a terminal witness can keep or lose membership.
The remaining pages have to show why the positive smoothness program leaves
exactly the objects tested by that class grammar.
For that reason every route below is recorded with five obligations:
the attempted payment, the calculation that made the route plausible, the exact
...
sharpened.
The record closes only after the branch table and the CM handoff explain how a
finite-time breakdown claim would have to carry one of these terminal objects.
...
The calculation is plausible because each body is a real mathematical tool:
...
that lets viscosity dominate above a critical scale.
...
The residue is deformation geometry and commutator control.
Its CM role is support until a same-surface drain theorem or a terminal face
landing is supplied.
...
The stopping point is the missing contraction or absorption theorem.
...
until a face landing identifies their witness role.
...
The residue is a dynamic low-frequency strain theorem.
Its CM role is support for retained tail and Field coherence.
...
Its CM role is support for source reserve, scale memory, and same-ledger
payment questions.
...
The residue is selector-good lower-envelope debt.
Its CM role is support unless it produces the selected packet or an exact
one-sided theorem.
...
time atoms.
The model calculation \(g_m=m1_{(-1/m,0]}\) has bounded \(L^1\) mass and blows
every \(L^p\), \(p>1\).
The stopping point is lack of uniform integrability or anti-atom control.
The residue is a zero-thickness native source atom.
Its CM role is zero-radius Pack or Part support after CM-test admission.
...
it does not automatically supply same-fluid source ancestry.
...
The residue is pressure-lobe debt.
Its CM role is support for pressure tethering or pure pressure Field failure.
...
Its CM role is Pack, Part, or Field support depending on retained carrier and
flux role.
...
Its CM role is Pack-side zero-radius failure unless a retained positive-scale
...
Its CM role is retained reserve support, not exit without a face landing.
...
oscillation, which should become a Field failure.
...
enstrophy.
The third body was compactness, which would turn controlled approximations into
an exact Navier--Stokes object.
The fourth body was derivative recovery on that exact object.

The intended motion was circular in the useful sense.
Tail control would keep high frequencies from carrying uncontrolled derivative
mass.
That would make \(Q(t)\) finite on the time interval under study.
The \(Q(t)\) bound would give enough compactness to select an exact terminal
object.
The exact terminal object would then recover the derivative bound needed to
restart the high-frequency tail estimate.
If all four payments were available on the periodic surface, the proof would
close by continuation.
...
A direct proof would need \(T(k,t)\) to be weaker than that dissipation on the
bad tail, or at least weak enough after summing over the active scales.
That is the scale payment.
It asks the equation to turn viscosity into a summable derivative tail.
...
\[
  Y(t)=\frac12\|\nabla u(t)\|_2^2 .
\]
For a smooth solution one obtains an enstrophy identity of the schematic form
\[
  Y'(t)+\nu\|\Delta u(t)\|_2^2
...
The standard Sobolev-interpolation route gives a local-theory inequality such
as
\[
  Y'(t)\le C_\nu Y(t)^3 .
\]
...
The \(Q(t)\) body therefore has to do more than repeat enstrophy control; it
has to find a new payment for the source term.

The third body is compactness.
If the tail and source payments were already available, compactness would be
the natural way to produce the object on which the final argument acts.
One splits an approximating sequence into low and high modes.
Low modes are finite-dimensional after time regularity is supplied.
High modes are small after a real tail estimate.
Then strong convergence passes the quadratic term,
\[
  u_n\otimes u_n\longrightarrow u\otimes u,
\]
and the limit remains a solution of the same equation.
...
it.
The compactness payment is therefore exact-object capture.
...
In the early geometric version of the route, the hoped-for drain looked like a
positive curvature or deformation term in a gradient evolution law,
\[
  \partial_t|\nabla u|^2
    = -\mathcal D(\nabla u) + \nu\Delta |\nabla u|^2
      + \mathcal N(\nabla u),
...
It does not automatically supply a positive geometric drain strong enough to
close this body.
The fourth payment is therefore a same-surface derivative recovery theorem.
...
No body is meaningless, and no body is a completed global proof.
The attempted loop reveals one obstruction packet:
\[
  \text{tail payment}
  \quad+\quad
  \text{source payment}
  \quad+\quad
  \text{exact-object capture}
  \quad+\quad
  \text{same-surface derivative recovery}.
\]
...
The paper then asks the Pack-first questions.
...
The residue is therefore not vague turbulence language.
It is a precise high-frequency nonlinear transfer packet.
...
\(\|\nabla u\|_2\) and \(\|\Delta u\|_2\), after which Young absorption leaves
\[
  \frac{dY}{dt}\le C_\nu Y^3 .
\]
...
The compactness body tried to turn approximate control into an exact solution
object.
...
In CM, this becomes the rule that support estimates do not enter the proof
...
On a curved carrier, a schematic gradient evolution contains a Ricci term:
\[
  \partial_t|\nabla u|^2=-\mathrm{Ric}(g)(\nabla u,\nabla u)+\nu\Delta_g|\nabla u|^2+C|\nabla u|^3 .
\]
...
velocity tower, moving coordinates, and terminal source ledger.
The failure is also clearer.
The packet still needs a same-surface contraction or absorption law strong
enough to make the survivor pay for itself.
Without that law, the packet is a sharper description of debt, not a discharge
of debt.
The CM proof uses this section to police vocabulary.
If a later object is merely a modern name for the same unpaid survivor, the
paper must carry it as residue until Pack, Part, or Field makes the proof
decision.
...
The missing theorem would show that lower-prefix memory creates a coercive
drain, that upper-tail leakage is blocked by a kernelized barrier, or that the
two are one terminal survivor in different coordinates.
Until then, the paper must keep them separate.
In CM terms, neither family is automatically an exit.
Each is a witness candidate waiting for a selected terminal packet and a first
face failure.
...
strength.
After Young absorption, however, the lower shell carries a remainder with an
extra \(2^k\) beyond the available energy surface.
The route therefore gets closer to the desired estimate but stops at an unpaid
half derivative.
...
They do not say that every active window sees a small enough coefficient to
absorb growth.
The survivor is dynamic low-frequency strain suppression.
In the PDF this branch should teach the reader why an endpoint half-derivative
gain is a theorem target, not a rhetorical strengthening of viscosity.
...
The attractive feature is that this object sees the whole lower-scale history
...
does not close the absorption.
The residue is active-square debt.
It later reappears as source reserve and lower-memory pressure.
For CM, LPAS is support unless the same terminal packet carries the active
...
The failure is that the quotient is not automatically controlled.
...
The survivor is therefore a missing decorrelation theorem, a signed-measure
...
\[
  |N_1|+|N_2|\le \epsilon\,\mathrm{dissipation}+C_\epsilon\Psi(E_D).
\]
The two-point strain side has a similar hope: selected pair geometry should
produce the good lower envelope needed by SG.4.
...
a sharp downstream obstruction.
Its CM role is support or readout pressure until it lands the exact same
Navier--Stokes packet in Pack, Part, or Field.
...
test.
It asks whether the object still has Pack, Part, and Field, not whether the old
...
the \(L^1\) mass stays bounded while every \(L^p\), \(p>1\), blows.
...
Calling it an exit immediately would skip the CM face test.
The paper must instead say what the object is, why the positive deletion route
stopped there, and how CM tests it as zero-radius Pack/Part support after
terminal admission.
...
it is tempting to treat pressure as a source shadow.
The failure is same-fluid ancestry.
Calderon--Zygmund structure acts on spatial slices and tensor components; it
does not automatically say that the pressure lobe belongs to the same native
source current.
The branch therefore splits pressure into a source-carried part and a pure
...
Global tensor cancellation does not automatically pay the selected positive
bill.
...
The hoped-for theorem says that incoming positive source flux through the
...
A pure zero-radius endpoint is Pack-side failure unless a positive-scale
...
Thus local energy and elliptic pressure are real support tools, but neither
one supplies the missing time anti-concentration theorem.
The residue is trace, flux, or temporal spreading debt.
The CM proof uses this as source-wall support, not as a finished deletion of
terminal atoms.
...
paper.
Therefore none may be silently used as if it has paid the source wall.
...
positive theorem or attach a selected terminal object to Pack, Part, or Field.
This section is one of the places where PDF honesty matters most.
...
creation.
The branch therefore cannot be called deleted.
It is supplier-quarantined until a same-ledger payment theorem is proved.
Alternatively, once admitted to the CM test, it can support a first face
failure.
The PDF has to preserve that fork: payment theorem on the positive side,
face failure on the contrapositive side.
...
still retain order-one positive and negative packets on the same ledger.
...
The hard case is same retained ledger, positive scale, order-one reserve, and
...
first Pack, Part, or Field face failure is derived.
...
localized by adjoint heat/Leray response into native-source machinery.
...
This branch is therefore not a fresh positive proof of smoothness.
It is a CM-facing translation rule for a selected terminal concentration.
...
With Pack and Part retained, that pure oscillatory branch should be Field
failure.
...
carry; the main proof tests that carried object through Pack, Part, and Field.
...
The four-body root therefore belongs before the branch families.
...
Projected flow, TPS, and SG.4 are tempting because they seem to put the defect
inside a parabolic or two-point equation where coercivity should be available.
...
They cannot replace the Pack, Part, Field test unless a theorem transfers the
...
BASAC gives the clean model: bounded \(L^1\) mass can still concentrate into a
terminal time atom.
...
theorems.
The source wall therefore has to be written as a productive failure.
...
Pressure sustain therefore splits into source-carried pressure and pure
pressure residue.
...
silently absorbed into the source wall.
...
Its natural CM landing is Field-facing, because the question is whether a
retained carrier and participation still give coherent positive-scale readout
when pressure carries a selected terminal residue.
...
The CM proof has to say exactly where that object lands: usually Pack-side
...
through the CM face test after terminal admission.
...
oscillation.
The CM proof then asks the same three questions of the selected survivor.
Does it have a positive material carrier?
Does the Navier--Stokes law still participate on that carrier?
Does the field have coherent positive-scale readout?
If all three survive, the branch is on the smooth same-solution side.
If one fails first, the branch gives \(\neg\Pack_Q\), \(\Pack_Q\wedge
\neg\Part_{N,Q}\), or \(\Pack_Q\wedge\Part_{N,Q}\wedge\forall r>0\,
\neg\Field_{N,r,Q}\).
That is the exact way the failure record becomes proof material.
...
The positive route would prove an extra half derivative, an intermediate shell
gain, or a coupled weighted-tail theorem.
The CM route uses it differently.
Once a terminal witness has been selected, the deficit asks whether the tower
can still supply a coherent \(N\)-level field on some positive radius.
If Pack and Part are retained and the lower-shell deficit prevents every
positive-scale readout, the branch is Field-facing.
...
survives, it supports Pack failure.
...
If both survive and the square prevents a positive coherent field scale, it is
Field support.
The same algebra can therefore have three different proof roles depending on
...
class.
The conversion check is this: can the same terminal branch carry the quotient
while retaining Pack and Part?
If no carrier remains, the quotient is evidence for Pack failure.
...
If carrier and participation survive but the quotient obstructs one coherent
positive-scale field, it is Field-facing.
...
ledger.
Zero moment can coexist with order-one positive and negative packets.
If the partners live on different carriers, the selected branch may fail Pack
or Part before sign even matters.
If the partners survive on the same carrier and same equation, the issue moves
to Field: does their exchange preserve a coherent field readout, or does it
destroy every positive-scale witness?
...
admission and first-face derivation.
...
anti-atom control.
The atom is terminal and source-like, but its first face is not determined by
...
If carrier and participation survive and the atom still blocks coherent
positive-scale readout, it is Field support.
...
The most natural landing, after Pack and Part survive, is Field: a pure
pressure lobe may destroy coherent readout while the carrier and equation
participation remain formally present.
That is why the pressure sections repeatedly ask for pressure-source tethering
or pure-pressure Liouville.
Those are the positive doors; CM face conversion is the contrapositive door.
...
The face conversion therefore begins after boundary bookkeeping.
...
If carrier and participation remain and the atom prevents a coherent positive
field scale, it is Field-facing.
...
retained, the branch lands first at Pack failure.
...
If participation survives but infinite refill destroys coherent positive-scale
readout, the landing is Field.
...
If both survive and reserve birth prevents a coherent readout on any positive
scale, Field fails.
...
The \(H^{1/2}\) branch should be read as conditional field analysis.
...
If Pack and Part are retained, that is exactly the shape of a Field failure:
the same branch exists and participates, but no positive coherent readout
survives.
...
Without the record, Pack, Part, and Field can look like labels placed on
...
Third, an object useful to the contrapositive proof is not automatically a
class exit.
Fourth, no support theorem enters the proof until it has a same-witness
landing.
...
theorem looks like a missing proof hole in the CM argument, and every terminal
object looks like an unlicensed shortcut.
...
The branch table near the end should therefore be read as a compression of the
preceding work, not as a substitute for it.
...
This is the difference between a proof record and a status matrix.

The same rule applies to source files and generated surfaces outside the PDF.
They are discovery surfaces and evidence surfaces.
They are not the reader-facing proof until the argument has been written into
the paper.
A theorem note may show why a supplier theorem is open.
A route graph may show how two branches connect.
A checker may verify that a section is present.
None of those artifacts pays the reader's burden.
The paper pays that burden by stating the attempt, calculation, failure,
residue, and CM role in ordinary mathematical prose.
That is the standard used in the pages that follow.
...
selected Pack, Part, or Field face.
...
Which calculation made that purchase plausible?
Where did the calculation stop?
What residue survived the stopping point?
What is the residue's exact role in the Pack, Part, Field grammar?
...
burden.
If the paper jumps from a named failed route directly to a Pack, Part, or Field
...
The route did not begin with Pack, Part, and Field as formal names.
...
Some were heuristic explanations.
Some were generated summaries of older work.
The live question therefore became a ranking question:
\[
...
The scale estimate feeds the energy/enstrophy bridge.
...
The old \(Q(t)\) hope was
\[
  Q(t)=\|u(t)\|_{L^2}^2+\|\nabla u(t)\|_{L^2}^2
\]
with the expectation that \(Q\) would behave like a monotone or controlled
quantity.
...
\[
  \int_{\mathbb T^3}(u\cdot\nabla)u\cdot\Delta u\,dx .
\]
After interpolation and absorption the proof reached
\[
  Y'(t)\le C_\nu Y(t)^3 .
\]
...
\]
The packet is successful only when the live survivor contracts, is absorbed, or
...
\[
  ...
  N_1(X)+N_2(X)
\]
suggests the absorption estimate
\[
  |N_1(X)|+|N_2(X)|
  \le
  \varepsilon\mathcal D_D(X)+C_\varepsilon\Psi(E_D(X)).
\]
A two-point packet gives a similar hope.
If a pair defect \(W_J\) satisfies a drift-diffusion equation with controlled
remainder, diffusion should force the pair readings to stay coherent.
...
\]
A norm estimate on a defect does not automatically give this one-sided
directional lower envelope.
It may control size while losing sign, direction, pair membership, and
selector-good status.

The branch therefore split.
Defect equations remained support for the proof.
They did not become the proof primitive.
...
Field is the place where coherent positive-scale readout is tested after
carrier and participation have survived.
...
\[
  ...
  \quad(p>1).
\]
Thus bounded \(L^1_t\) source mass can converge to a terminal atom without
giving uniform integrability.
...
\]
The first term can join the source ledger.
...
\[
  ...
  \Longrightarrow
  \text{test it as the same terminal object}
  \Longrightarrow
  \text{derive the first Pack/Part/Field face failure}
  \Longrightarrow
  \Exit(Q;\Owork).
\]
The bad object is not automatically an exit.
It has to enter the terminal CM test.
But once it is admitted and a first face failure is derived, it is no longer an
enemy of the proof.
...
\[
  ...
  &\Longrightarrow
  \text{same-fluid CM terminal packet tree}\\
  &\Longrightarrow
  \text{first failed Pack/Part/Field face}\\
  &\Longrightarrow
  \Exit(Q;\Owork).
  \end{aligned}
\]
...
The Pack-first questions are now unavoidable:
\[
  \begin{array}{ll}
...
\[
  ...
  O_{\Fail}.
\]
The pass branch keeps the same-fluid service:
\[
  O_{\Pass}
  \Longrightarrow
...
It is a terminal object capable of finite nonsmoothness and admitted to the CM
test:
\[
  O_{\Fail}
  \Longrightarrow
...
same terminal face-failure branch.
...
\[
  ...
  \int_W J_+ = \int_W J_-,
\]
while both \(J_+\) and \(J_-\) remain order-one on the same retained terminal
ledger.
...
It may be pass-side service.
It may be admitted to the terminal CM test.
It may produce a Pack, Part, or Field face failure.
...
It is not automatically CM closure.
An endpoint readout theorem is pass-side support after Pack, Part, and Field
survive.
It is not the upstream class test.
An Euler comparison can reveal a slip picture.
It is not Navier--Stokes proof material until an exact fixed-viscosity packet is
landed in the same witness tree.

The manuscript therefore treats topology as part of the proof.
...
\[
  ...
  &\Longrightarrow
  \text{calculation that made the payment plausible}\\
  &\Longrightarrow
  \text{exact stopping point}\\
  &\Longrightarrow
  \text{surviving terminal object}\\
  &\Longrightarrow
  \text{Pack, Part, Field, pass-side readout, or supplier quarantine.}
  \end{aligned}
\]
...
The scale statement would keep high frequencies from carrying uncontrolled
energy.
The \(Q(t)\) statement would bind energy and enstrophy into one controlled
quantity.
The compactness statement would turn controlled approximations into an exact
Navier--Stokes object.
The gradient statement would recover the derivative control needed to return to
the scale statement.
The proposed proof was therefore circular in the intended good sense: every
body would feed the next body and the fourth body would close the loop.

That shape explains both the attraction and the failure.
The attraction is that each body is a familiar mathematical move.
High-frequency dissipation is real.
The kinetic energy identity is exact.
Compactness is the standard way to pass from approximations to solutions.
...
language that is not automatically a periodic flat-torus theorem.
The circuit therefore identifies the correct proof architecture and also shows
...
The useful dream is that the transfer term satisfies a tail estimate such as
\[
  T(k,t)\le Ck^{-\alpha}
\]
...
\[
  \|\nabla u(t)\|_{L^2}^2
  \sim
  \int_0^\infty k^2E(k,t)\,dk
  <\infty .
\]
This is why the route looked like a proof.
It turns the intuitive sentence ``viscosity wins at small scales'' into an
actual summability condition.
...
\]
and after Young absorption it leaves
\[
  \frac{dY}{dt}\le C_\nu Y^3 .
\]
...
It consumes them.
The compactness attempt therefore survives as a precise packet-capture lesson:
once a terminal or approximate object is controlled in the right way, compactness
...
\]
with a schematic gradient evolution
\[
  \partial_t|\nabla u|^2
  =
  -\operatorname{Ric}(g)(\nabla u,\nabla u)
...
prove a same-surface absorption law strong enough to send Body IV back to Body I.
...
After Young absorption, the route leaves a lower-shell remainder with an extra
factor \(2^k\).
That factor is the point where the estimate stops.
The installed energy surface controls \(2^{2k}\|\Delta_k u\|_2^2\) in time, not
\(2^{3k}\|\Delta_k u\|_2^2\).
The residue is therefore a half-derivative or equivalent lower-shell gain
target.
...
That would turn low-frequency strain into a coefficient absorbable by the
viscous scale \(2^{2j}\).
The attractive point is the scale matching:
...
support target, not a completed smoothness proof.
...
This looked promising because it packages the whole lower prefix instead of
chasing one shell at a time.
...
For the later class-membership proof, that residue is not automatically bad
enough to prove exit.
It has to be attached to the selected terminal object and then tested for Pack,
Part, and Field.
...
\]
visible.
The signed commutator could then be written schematically as
\[
  \mathfrak c_\ell
  =
...
The route therefore stops at a real missing theorem: eigendirection
...
\]
The two-point strain route had the same hope in a more physical form: make a
defect equation for pair strain and prove that selected pairs renew the lower
envelope needed by SG.4.
The failure is selector production.
...
\[
  g_m(s)=m\,\mathbf 1_{(-1/m,0]}(s)
\]
has bounded \(L^1\) mass and converges to a terminal atom, while
\[
  \int_{-1}^0g_m(s)^p\,ds=m^{p-1}\to\infty
\]
for every \(p>1\).
That one calculation explains the whole wall.
An \(L^1_t\) source ledger can carry terminal mass without giving uniform
integrability.
...
It tests the atom as a terminal obstruction and asks which witness face it loses
first.
...
time slice.
It does not automatically give transported same-fluid source ancestry.
...
In the later witness tree, zero-radius ancestry is Pack-side unless a retained
...
One can have order-one square reserve and zero first moment:
\[
  J=J_+-J_-,
  \qquad
  \int_WJ_+=\int_WJ_- .
...
CM test and a first face failure is derived.
...
pure oscillation should become a Field failure only after same-ledger extraction
and oscillation-to-Field lemmas are proved.
...
Those are the Pack, Part, and Field questions.
The pass branch is the branch that keeps all three requirements and therefore
...
bad.
It becomes class exit only when the first failed witness face has actually been
derived.
...
Compactness needed the scale and energy inputs before it could produce a useful
limit.
...
\[
  ...
  \int_0^\infty k^2E(k,t)\,dk
  \le
  C\int_0^\infty k^{2-\beta}\,dk
  <\infty .
\]

The natural proof picture is this.
There should be a critical scale \(k_{\mathrm{crit}}\) where nonlinear transfer
and viscous loss balance:
\[
  T(k_{\mathrm{crit}},t)
  =
  2\nu k_{\mathrm{crit}}^2E(k_{\mathrm{crit}},t).
\]
Above that scale, viscosity should dominate and drive the tail down.
With \(T(k,t)\le Ck^{-\alpha}\) and \(E(k,t)\sim k^{-\beta}\), the formal
balance gives
\[
  k_{\mathrm{crit}}
  \sim
  \left(\frac{C}{2\nu}\right)^{1/(2+\beta-\alpha)}.
\]
This is why the route was attractive.
It gives a concrete way to think about viscosity winning at sufficiently high
frequency.
...
The pointwise high-frequency transfer law remains open:
\[
  T(k,t)\le Ck^{-\alpha}
\]
with the exponent needed to make the derivative tail summable.
That transfer law is already a cascade-control theorem.
...
\[
  ...
  \|\nabla u\|_{L^2}^{3/2}\|\Delta u\|_{L^2}^{3/2}.
\]
Young's inequality absorbs part of \(\|\Delta u\|_{L^2}^2\) and leaves
\[
  \frac{dY}{dt}
  \le
  C_\nu Y^3 .
...
\[
  ...
  \frac{Y(0)}{\sqrt{1-2C_\nu Y(0)^2t}} .
\]
The calculation therefore shows exactly what the \(Q(t)\) route would have to
improve.
It needs the nonlinear source to be paid by something stronger than the
standard energy/enstrophy bookkeeping.
...
\[
  ...
  +\sup_{k>k_0}\frac{E(k)}{k^\beta}.
\]
The argument is the standard low/high split in a tailored space.
Write
\[
  u_n=u_n^{(<K)}+u_n^{(\ge K)} .
\]
The low-frequency part has only finitely many Fourier modes and is compact once
the time side is controlled.
The high-frequency part is small when the scale tail is small:
\[
  \|u_n^{(\ge K)}\|_{L^2}^2
  \lesssim
  \int_{k\ge K}E_n(k,t)\,dk .
\]
If \(E_n(k,t)\le Ck^{-\gamma}\) with \(\gamma>1\), the tail can be made small by
taking \(K\) large.
For derivative-level compactness one needs the stronger tail matched to the
derivative being passed.
...
\[
  ...
  -\operatorname{grad}_g p+\nu\Delta_g u .
\]
The gradient calculation has the schematic form
\[
  \partial_t|\nabla u|^2
  =
  -\operatorname{Ric}(g)(\nabla u,\nabla u)
...
Positive curvature acts like a geometric drain on derivative growth.
...
The fourth bridge therefore had to be either pulled back to the chosen
...
\[
  ...
  G=AA^\top .
\]
In those variables, transport is absorbed into the geometry and viscosity
...
The missing theorem is a same-surface drain/absorption law strong enough to
return from Body IV to a stronger Body I tail theorem.

\subsection{Two Survivor Families}

After the modern four-body pass, the written proof record had two terminal survivor families.
...
\[
  ...
  one pay the other}.
\]
The written proof record reached reductions in both directions.
...
upper-tail packets, and source reserve as separate objects until a face landing
is actually derived.
...
\]
The first term is absorbed by dissipation.
...
This route made a real gain and still stopped.
The off-diagonal kernel is installed at the algebra level.
The missing theorem is an extra \(k\)-side gain, roughly a half derivative on
the intermediate shell, or an equivalent coupled weighted tail theorem.
...
\[
  ...
  B_j(t)=\frac{A_j(t)}{\nu 2^{2j}} .
\]
The hoped-for statement was that the active shell sees
\[
  B_j(t)\ll1
\]
...
The finite-shell reduction is straightforward.
Fix \(j\).
Low shells \(k\le j-C\) have a separation gain, and the high shell has viscous
time \(2^{-2j}\).
On one shell, the desired payment looks like
\[
  ...
  +\mathrm{lower\ order}.
\]
The proof would close once \(A_j(t)\le\varepsilon\nu 2^{2j}\) on the active
window.
...
Third, pointwise absorption would need
\[
...
The route therefore stops at one explicit object:
\[
  \mathcal P_j^\downarrow\,2^{-j}D_j^2 .
\]
...
\[
  ...
  N_1(X)+N_2(X),
\]
with the hoped-for estimate
\[
  |N_1|+|N_2|
  \le
  \varepsilon\,\text{dissipation}+C_\varepsilon\Psi(E_D).
\]
The route was attractive because a projected noncommutative flow with a
coercive energy and a commutative shadow could act as a sidecar: prove the
projected theorem, shadow it back to the ordinary equation, and inherit
continuation.
...
deficits.
D.2/TPS support therefore leaves SG.4 open.
...
sorted first through the Pack/Part side.
...
The transported source ancestry needed by the native source-wall machinery
remains open.
...
The direct quantization theorem therefore fails from the installed inputs.
...
Zero-radius donor ancestry is already a Pack-side failure.
The retained positive-scale first-created reserve remains supplier-quarantined
until a same-ledger payment theorem or a CM-test admission plus first face
failure is proved.

\subsection{Zero-Moment Signed Pair}

The zero-moment attempt sharpened the source-reserve survivor again.
It showed that zero first moment can coexist with order-one mass.
...
\[
  ...
  \int_W(J_++J_-)\simeq1 .
\]
The square reserve can see an order-one pair while the signed first moment
vanishes.

Most cases route cleanly.
A partner on the wrong carrier breaks Pack, Part, or Field.
...
\]
with order-one square reserve and zero first moment.

The tempting proof says that one packet must produce visible active height flux.
...
captured by paraproduct native-source machinery.
...
\[
  ...
  \Legal .
\]
With retained Pack and Part, the local public translator gives the Field-facing
critical readout.
...
With retained Pack and Part, that pure oscillatory branch should be a Field
...
\[
  ...
  \Member(Q;\Owork).
\]
The fail branch must do real work:
\[
  O_{\mathrm{fail}}
  \Longrightarrow
  ...
  \forall r>0\,\neg\Field_{N,r,Q}.
\]
Only after the first face failure is derived does the class-exit conclusion
apply:
\[
  \Exit(Q;\Owork):=\neg\Member(Q;\Owork).
\]
...
Navier--Stokes class or loses Pack, Part, or Field.
...
Reduced terminal survivor candidate; class exit requires face landing \\
Signed descent &
...
Zero-radius Pack/Part witness support after CM-test admission \\
Pressure sustain &
...
Pack/Part/Field support depending on retained carrier and flux role \\
Quantized parent / Zeno &
No quantized spend; terminal ancestry can shrink indefinitely &
Zero-radius residue lands first at Pack-side failure unless a positive-scale carrier is retained \\
Public local \(L^3\) &
...
that would close the classical proof, or else exposed a terminal object that has
to be tested as the same Navier--Stokes solution.
...
When the answer is yes through all three questions, the branch is the pass side.
It gives the same-solution membership branch.
When the answer first fails, the branch supplies the face failure:
\[
  \neg\Pack_Q,
  \qquad
  \Pack_Q\wedge\neg\Part_{N,Q},
...
\]
Those are the only exits used by the class proof.
The record is therefore not a second proof route.
It is the intake map for terminal evidence: each failed smoothness strategy
names the debt it could not pay, preserves the solution-surface on which that
debt lives, and hands the remaining object to the same Pack/Part/Field test
...
The proof succeeds only where the terminal object either remains a member of
the working class or loses one of the three witness requirements that define
membership.
