...
\begin{lemma}[Local strong solution and uniqueness]
\label{lem:local}
Let \(s>5/2\), let \(t_0\in\R\), and let \(a\in H^s(\R^3)\) be
divergence-free. There is a
\(\delta=\delta(\nu,s,\norm{a}_{H^s})>0\) and a unique solution of the
Navier--Stokes equation on \([t_0,t_0+\delta]\), with \(w(t_0)=a\), such that
\[
 w\in C([t_0,t_0+\delta];H^s)
 \cap L^2(t_0,t_0+\delta;H^{s+1}).
\]
If \(a\in\Hs\), every Sobolev order persists on a common possibly smaller
interval and \((w,q)\) is smooth up to \(t=t_0\). Its temporal jet at \(t_0\) is
the pressure-normalized recursion \eqref{eq:jet-pressure}--\eqref{eq:jet-velocity}
with \(V_0^*=a\).
\end{lemma}

\begin{proof}
The Friedrichs approximations satisfy the standard commutator estimate
\[
 \frac12\frac{d}{dt}\norm{w}_{H^s}^2
 +\nu\norm{\nabla w}_{H^s}^2
 \leq C_s\norm{\nabla w}_{L^\infty}\norm{w}_{H^s}^2.
\]
Since \(s>5/2\), Sobolev embedding controls
\(\norm{\nabla w}_{L^\infty}\) by \(\norm{w}_{H^s}\). A uniform short-time
bound, compactness of the regularized sequence, and the equation give
existence with the displayed regularity. Applying the corresponding estimate
to the difference of two solutions and using Gronwall proves uniqueness.
Repeating the estimate after differentiating the equation gives persistence of
each \(H^m\) norm; a diagonal choice of the same base lifespan works because
the local time is fixed by the \(H^s\) bound, while the higher estimates are
linear once that base solution is known. Finally, differentiating the equation
at the initial time and using the normalized pressure Poisson equation yields
the stated jet recursion.
\end{proof}
...
\(t_0<T_*\) sufficiently close to \(T_*\). The continuous \(H^s\) trace gives
a uniform local lifespan, and strong-solution uniqueness identifies that
solution with \(u\) on the overlap. Both descriptions restart the same
datum-generated history.
