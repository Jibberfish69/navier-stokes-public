...
aim is to gain decay from scale separation. If a lower scale \(k\) is separated
from an active scale \(j\), the interaction kernel should carry a factor that
shrinks as \(j-k\) grows. The branch asks whether that separation gain is strong
enough to pay the remaining lower-shell bill.

The motivating schematic estimate has the form
\[
  \big|\langle P_j((P_k u\cdot\nabla)u_{\sim j}),u_j\rangle\big|
  \leq 2^{-\alpha(j-k)} A_{j,k}(t),
\]
where \(k<j-C\), \(\alpha>0\), and \(A_{j,k}\) contains the active and lower-shell
norms. The desired outcome is summability over \(k<j-C\). A true lifted-band gain
would turn separation into payment.
...
paid by the basic energy surface. In schematic form, the gain gives
\[
  2^{-(j-k)}\cdot 2^j = 2^k,
\]
...
ledger. Each move sharpens the residue. Each move also has to explain what
happens at a terminal time if the lower-shell derivative demand survives.

In the CM-exit program, the lifted-band residue is support until it is attached
to a selected terminal object. If it enters as a lower-shell packet failure, it
...
scale readout, it presses the Field face. The section therefore records the
branch as a technical source of terminal witnesses, not as a closed positive
smoothness proof.
