...
is summability over \(k<j-C\). A true lifted-band gain turns separation into
payment.

The branch isolates one precise theorem burden. The unpaid lower-prefix object is
not treated as a black box. It is placed into a kernel, and the required theorem
is summable kernel decay over the separated range \(k<j-C\).
...
records carrier-window support. If it breaks the same nonlinear participation
law, it presses the Part face. If every coherent positive-scale field
realization is lost, it presses the Field face and directly exhibits
nonparticipation. The branch is a technical source of
