...
\]
One stronger positive theorem would dominate both \(P_j^{\downarrow}\) and \(T_j^{\uparrow}\) by the same dissipative budget. The CM proof does not require that collapse: each selected lower-prefix or upper-tail terminal family is tested separately through Pack, Part, and Field.
...
The branch conclusion is not provisional in the CM sense. The lower-prefix / upper-tail split is support for the CM-exit program, and each selected terminal family receives the same membership test: continuation readout, Pack failure, Part failure, or Field failure. Its paper role is to show why branch organization is necessary: a terminal obstruction can have several faces before the membership test decides its final membership conclusion.
