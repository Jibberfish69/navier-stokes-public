...
Hypothesis `(4)` is now supplied by the discharged classical scale-barrier
package on this same approximation surface. Therefore `C-U1` and `C-U2` are now
fully discharged downstream consequences on the fixed classical scheme.

In other words, the compactness bridge is no longer a live independent
frontier. It is the exact downstream closure package imported once the upstream
scale barrier is in place.
