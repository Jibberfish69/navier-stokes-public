...
## External-style geometric input to isolate

### Hypothesis `TCJ` (transport coherence on the good family)

Use the same transport-coherence packet as in
`cycle-exact-angular-observability-packet.md`: a transported good family
`G_J^{pair}(I)` on the strip `I=[t_0,t_1]`, a measurable transported pair
direction field `e_{ab}(t)`, almost-everywhere trajectory uniqueness, and an
asymmetric Lusin-Lipschitz transport control absorbed into a strip coherence
ledger `\mathcal E_{J,\mathrm{coh}}(I)`.
...
After closing the transport step as `TCJ'`, the surrogate-construction step as
