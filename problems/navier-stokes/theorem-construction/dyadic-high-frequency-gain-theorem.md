...
### Program 4. All-scale transport inefficiency

Prove the broader transport-law route recorded in
`all-scale-frequency-transfer-inefficiency-theorem.md`: an all-scale spectral
monotonicity / transport-loss theorem in which upward nonlocal transfer across
every dyadic pair `k<j` pays a summable gap cost. In that program, the
high-frequency shell gain would no longer be a standalone local miracle; it
would appear as a corollary of a stronger directional transport theorem.
...
The new all-scale route broadens that frontier rather than closing it locally:
the kernelized lifted-band note shows a real spectral-gap loss on one open
packet, but the full all-scale transport-loss theorem remains strictly stronger
than the current Euclidean package.
