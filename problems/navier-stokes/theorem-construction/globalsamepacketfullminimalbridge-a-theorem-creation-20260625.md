...
- target label: GlobalSamePacketFullClockFromOriginalData.A
...
GlobalSamePacketFullMinimalBridge.A. Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. This supplies the exact noncircular bridge needed for GlobalSamePacketFullClockFromOriginalData.A.
...
- selection_reason: This theorem is the right next move because it changes the proof economy: Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. It is source-backed by 8 local ingredient(s). It deliberately reduces the remaining work to GlobalSamePacketFullMinimalBridge.A.CoreSubprimitive rather than pretending the frontier is closed.

### Candidate Theorems

- GlobalSamePacketFullMinimalBridge.A (minimal_bridge; score=78)
- GlobalSamePacketFullContrapositiveExit.A (contrapositive_exit; score=72)
- GlobalSamePacketFullCompactnessDefectRigidity.A (compactness_defect_rigidity; score=71)
- GlobalSamePacketFullMonotoneBudget.A (monotone_budget; score=70)
...
- depth 0: GlobalSamePacketFullMinimalBridge.A via minimal_bridge -> GlobalSamePacketFullMinimalBridge.A.CoreSubprimitive
...
- next subprimitive: GlobalSamePacketFullMinimalBridge.A.CoreSubprimitive -- core noncircular subprimitive for GlobalSamePacketFullClockFromOriginalData.A
