...
- selection_reason: This theorem is the right next move because it changes the proof economy: Find the strictly weaker same-material bridge that advances GlobalSamePacketFullClockFromOriginalData.A while preserving the full pressure-viscosity-incompressibility-velocity packet. It is source-backed by 8 local ingredient(s). It deliberately reduces the remaining work to GlobalSamePacketFullMinimalBridge.A.CoreSubprimitive rather than pretending the frontier is closed.

### Candidate Theorems

- GlobalSamePacketFullMinimalBridge.A (full_same_material_minimal_bridge; score=156)
- GlobalSamePacketFullCompactnessDefectRigidity.A (full_same_material_compactness_defect; score=124)
- GlobalSamePacketFullMonotoneBudget.A (full_same_material_monotone_budget; score=120)
