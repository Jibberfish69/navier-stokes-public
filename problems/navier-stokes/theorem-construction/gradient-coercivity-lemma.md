...
```
where `\mathcal{C}_j` denotes the repaired high-side remainder in the dyadic
decomposition of `P_j((u\cdot\nabla)u)`, namely the lifted high-side remainder
plus the genuine high-high commutator packet after the strict low-mode and
threshold-local spill collar have been separated.
...
3. Bound the strict low-mode low-high/high-low pieces by `C \mathcal{L}_N(t) E_N(t)` using Bernstein and divergence-free cancellation.
4. Separate the threshold-local spill collar from the lifted high-side remainder, and package the unresolved lifted remainder plus genuine high-high pieces into `\mathcal{C}_j`.
5. Invoke the scale-barrier tail input to absorb an `eta`-fraction of dissipation and leave a summable remainder.
6. Combine and integrate in time; apply Gronwall using the automatic `L^1_t` control of `\mathcal{L}_N`.
...
- Still missing for full discharge:
  - a complete Euclidean derivation of Assumption 1 from already accepted scale-barrier inputs, with the threshold-local spill collar and the lifted high-side remainder both aligned at theorem grade and no import of geometric or modified-equation structure.
