...
This lemma supplies the intended gradient-side cubic-tail absorption mechanism
on the classical equation. Under the stricter audit standard, however, the
gradient bridge does not yet count as fully discharged until the packet
reductions are expanded line by line and audited inside the live theorem
surface.

## Honest Boundary

This lemma closes the cubic-tail absorption step on the current theorem
surface. It does not by itself discharge the whole gradient bridge, because the
packet split feeding that cubic tail still needs theorem-grade alignment at the
proposition level.
