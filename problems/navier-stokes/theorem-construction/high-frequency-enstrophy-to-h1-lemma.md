...
high-frequency enstrophy `E_N` into the full Euclidean `H^1` continuation norm.

It is mathematically elementary, but it is theorem-critical because the
continuation bridge uses the full `\|\nabla u(t)\|_{L_x^2}` norm rather than
only the dyadic tail `E_N(t)`.
...
This is the desired `H^1` continuation bound.

## Output

This lemma removes the last soft frequency-completion step between the dyadic
gradient-transfer inequality and the classical continuation norm.
