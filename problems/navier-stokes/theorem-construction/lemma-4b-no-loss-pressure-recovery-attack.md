...
This is the clearest same-derivative-depth loss source: the loss is measured
against the natural Dirichlet control for \(U_\alpha\), not by a scale-size
comparison.
...
## Direct attack B: same-derivative-depth pairing estimate
...
and there is no current theorem reducing it to a same-derivative-depth
...
## Direct attack B'. Same-derivative-depth pairing in the energy identity
...
So the same-derivative-depth pairing route is structurally correct, but still
not closed.
...
The first term is the only candidate for a same-derivative-depth coercive
interaction, but it is **not** automatically harmless merely because it is in
divergence form.
