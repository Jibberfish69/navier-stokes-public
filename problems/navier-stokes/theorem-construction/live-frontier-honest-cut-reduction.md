# Live Frontier Honest-Cut Reduction
...
3. or a preserved secondary whole-space discharge branch.
...
do not name new theorem slots. They name the internal proving surface and the
selected theorem package in which the actual unresolved theorem sits.

**Proof.**
The route lock fixes the internal proving surface to the periodic/operator
branch, with whole-space `\mathbb R^3` treated only as downstream export. The
...
continuation-side interfaces on the time window. They become relevant only once
the selector-defect packet has produced the control needed to feed the
continuation criterion on that same window. Likewise the `\lambda_2^+` bridge is
...
separate whole-space discharge branch. On the Euclidean strict-shadow branch,
once the exact strict-shadow hypotheses hold, both `\mathrm{ShadowDef}(X)` and
...
The coarse seven-item frontier count reduces honestly to:
...
not the older expanded quartet bookkeeping.

**Proof.**
The strong/defect bridge note localizes the cycle-exact branch to the
directional theorem, with the exact-potential specialization collapsing to the
older directional-only `PD.70z` target and the normal-covector route reduced to
the explicit sufficient upgrade families `(CN.13j)`, `(CN.13q)`, and
`(CN.13ab)`-`(CN.13ad)`. The same note further states that on the general Hodge
