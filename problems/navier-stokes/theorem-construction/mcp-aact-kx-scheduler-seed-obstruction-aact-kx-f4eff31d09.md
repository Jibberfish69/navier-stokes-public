...
Target: prove the scheduler restart seed and finite terminal-tail coverage needed by the repaired local `AACT.KX` absorption lemma.
...
The obstruction is the seed. Carrying forward the previous interval gives a bounded value of `X_R`; the absorption lemma needs a fresh small averaged affine-excess value at the restart frame. The current cited route needs an added theorem producing that seed from `SCF_avg^m`, transported-frame minimization, or a finite-cover small-excess selection principle.
