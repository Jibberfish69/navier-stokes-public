...
Every mixed endpoint entry reduces to one of the four primitive rows above. Thus `END.Cross_avg` introduces no additional endpoint type.

## Closure

`CFI.A_avg` supplies persistence of `CM_avg`. The four eliminators remove the four primitive faces, and the cross row reduces mixed cases to those rows. Hence
