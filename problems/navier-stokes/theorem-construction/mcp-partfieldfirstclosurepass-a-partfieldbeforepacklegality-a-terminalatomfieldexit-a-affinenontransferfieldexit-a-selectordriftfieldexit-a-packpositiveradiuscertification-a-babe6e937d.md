...
putting packet-local Silver notation on the outside-CM participation-field record-window side.  That is no longer
current authority.
...
The outside-CM participation-field record-window audit uses the original equation-level participation law: same
datum, same maximal branch, same velocity-pressure evolution, viscosity,
pressure coupling, and incompressibility.  It does not use a outside-CM participation-field record-window
`Part_{N,Q}` predicate.
...
record, not a outside-CM participation-field record-window singularity filter.
