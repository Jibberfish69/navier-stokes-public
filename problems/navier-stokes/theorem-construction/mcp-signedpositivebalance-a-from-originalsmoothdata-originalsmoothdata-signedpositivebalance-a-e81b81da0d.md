# SignedPositiveBalance.A from OriginalSmoothData

## Target

Prove

```math
OriginalSmoothData\Longrightarrow SignedPositiveBalance.A.
```

Concrete form:

```math
\int_{Bad_N(\eta)}(\mathfrak S_{N,loc}^{active})_+\,dt
\le C_{SPB}
\int_{Bad_N(\eta)}(\mathfrak S_{N,loc}^{active})_-\,dt+o_N(1).
```

This estimate depletes the native positive source carrier on the selected
terminal bad windows once supplied.

## Direct tests

Global signed Littlewood-Paley balance and unweighted dyadic exchange identities act before terminal localization, one-sided weights, cutoffs, and positive-part extraction. They do not compare positive and negative parts of the selected native source carrier.

The weighted lifted remainder has the live principal strain term

```math
R_{j,k,\ell}^{lift}:=2^{2j}\langle \mathcal C_j(\nabla a_k,\Delta_j u),\Delta_j u\rangle.
```

Its sign can be positive on selected packets aligned with expanding strain. Current route inputs provide no negative-part dominance theorem on the same bad windows.

Taking absolute values bypasses sign balance and returns to active-square residual-tail / ScaleCriticalTreeCarleson.A.

Pressure, cutoff, and localization estimates route legal losses. They do not create positive/negative balance for legal interior source production.

## Result

`SignedPositiveBalance.A` is absent from current original-data inputs. It remains a possible source-control theorem, with no installed proof.

## Verdict

The signed-positive-balance branch stays open and reduces to the same
source-pulse wall.  The remaining pressure-strain route must prove a same-window
negative-part dominance or localized positive stretching depletion theorem; no
such theorem is installed here.
