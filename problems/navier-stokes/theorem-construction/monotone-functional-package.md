...
This package has a narrow and exact job:

- to specify exactly what the monotone bridge contributes, and
- to specify exactly what it does not contribute.
...
Discharged as a bridge consequence: `Q(t)` is necessary infrastructure, not the
whole theorem, and its exact insufficiency handoff is now explicit.
...
This package should now be read as a source-grounded same-surface bridge object:
...
- the bridge is localized as a same-surface theorem object inside the Euclidean Route B packet,
- and it remains insufficient by itself, exactly as the four-bridge architecture requires.
