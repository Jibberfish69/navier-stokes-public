...
Coefficient stopping turns these into an `O(\varepsilon_M)` perturbation in the
heat-scale packet calculus.
...
The Duhamel term is `O(\varepsilon_M)`, so the gauged packet remains in the
`j`-frequency collar and heat tube:
...
`A_k(x,t)-A_k(X_T(t),t)`, and cutoff transport must be `O(\varepsilon_M)` after
coefficient stopping and after choosing the shell gap `M` large.
...
strain error `O(\varepsilon_M)`, termwise residual `L_t^2H_x^{-1}` control, and
