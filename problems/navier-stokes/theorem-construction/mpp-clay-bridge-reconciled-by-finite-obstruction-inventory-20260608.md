...
Status: object-level reconciliation installed; supersedes the June 1 generic
Clay-bridge blocker for the currently promoted CM proof surface.

## Claim

The older local burden
...
while the complementary pass branch remains the only in-class continuation
branch:
...
inadmissible."  That statement tries to delete the fail branch after using it.
The CM route instead proves that a finite terminal obstruction is either the
pass branch, which continues, or the fail branch, which is a class-exit witness
and not an in-class nonsmooth continuation branch.
...
The June 1 gap audit remains useful as a warning against the shortcut

```text
Exit(Q) alone contradicts Clay breakdown by definition.
```
