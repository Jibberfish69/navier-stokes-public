...
The intended proof should be split into four exact steps.
...
inequalities `(CSP.14v)`--`(CSP.14w)` are then immediate because the annular
...
should give the exact decomposition
...
2. `\mathcal C^{visc,\delta}` is the usual viscous cutoff commutator, but now
   on the moving collar cutoff `\chi_{\rho,\psi,t}^{\delta}`;
...
The new terms created here must be expressed through `(CSP.13)`:
...
together with the usual higher-rung strain/cascade and viscous localization
terms.
...
The exact feeder map should therefore be read term-by-term:
...
3. `(TB.18)`, `(TB.26a)`, and `(TB.28)` supply the only packing-side bridge
   quantities that should enter when transported separations are converted back
   to Eulerian collar data;
...
linear in the remaining ledgers, so `(CSP.25g)` is immediate. The packet bridge
...
a first-rung defect that quotients out the harmless affine/shear sector, the
...
The definition `(CSP.25e)` makes `(CSP.25i)` immediate from `(CSP.25h)`. The
...
3. Corollary `FCI.A1a` reduces the genuinely inhomogeneous packet source on
   ...
   reduction note `FCC.C1`/`FCC.C2` already absorbs the cutoff channel into the
   viscous/coefficient side once `\mathfrak b_{\psi}^{cut}\in L^1(I)`, so the
   ...
   matching nonpressure survivor exactly: after cutoff absorption, the
...
```math
\boxed{
\text{can the same-fluid packet geometry carry enough one-rung-higher control on the moving collar to keep the common collar envelope finite on the interval?}
}
\tag{CSP.9}
```
