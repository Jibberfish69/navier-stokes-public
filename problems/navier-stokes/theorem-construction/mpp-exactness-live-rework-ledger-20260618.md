...
## Scale / Asymptotic Estimate Rework

Old claim being spent:

The live scale scan found three proof-force uses of `\sim` outside relation
notation.  They appeared in the annular twist/collar branch and the minimal
Zeno trace no-escape note:

- one unit of Dehn twist was said to cost `~ 1/L` on a degenerating collar;
- connection action was said to be `~` an integral of `g^{-1}g_s`;
- a graph-curvature countermodel used
  `\int_{\Sigma_k}|h_k|^2\,d\mu_k \sim k`.

Exact replacement:

The repaired notes replace those scale assertions by exact inequalities or
definitions:

- for the standard one-twist collar test map `Theta_1;L`,
  \(0\le\mathcal A_{tw}(\Theta_1;L)\le C_{tw}L^{-1}\to0\), with
  \(C_{tw}\) independent of \(L\);
- the connection-action readout is defined as
  \(\mathcal A_{\rm conn}(\mathcal Z):=\int_I |g^{-1}g_s|\,ds\), while the
  modulation-forcing alternative is named separately as a lower-bound theorem
  for \(\Pi_{\rm mod}F^{mod}\);
- for the graph
  \(\Sigma_k=\{(x,y,k^{-3/2}\phi(x,y)\sin(kx))\}\), the countermodel now states
  constants \(c_\phi,C_\phi,K_\phi>0\) such that
  \(c_\phi k\le\int_{\Sigma_k}|h_k|^2\,d\mu_k\le C_\phi k\) for
  \(k\ge K_\phi\), and supplies the graph calculation:
  small slope makes the second fundamental form integral comparable to
  \(\int|D^2f_k|^2\), while
  \(f_{k,xx}=-k^{1/2}\phi\sin(kx)+O(k^{-1/2})\).

Downstream consequence:

The collar branch now spends a precise degeneration statement: twist action has
infimum zero along collars with \(L\to\infty\), so a fixed-modulus action ledger
cannot close the degenerating-collar case.  The minimal Zeno trace route now
requires either the exact connection-action readout or an explicit modulation
forcing theorem.  The curvature countermodel now shows a genuine obstruction to
using raw diagnostic packet curvature as an NS-controlled currency unless the
...
- `theorem-construction/mpp-forward-gold-minimal-zeno-trace-noescape-attempt-20260618.md`;
- `source-frontier.yaml` was also repaired structurally so the live authority
  YAML parses; its critical-element status now records the exact three-input
  conditional closure in a block scalar.

Intentionally preserved hits:

- remaining `\sim` hits are selector-neighbor, survivor-equivalence, or
  summation-over-neighbor notation, not asymptotic estimates;
- `approximate normal` and `Approximate-identity convergence` are technical
  mathematical terms;
- `scale estimate` / `same-scale estimate` hits that remain are guard sentences
  or exact theorem labels that explicitly reject using scale language as proof.

Validation:

The focused asymptotic `\sim` scan now returns only relation/summation notation,
...
## Live Frontier And Continuation-Packet Authority Repair

Old claim being spent:

`live-frontier-honest-cut-reduction.md` said the old seven-item frontier list
"should be read" as historical and absorbed route pivots "should not" remain in
the active theorem packet.  `mpp-continuation-packet-generated-cm-contrapositive-20260514.md`
said the CM contrapositive "should generate" failure surfaces from the
continuation packet and that the missing `H^s` readout "should not" steer the CM
calculus.

Exact replacement:

The honest-cut note now states that the old seven-item list is historical, not a
set of independent current lemmas, and that absorbed route pivots do not sit in
the active theorem packet once integrated into the live authority cut.

The continuation-packet note now states the CM rule directly: the CM
contrapositive program generates its failure surfaces from the continuation
packet, and a missing `H^s` readout bridge is not a fourth CM failure face and
does not steer the CM-exit calculus as a PDE failure type.
...
Historical frontier dashboards and absorbed Marvin route pivots cannot be spent
as current theorem frontiers.  CM failure surfaces remain exactly the
carrier/Pack, participation/Part, and one-field/Field slots generated by
negating the continuation packet; readout-bridge absence remains a bridge gap,
not a new PDE face.

Second downstream pass and validation:

A fixed-string residue search over these two notes plus the root route table and
`live-theorem-edge.yaml` finds no remaining `should`, `O(1)`, `\lesssim`,
`\gtrsim`, `approximately`, `roughly`, `same order`, `typical`, `immediate`, or
