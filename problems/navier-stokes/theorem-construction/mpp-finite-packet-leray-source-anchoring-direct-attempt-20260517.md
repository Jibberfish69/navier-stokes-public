...
Therefore the sharpened pressure/Leray route does not close `B_ASAC^closed`. The canonical obstruction remains a zero-radius terminal Zeno source atom, Part-side when the same original-NS record remains and true packet/window evidence only when that record is absent pending a genuinely new production theorem.