...
For item `3`, the bound `(FC.18)` places `u(t)` in a bounded subset of `H^s`
up to time `T`. Standard local well-posedness in `H^s`, `s>5/2`, therefore
restarts the solution at any time sequence `t_n\uparrow T` with a lifespan
depending only on the uniform `H^s` bound. Hence the solution extends past
`T`.
...
```math
\boxed{
\text{no finite-time packing-detached / blown endpoint can occur before }T,
}
```
...
```math
\boxed{
\text{no finite-time tower-jump endpoint can occur before }T,
}
```
...
```math
\boxed{
\text{no finite-time tower-dead endpoint can occur before }T
...
More concretely, the witness would need to split into the three endpoint
channels already isolated in the companion note:

```math
\text{transport-dead / tower-dead}
\Longleftrightarrow
\text{failure of dynamic participation in a still-live neighborhood;}
\tag{FC.26d}
```

```math
\text{packing-detached / tower-blown}
\Longleftrightarrow
\text{failure of the common packing law / common deformation class;}
\tag{FC.26e}
```
