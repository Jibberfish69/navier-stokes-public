...
status: full-output-terminal-uniform-l1-service-storage-open
created: 2026-06-28
...
## 21. Log-density square function still needs actual-current capture

The paid parent-current span repair forces one more audit of `(130)`.  A
catalogued zero-current direction is fresh relative to the paid current history
until its first production updates
...
After `(134)`, repeated same-carrier production in that coordinate is Schur
current work.  So the dangerous case is not a repeated perpendicular channel.
It is a retained selected high-high event that is never admitted into the
update `(134)`.

Therefore the log-density equation `(130)` must be treated as a capture theorem
for the actual selected density.  Let \(f_{\rm act}\) be the actual retained
selected density and \(f_{pc}\) the exponential density generated by the
...
The needed admission/capture line is

\[
d\mu_{\rm root}^{sel}
\le
C f_{pc}\,d\mathcal R_{\rm root}
+dPaid_{\rm motion/return/visc/legal/stop}
+dEnd_{\rm same\text{-}witness}.
\tag{136}
\]
...
the standard stopped martingale consumer gives the actual selected
root-density gain `(129)`.

So the current sharpened target is

\[
\text{prove actual selected-density capture `(136)' plus parent-current square
function `(137)' from the original coupled packet.}
\tag{138}
\]
...
\[
\text{a parent-known carrier exponential}
\not\Longrightarrow
\text{control of the actual selected density without capture `(136)'.}
\tag{139}
\]
...
## 31. Root-fixed terminal-uniform AC storage pays the no-jump clause

The no-jump clause in `(198)` is not an extra reserve if the storage is really
built from the original smooth preterminal packet with terminal-uniform service.
Local AC on \([0,T-\varepsilon]\) is automatic and does not rule out a terminal
atom.  For a fixed parent \(P\), root-fixed full-packet storage must have the
form
...
\qquad
\int_{t_0}^{T}(\ell_P)_-\,dtdx\le C_N(u_0)+E_{\rm paid}(P),
\tag{203}
\]
...
where the un-paid part of \(\lambda_P\) is \(L^1\)-absolutely continuous with a
terminal-uniform bound on the same root history.  Then for every terminal
cylinder \(T_\rho(z_0)\),
...
Absolute continuity of terminal-uniform \(L^1\) integrals on the fixed root
history gives
...
bound alone permits one terminal jump; \(L^1_{\rm loc}\) preterminal AC allows
service concentration at \(T\); a child-renormalized AC storage permits
reset/rebase aliasing.  Root-fixed terminal-uniform AC storage is the required
object.
...
Partial, not Gold closed.  The strict affine producer is root-fixed
terminal-uniform \(L^1\) full-packet service storage `(203)`--`(204)`.
