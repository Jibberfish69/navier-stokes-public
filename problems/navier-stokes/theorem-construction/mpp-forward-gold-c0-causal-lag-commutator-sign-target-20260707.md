...
completion_truth: >-
  ...
  depending on aspect ratio. Thus the stretched-vortex branch needs an
  aspect-ratio/annular-compensation law for the retained velocity profile, not
...
trace-free second angular moment of that source.
...
Adding a radially symmetric positive annulus to represent the strain-dominated
outer source leaves this quadrupole essentially unchanged, because a spherical
annulus has no trace-free \(l=2\) moment:

```text
L/R = 0.35 with annulus   mu_sv =  0.43455
L/R = 3.00 with annulus   mu_sv = -0.33691
```
...
source generated by the coherent vortex profile must have a fixed helpful
quadrupole moment after collars and banks are owned. If the real retained
profile can be prolate in the source-level sense above without paying or routing
out, then the proposed \(c_0\) sign is false on that branch.
