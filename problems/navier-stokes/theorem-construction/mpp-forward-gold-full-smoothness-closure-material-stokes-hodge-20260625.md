...
by treating the top-strain/Hodge-selector storage as though it paid the whole
pressure-stress and cubic-collar annular service.
...
## 2. The exact intermediate identity
...
This identity pays the top-strain/Hodge-selector coordinate.  It does not pay
the whole moving annular pump.  The checked pressure/cubic cancellation notes
show that the non-top-strain-aligned remainder is the same transported
annular pressure-stress and cubic-collar service:
...
That is the remaining positive clock.  It is order-locked to the same
...
Thus the finite-breakdown contradiction is reduced exactly to `(FSCA.10)` or
to an equivalent bounded-below same-material storage that pays that positive
