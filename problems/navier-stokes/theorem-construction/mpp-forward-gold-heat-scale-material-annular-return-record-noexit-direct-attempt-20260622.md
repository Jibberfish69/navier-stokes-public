...
The material deformation equation is
...
So the heat-scale deformation clock is

```math
\mathcal K_m(I)
:=
\int_I
\|\nabla_yV_m(s)\|_{L^\infty(\widehat A_m(s))}\,ds .
\tag{HMR.8}
```

Finite \(\mathcal K_m(I)\) controls the material annulus geometry on \(I\):
...
bound for `(HMR.8)` on the delayed heat-time return window.
...
For example, the first coefficient rung already needs `(HMR.8)`, and the
...
Failure of `(HMR.19)` is not a hidden center atom.  It is the heat-scale
...
Any proof of `(HMR.22)` would close the pure forward-gold no-exit branch,
because the retained heat-scale annular return is already attached to the
four-body packet.  Without `(HMR.22)`, the correct landing is the Part-side
