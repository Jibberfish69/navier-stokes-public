...
completion_truth: >-
  ...
  high-Reynolds edge cap. The viscous edge estimate gives integrated production
  bounded by C_edge*Re*A0 on the neutral branch, and the pure Crow edge mode has
  no intrinsic positive seed floor. For any fixed admissible A0 in the
  normal-form window, the K5-only supremum over local Reynolds grows like Re.
...
That estimate is enough on the unit/marginal family. It is not a uniform
high-Reynolds cap.

## 3. Supremum failure

The pure edge mode has no intrinsic seed floor and no intrinsic Reynolds ceiling.
Choose any fixed admissible amplitude
...
inside the normal-form window. The K5-only residence price allowed by the
viscous estimate then scales as

\[
\mathcal I_{\rm K5}(\operatorname{Re},A_{\rm fix})
\lesssim C_{\rm edge}\,\operatorname{Re}\,A_{\rm fix},
\tag{K5H.4}
\]
