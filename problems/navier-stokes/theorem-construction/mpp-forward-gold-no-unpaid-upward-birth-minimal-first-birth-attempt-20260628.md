...
record whose positive increments generate \(b_Q^{term}\).  Let

\[
A_Q^{inv}:=\int_{I_Q}w_Q^{-1}\,d\mu_P,
\qquad
H_Q:={|I_Q|^2\over A_Q^{inv}} .
\tag{251}
\]

The parent connection identity gives only the harmonic square payment
...
inverse-weight lower bound

\[
\Delta_Q^+\ge \eta\,A_Q^{inv}
...
\Longrightarrow
\text{harmonic square payment `(252)' and packing `(258)'.}
\tag{259}
\]
...
\[
\text{prove selected terminal admission `(254)' or inverse-weight packing
`(256)' for retained annular-return and donor events.}
\tag{260}
\]
...
- What does it mean physically? Tiny reversible or signed terminal record
  ...
  when the stopped selector retains them at a definite inverse-weight size or the
...
## 41. Parent-subtracted selected action: replay cannot be recharged

The first-exit line `(287)` cannot be proved from ordinary Lipschitz stability
of the selected level functional alone.  Write the selected level as
...
A compactness/no-loss theorem for \(F_Q\), or a positive-part Lipschitz bound,
can control changes of a retained packet.  It does not say that an old positive
parent level may be charged again as a fresh child bill.  The obstruction is the
level-replay model

\[
Z_{Q_n}^{cap}=Z_0+\varepsilon_n e,
\qquad
F(Z_0)=1,
\qquad
\delta_P(Q_n^-,Q_n)^2\simeq\varepsilon_n^2,
\qquad
F(Z_{Q_n}^{cap})\simeq 1,
\tag{292}
\]

with \(\sum_n\varepsilon_n^2<\infty\).  If the selector is allowed to retain
each \(F(Z_{Q_n}^{cap})\) as a new child bill, then

\[
\sum_n\delta_P(Q_n^-,Q_n)^2<\infty,
\qquad
\sum_nF(Z_{Q_n}^{cap})=\infty .
\tag{293}
\]

Thus the actual selected bill must be parent-subtracted before the positive
part is counted.  Let \(\mathcal E_{Q^-}^{par}\) be the parent-announced
inherited span generated by the old active detector, terminal record, material
metric, active pressure quotient, and connection.  Define the fresh selected
increment
...
Every term in `(294)` is an increment relative to the parent record.  The
selector-capture theorem needed by `(287)` is therefore
...
Equivalently, zero fresh increment means no new selected action:

\[
\mathcal I_P(Q)=0
\Longrightarrow
Q\in Bundled_P\cup CM_P^{end}
\quad
\text{after stop/legal defects are removed.}
\tag{296}
\]

Once `(295)` is proved, the Bessel side is exactly the supplied
duality-and-packing mechanism:

\[
\sum_{Q\subset P}\mathcal I_P(Q)
\le
C_N\|\mathfrak J_P\|_{\mathfrak C_P^\ast}^2
+C_N\mathcal V_P^{act}
+C_NE_{\rm paid}(P),
...
\[
\text{parent-subtracted selected increment + parent Bessel packing}
\Longrightarrow
\text{no replayed level can be recharged as a new zoom bill.}
\tag{299}
\]
...
\[
\text{prove the actual stopped selector admits the parent-subtracted
replacement `(295)', or prove non-incremental replay cannot be
singularity-forcing selected action.}
\tag{300}
\]
...
- Is ordinary Lipschitz/no-loss enough? No.  It controls passage of the selected
  carrier, not repeated charging of an inherited positive level.
- What does the supplied duality-and-packing proof add? It identifies the only
  admissible currency: parent-built adjoint capacity plus positive material
  variation, both counted as increments before selection.
- What is now the exact hinge? The actual selector must see fresh
...
strain/Hodge--Stokes graph `(143)`--`(145)`.  Raw graph packing also fails if
...
terminal bill on the low-normalized dust branch.  Treating every low-normalized
...
`(186)`, tightness-or-paid-escape, critical parent-capacity packing `(175)`,
`(169)`, `(163)`, and `(162)`.  This supplies active same-carrier adjoint
admission `(155)` and quotient graph-capacity Bessel packing `(156)`.  If
`(190)` fails, the endpoint pulse is no longer a hidden Gold reserve; after
same-witness admission it is the CM Part/Field endpoint-face branch `(196)`.
The admitted same-carrier positive variations and active pressure-Hessian
readouts must pack finitely because fresh endpoint-invisible affine cycles and
selected pressure-Hessian carriers cannot keep escaping into smaller high-weight
tubes, staying as fixed-core terminal critical atoms, or hiding in moving null
...
formulations `(83)`, `(90)`, `(91)`, `(120)`--`(121)`, `(127)`, raw `(132)`,
raw `(145)`--`(147)`, child-fitted `(153)`--`(156)`, raw `(169)`, raw `(175)`,
and generic `(192)`--`(194)` are valid only insofar as they prove the
