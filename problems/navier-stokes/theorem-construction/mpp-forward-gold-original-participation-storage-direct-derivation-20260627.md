...
status: direct-derivation-reaches-signed-balance-square-coercivity-open
created: 2026-06-27
...
effect: >-
  ...
  route variables.  The derivation does not produce the required one-way square
  coercivity from installed inputs.  The exact missing line remains a
  bounded-below original participation storage / signed coercivity theorem for
  the whole same-material packet.
---
...
Completing the square gives the useful estimate
...
To turn `(13)` into `(1)`, the positive part of \(D_t\mathfrak S_J\) must become
a bounded-below storage drop.  That requires a same-packet bound of the form
...
The original NS energy law `(5)` does not supply `(15)`.  It supplies the signed
stress-work balance and raw viscous dissipation.  The Schur identity `(11)` puts
the selected route variables in the right same-packet storage, but it still
contains the endpoint Schur size, negative return, and inverse-Gram route
motion.  Those terms need a signed coercivity/no-free-return theorem of their
own.
...
terminal Schur growth, negative return, and inverse-Gram route motion for the
whole original pressure-viscosity-incompressibility-velocity packet.
```
