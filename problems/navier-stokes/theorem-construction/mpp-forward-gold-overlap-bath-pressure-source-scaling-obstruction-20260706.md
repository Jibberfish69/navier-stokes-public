...
completion_truth: >-
  ...
  kernel's degree-two spherical harmonic spans Sym_0(3). Such mean-zero source
  patterns can be realized at scaling level by divergence-free shear/quadrupole
  cells with |grad u|~H^(1/2), hence energy cost H D^5. For overlap retuning,
  H=|dot A|+G^2 and
  ...
  localization/producer curvature, Gate-8 no-recount allocation/compensator,
  faster-than-producer forced tracking, or a true dynamic bath construction. Follow-up
  ...
  D/R<=beta^(2/5) keeps it inside q=4/5. Follow-up subcell Reynolds threshold
