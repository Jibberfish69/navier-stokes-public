# Pressure-Helmholtz--Leray capacity Bellman root test
...
For a laminar material node `Q`, let `U_Q` be the gauge-reduced pressure-Helmholtz--Leray-stress potential on the transported parent collar. Define a capacity/Dirichlet reserve
...
This is nonnegative, same-packet, and monotone under restriction of admissible collars. It is the natural candidate for a root-positive-flux reserve because child stress-work must pass through the parent pressure-Helmholtz--Leray collar.
...
At the root, the Dirichlet reserve is controlled by an elliptic pressure-Helmholtz--Leray source norm. Schematically,
...
The pressure-Helmholtz--Leray capacity route therefore reduces to a precise embedding:

```math
\boxed{\text{PressureHelmholtzCapacityCarleson.A}}
```

For the original material history, prove that the pressure-Helmholtz--Leray-stress capacity measure of transported laminar collars is Carleson with respect to the Gold high-ratio action measure.
