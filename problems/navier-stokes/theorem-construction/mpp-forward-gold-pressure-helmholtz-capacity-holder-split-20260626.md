# Pressure-Helmholtz--Leray capacity Holder split

Status: capacity Carleson route narrowed; unconditional closure still open.

## 1. Candidate from previous note

The pressure-Helmholtz--Leray capacity Bellman route proposes a nonnegative reserve

```math
B_{cap}(Q)=\operatorname{Cap}_{A,G}(U_Q;A_Q)
```

for the gauge-reduced pressure-Helmholtz--Leray-stress potential on each laminar collar.
...
Therefore the pressure-Helmholtz--Leray capacity Carleson theorem reduces to:
...
The pressure-Helmholtz--Leray capacity Bellman route gives a concrete PDE representation for the desired root reserve, but the current installed estimates close only the velocity part. The surviving hard piece is the selected critical-strain capacity reserve. This is the next exact hinge, not a completed proof.