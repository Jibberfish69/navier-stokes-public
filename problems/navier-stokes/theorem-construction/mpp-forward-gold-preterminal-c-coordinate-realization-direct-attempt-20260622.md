...
status: preterminal-c-realized-fixed-atlas-crossscale-evolution-derived-moving-atlas-identity-reduces-to-modulation-and-jump-variation
...
edge_effect: >-
  ...
  direct attempt identifies the remaining obstruction exactly:
  d_sigma Y=grad_yY(-y+2sV+r^{-1}dot x-r^{-2}dot T V), so fixed center/time is
  already compatible, and only normalized center/time modulation plus
  label/sign/collar/selector jump variation remains to be carried as
  S_N/G_N/Q_N same-packet drift.
parents:
...
Thus fixed center/time is already compatible. The attempt still does not
complete the four-body symmetrizer for a general retained terminal family
because the normalized modulation terms and the label/sign/collar/selector
jump variation must be carried by same-packet \(S_N/G_N/Q_N\) drift channels.
...
with content `(PCN.13)` for the moving-atlas case.
