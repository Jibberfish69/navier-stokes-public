# Same-history affine-burst no-Zeno dichotomy

Status: direct no-Zeno proof not discharged; exact dichotomy isolated.

## 1. Target

The current Gold hinge asks for one strict gain in any equivalent form:

```math
\sum_{\ell\ge0}2^\ell\nu_\ell<\infty,
```

a strict good-lambda coefficient below the half-barrier,
...
The raw ledgers can still be summable along shrinking scales:

```math
\sum_k R_{raw}(Q_k)<\infty.
```

Thus a no-Zeno theorem must prove that the same-history condition creates a separate nonreusable charge.

There are only two cases.

## 3. Case I: repeated material core

The bursts reuse the same material ancestry line or a nested core with persistent material identity. Then every new unit burst must spend at least one of:
...
This gives a candidate storage, but its finite root bound is exactly the missing theorem unless one proves it from original data. Installed energy, viscosity, fixed-annulus, and fixed-collar ledgers do not bound this positive material-time variation.

So repeated-core no-reuse reduces to:

```math
\boxed{\text{RepeatedCoreNonreuseStorageBound.A}}
```
...
So shrinking-core escape reduces to:

```math
\boxed{\text{ShrinkingCoreEscapeCapacityLowerBound.A}}
```

## 5. Result

`GlobalSameHistoryAffineBurstNoZeno.A` is not yet proved. Its exact noncircular content is the following dichotomy theorem:

```math
\boxed{\text{NoZenoReuseEscapeDichotomyStrictCharge.A}}
```
