...
The next measure-level production criterion is now explicit.  In the finite
selected tower packet, the retained four-body packet carries the nonnegative
selected carrier measure with density \([h_P]_+\).  The remaining compactness
work is to prove that these measures are tight on the normalized same-carrier
packet space and that their weak limit is identified with the canonical selected
carrier of the limiting Navier-Stokes profile up to \(K_C^+\).  Under those
compactness conditions, `(SAP.32)` follows.
