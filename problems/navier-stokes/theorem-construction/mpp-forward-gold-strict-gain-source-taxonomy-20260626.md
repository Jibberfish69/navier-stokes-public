# Strict gain source taxonomy

Status: narrowed; no strict gain proved.

## 1. Current hinge

The current hinge is the single remaining Gold wall:
...
There are only four possible sources of a strict gain.

First, pure volume or raw heat thinning. This fails because the installed critical half-tail has finite raw mass and infinite first ratio moment.
...
The next theorem should not be another coordinate restatement of good-lambda, capacity, laminarity, or CKN density. It must prove a strict no-reuse charge.

```math
\boxed{\text{SameHistoryAffineBurstNoReuseStrictGain.A}}
```
