...
edge_effect: >-
  ...
  still requires finite frame/coefficient and velocity-pressure records. The
  same metric identity gives the exact deformation-factor lower bound: changing
  a retained material line length or collar thickness by factor Lambda costs at
  least log Lambda in the strain clock. Current energy and local energy inputs
  do not produce a finite same-carrier strain-metric/tower clock: a heat-scale
  pulse can create order-one material deformation at each scale while paying
  only radius-weighted physical dissipation. Thus the retained branch is paid by
  A_4B, while loss of this material same-packet record remains the CM Part face
  unless a new gold no-exit theorem is proved.
parents:
