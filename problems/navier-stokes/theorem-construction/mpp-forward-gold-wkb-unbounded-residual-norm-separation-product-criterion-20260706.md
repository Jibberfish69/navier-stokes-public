...
completion_truth: >-
  ...
  growth itself is a same-fluid backed row. The raw-CZ follow-up types the
  apparent pressure-kernel growth: in cell-mass coordinates the raw
  Calderon-Zygmund kernel may have B_M~M, but this is only the conversion from
  density to mass coordinates, and the product law becomes density-level
  anti-equidistribution. If exact or B_M^(-1/2)-near uniform legal clouds exist,
  the residual route still fails. This proves no same-fluid backing for B_M, no
  legal separation theorem, no WKB construction/exclusion, no confinement
...
For the raw pressure/CZ kernel, the apparent mass-coordinate growth is already
typed. If \(a_b=\omega_b f_b\) and \(\omega_b=1/M\), then a scale-invariant
density \(L^2\) bound gives
\[
  |Q(a-u)|
  \le
  C_{\rm CZ}M\|a-u\|_2^2.
  \tag{UR.21}
\]
The condition \(B_Md_M^2\gtrsim1\) is then just
\[
  \|f-1\|_{L^2_\omega}^2\gtrsim1.
  \tag{UR.22}
\]
So raw CZ contributes no new unbounded payer; it asks for density-level
...
Thus the non-raw unbounded-residual door is now:
\[
  \text{certify }B_M
  \quad+\quad
...
clouds still defeat the residual route. In the raw CZ case, the apparent
\(B_M\sim M\) growth is only density normalization, so the required separation
is \(\|f-1\|_{L^2_\omega}\gtrsim1\).
