...
The transported viscous cutoff collar density

```math
\nu |V_n|^2|\nabla\eta_n|^2\,dy\,ds
```

is included in the same Body-II payment measure.  Thus the collar energy
generated by the identity for \(\chi_n=\eta_n^2\) is not a separate
post-four-body estimate.
...
```math
\nu |V_n|^2|\nabla\eta_n|^2\text{ persists}
\quad\Rightarrow\quad
\text{visible Body-II cutoff-collar payment.}
```
