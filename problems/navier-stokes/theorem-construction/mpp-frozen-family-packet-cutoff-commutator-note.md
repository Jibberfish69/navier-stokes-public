...
\nu\,D_N(h,t;\psi)
+
4\nu\,\big(\Phi_{\psi}^{glob,(1)}(t)\big)^2\,E_N(h,t;\psi).
\tag{PCC.B0}
```
...
Summing in `k` gives `(PCC.B0)` after observing
...
side is already the transported packet-factor package from `FCC.A`, and the
current interval-closure readout for that package still uses the paired
low-carrier ledger `(CSP.25b)`. The companion cutoff reduction note now goes
farther than this packet-local writeup: `(FCC.C1)` combines the coefficient
closure `\mathfrak b_{\psi}^{cut}\in L^1(I)` with the pressure and
strain/cascade channels to recover the full frozen-family theorem `(FCI.5f)`.
...
\text{ and then use }(FCC.C1)\text{ to reduce the fixed-family source theorem to the pressure and strain/cascade channels.}
\tag{PCC.C}
```
...
1. `(PCC.B0)` spends the summed viscous packet dissipation
   `\sum_{\ell} D_N(\eta^\ell,t;\psi)`, which is part of the same neighboring-
   response witness and is not yet independently propagated on disk;
...
\newline
\text{and the first direct packet bounds still feed back through the same frozen-family propagation.}
}
\tag{PCC.D}
```
