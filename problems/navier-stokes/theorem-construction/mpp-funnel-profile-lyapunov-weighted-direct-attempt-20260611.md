...
ns_viewer:
  ...
  edge_effect: "Attempts FunnelProfileLyapunov.A by using a Gaussian weighted profile energy in backward self-similar variables. The linear rescaled operator gives coercive damping, but the nonlinear and pressure bulk terms remain scale-critical and unsigned."
  upstream_origin:
  ...
  downstream_consequence: "The natural weighted profile energy is a useful identity, not a closure theorem. FunnelProfileLyapunov.A now requires a new bound on the weighted nonlinear-pressure production term or a cancellation theorem for that term."
---
...
is to weight the profile energy by the Gaussian
...
on either the whole profile space, when justified by cutoff limits, or on a
bounded ball with the corresponding boundary errors retained.

For a smooth compactly supported profile calculation, multiply the profile
...
It is scale-critical in the same profile variables. The identity gives a
...
and the profile energy decays along late times.
...
This direct attempt proves a conditional profile identity:

```text
weighted linear profile loss
= nonlinear-pressure profile production.
```

It does not prove unconditional `FunnelProfileLyapunov.A`.

The exact new forward-positive subtarget is:
...
That is a genuine new estimate. Without it, the Gaussian profile energy is an
