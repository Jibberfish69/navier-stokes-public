---
theorem_id: goal-decide-the-confinement-constant-20260705
status: goal-surface-not-a-theorem-supersedes-close-goal-of-same-date
created: 2026-07-05
problem: navier-stokes
route: lane goal / confinement constant program
authorship: >-
  Goal design: Claude (Fable) with owner authorization, 2026-07-05.
  Physical story and lane laws: Thomas Birnie. Supersedes the
  2026-07-05 "close the MPP" goal description as the operative goal;
  the old goal's claim/story/laws remain valid reference.
completion_truth: >-
  Not mathematics. The operative lane goal. Altitude change from the
  superseded goal: CLOSE is replaced by DECIDE. Closure cannot be
  scheduled; decidability can. Every work unit below carries a kill
  condition stated before work begins. The forbidden terminal state is
  the eternal ledger.
---

# GOAL: decide the confinement constant

## The claim to be decided

Either there exists c > 0 such that every producing configuration of
the true field pays geometric eviction at rate at least c times its
production - or there exists a producing class that evades it. One is
true. The goal is reached on the day we know which.

## Why decide, not close

Every route in the repo and the literature ends at the same
tied-scaling inequality; the constant is its entire content. If the
constant holds AND composes onto the record ladder, the gold route
closes - two further gates, taken in order, never assumed. If it
fails, the physical story receives its honest boundary as a theorem.
Both endings are deliverables. The MPP finishes only through the
first, and no schedule may presume it.

## State carried in (2026-07-05)
...
dominated (KH clock). Self-cell reduced to pair (LIA length
    ...
    the up-scale recursion / exact matrix schedule. Follow-up summability
    criterion filed: with \(\beta_n=e^{-x_n}\) and holder ratio
    ...
    Poisson-source patterns on scale \(D\) at velocity-energy cost \(HD^5\).
    With
...
4. CROWD CELL - THE SUMMIT, NAMED AS SUCH. (a) dilute limit: floor
   ...
   Custody/bank variation has now been split the same way. A uniformly filled
   label cloud can circulate with nonzero population/refresh variation, and a
   whole bank can move in log scale, while \(f\equiv1\). Thus weight variation,
   refresh/reassembly variation, and bank log-variation are explicit
   ...
   Follow-up record-length-cap energy-density obstruction filed: the long K5
   packet with \(|a_L|^2L=A^2\) keeps total packet energy and total packet
   carrier-production proxy fixed while \(kL/\operatorname{Re}\to\infty\). The
...
6. EXTERNAL AUDIT. When pair cell + kill integral are referee-grade
   ...
   standing stone; refutation = one email instead of one month.

## Laws kept

Theory currency; exact identities and fractions; named thresholds
quarantined to certification; completion truth on every note; Codex
gates; same-day corrections; the physical story sets the targets.

## Laws added

Kill condition stated before work begins, per unit. No unit whose only
possible outcomes are "reduced" or "narrowed." Progress = decided
questions, not filed notes. A result that cannot be stated in four
plain sentences to a stranger is not yet a result.

## Exits (all honorable, one mandatory)

(i) Constant proved on the full family and composed -> proceed to the
Clay interface. (ii) Constant killed -> file the obstruction theorem;
the story rests at its true edge. (iii) Budget spent undecided ->
archive at a clean human-readable interface, stated as stalled, not
nearly-done. Forbidden: the eternal ledger.

## Honest prior, on the record

Most likely stall point: the crowd cell - it is the turbulence closure
problem. This goal is built so that even that ending leaves a decided
boundary and a stack of true, checkable mathematics.
