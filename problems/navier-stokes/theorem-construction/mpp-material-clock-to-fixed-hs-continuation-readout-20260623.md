...
edge_effect: >-
  ...
  removes the need to route the final periodic continuation step through
  Pack/Part/Field service language. The object is the transported whole-torus
  material history generated by the original smooth solution.
parents:
...
Fix \(s>5/2\).  Choose an integer \(N_s>s+2\).  The
continuation-depth material record \(\mathcal P_{N_s}^{mat}\) is required to
control, on the transported whole-torus material atlas,
