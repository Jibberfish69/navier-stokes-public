...
The next exact move is to package them into one material packet whose evolution
...
### Candidate Theorem MP-first-exit

Fix a finite depth `N` and a compactly supported material packet `\psi`.
If a finite-time class exit occurs on that packet, then at least one of the
following failures must occur:
...
```math
\boxed{
\text{finite-time class exit forces failure of finite packing class,}
}
```
...
That is the exact first-exit theorem shape for the material packet.
...
Show that dead-tower failure and jump-tower failure are too rigid to arise
inside a positive-time viscous field.
...
2. the rigidity layer for dead/jump failure;
