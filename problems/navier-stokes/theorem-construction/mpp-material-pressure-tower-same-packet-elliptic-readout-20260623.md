...
edge_effect: >-
  Installs the exact reason the pressure tower in the material record is not a
  pressure-only assumption. Pressure is the Lagrange multiplier enforcing the
  same incompressibility constraint on the same transported material packet.
  Its finite-depth tower is recovered from the velocity and coefficient towers
  by the material elliptic pressure equation, with local harmonic/collar pieces
  retained in the same interface-current record.
parents:
