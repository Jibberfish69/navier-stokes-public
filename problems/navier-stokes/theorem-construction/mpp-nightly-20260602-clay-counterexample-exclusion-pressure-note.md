...
as Pack/Part/Field Exit(Q), but does not yet prove that such classified exit
...
4. `mpp-no-third-branch-clay-closure-audit-20260525.md` rules out an in-class
   branch that both keeps `Member(Q)` and carries the derived face failure.
5. `mpp-clay-terminal-witness-invalidation-20260523.md` proves that a
   ...
   packet tree and is exhausted by a first Pack/Part/Field face failure.
...
declares a first failed Pack/Part/Field service.

Therefore W_* can only be used against smooth continuation by showing that this
face failure is a genuine terminal obstruction, not by treating it as a legal
...
ClayCounterexampleExclusionBridge.A:
  ...
  Pack/Part/Field face is derived cannot also serve as a legal in-class
...
2. CM exhaustion clause: the witness enters the same-fluid terminal tree and
   declares its first Pack/Part/Field face failure.
3. Alternative-exhaustion clause: after face failure, the object is not a
