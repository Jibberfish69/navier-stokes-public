...
Route tested after `B_ASAC^closed` was corrected to zero-radius source-residue support, Part-first when the same original-NS pointwise record remains and true `not Pack_Q` only when that record is absent.
