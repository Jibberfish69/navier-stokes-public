...
1. `tower-dead / transport-dead`:
   failure of dynamic participation in a still-live neighborhood;
2. `tower-blown / packing-detached`:
   failure of the common packing geometry;
3. `tower-jump`:
...
This is the one-field coherence witness. It is designed to detect tower-jump and
loss of local tower compatibility across neighboring points.
...
\Gamma(t)<\infty
\Longrightarrow
\text{no packing-detached / tower-blown endpoint at time }t.
\tag{RW.15}
```
...
\Longrightarrow
\text{no tower-jump endpoint at time }t.
\tag{RW.16}
```
...
\Longrightarrow
\text{no transport-dead / tower-dead endpoint at time }t
