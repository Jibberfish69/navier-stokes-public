---
ns_viewer:
  theorem_id: same-witness-clay-exclusion-propagation-direct-attempt-20260609
...
The original version of this note stated that public Clay finality still needed
...
It is not a remaining finite-breakdown branch that must be separately deleted by
...
Test whether the two newest branch results prove:
...
```text
Pack + Part + Field retained, but tower-blown still remains outside CM.
```

## Where The Propagation Fails

The first Pack-failure branch is different.

On a first Pack failure, the branch has:

```text
not Pack_Q.
```

So the retained CM hypothesis needed by the tower bridge is absent. The proof
cannot use:
...
to exclude a branch whose first fact is:

```text
not Pack_Q.
```

The Pack/tower dichotomy also does not exclude the branch. It proves:

```text
no tower-blown => Pack_Q.
```
...
The old public-finality reading is superseded for Silver. In the Gold/positive
supplier context, one sufficient theorem remains:
...
In the current repo that positive supplier target is sharpened as:
...
The checked reentry note reduces that to a non-alias first-Pack-survival
mechanism, with the next child:

```text
RetainedPositiveScaleNativeReserveBirthCharge.A
```

or an equivalent theorem such as:
...
`ClayCounterexampleExclusionBridge.A / NoGenuineExitFromSmoothData.A` is not
proved by the Pack/tower branch results.
...
1. the old selector/modulus Pack leaf is not the live wall;
2. the tower-blown retained-CM residue is consumed correctly;
3. tower-blown cannot be promoted to a pure CM row by definition.

The remaining Gold/positive supplier wall is:
...
or an equivalent non-alias first-Pack-survival mechanism.
...
non-alias child `RetainedPositiveScaleNativeReserveBirthCharge.A` or an
equivalent first-Pack-survival theorem.
