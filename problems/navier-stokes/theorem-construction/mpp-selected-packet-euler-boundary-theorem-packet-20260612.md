...
| `raw_terminal_source_residue_pack_or_part_exit` | `not Part_{N,Q}` or Field-window evidence outside CM | clean subcase only | raw source or pressure-source residue blocks Euler entrance until the residue is removed or typed as lawful inviscid momentum with no extra source measure. |
