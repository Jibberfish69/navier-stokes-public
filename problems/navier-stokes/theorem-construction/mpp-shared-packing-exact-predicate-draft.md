...
## Exact Meaning Of Packing-Detached / Blown

Under this predicate, the route-faithful meaning of a packing-detached or blown
endpoint is:
...
\mathrm{Pack}_{[0,T)}(u)
\Longrightarrow
\text{no packing-detached / blown endpoint occurs on }[0,T).
}
\tag{SP.8}
```
...
1. formalize the shared participation predicate;
2. formalize the shared one-field predicate;
3. prove that any alleged finite-time singularity forces failure of at least one
   of those exact predicates before it can count as an internal NS event.
