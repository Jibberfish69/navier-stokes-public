...
```mermaid
graph TD
  P0["P0: Classical target fixed"] --> M1["M1: Named monotone bridge Q(t)"]
  M1 --> M2["M2: Post-monotone cascade carrier"]
  M2 --> S1["S1: Dangerous-scale barrier"]
  S1 --> S2["S2: Dangerous-scale strength interface"]
  S2 --> K1["K1/K2/K3: Compactness and nonlinear closure"]
  K1 --> G1["G1: Gradient-transfer coercivity"]
  G1 --> G2["G2: Tail-energy recertification"]
  G2 --> C1["C1/C2: Same-surface and admissibility audits"]
  C1 --> C3["C3: Combined closure sufficiency"]
```
...
```mermaid
graph TD
  L0["Layer 0: Clay target and route lock"] --> L1["Layer 1: Source-grounded four-part architecture"]
  L1 --> L2["Layer 2: Theorem-primary proposition chain"]
  L2 --> L3["Layer 3: Exact live survivor and packet recurrence"]
  L3 --> L4["Layer 4: Modernization overlays"]
  L4 --> L2
  L5["Layer 5: Sidecar tracks and cross-folder feeds"] -.support / derivation only.-> L4
  L5 -.never overwrite theorem-primary truth.-> L2
  L6["Layer 6: Lagging generated surfaces and stale graph artifacts"] -.audit / do not trust raw.-> L2
```
