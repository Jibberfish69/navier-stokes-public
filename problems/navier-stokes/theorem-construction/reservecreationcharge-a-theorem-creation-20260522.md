...
- selection_reason: This theorem is the right next move because it changes the proof economy: Convert static donor-square reserve into a dynamic law: new terminal reserve is either inherited from Past(W) or paid by Charge_N(W). It is source-backed by 3 local ingredient(s). It deliberately reduces the remaining work to TerminalReserveFirstAppearanceCharge.A rather than pretending the frontier is closed. The internal solver also discharged the residual chain through ReserveCreationCharge.A -> TerminalReserveFirstAppearanceCharge.A -> ChargeLedgerInjection.A.
...
- WindowwiseReserveCarleson.A (windowwise_carleson_reserve; score=33)
- ScaleCriticalTreeCarleson.A (contrapositive_exit; score=84)
- ScaleCriticalTreeCarleson.A (compactness_defect_rigidity; score=83)
- ScaleCriticalTreeCarleson.A (monotone_budget; score=86)
...
- verdict: proved
- certification_level: theorem-creation:installed-proof-found
- rationale: A matching installed theorem-grade support entry already exists in the theorem packet.
