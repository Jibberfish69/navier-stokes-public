...
The difference between the original `S_{j-4}` interactions and the strict
low-mode pieces is split into a threshold-local spill packet
...
and a lifted high-side remainder
...
Moreover, `\mathcal{T}^{spill}_{j,n}` contains only the fixed threshold collar
`N-3 \le k \le N+M`, while `\mathcal{T}^{lift}_{j,n}` contains only modes
strictly above that collar and is therefore handed to the high-side remainder
packet together with the genuine high-high interaction. This lemma discharges
only the strict low-mode pieces.
...
Finally, the frozen-cutoff remainder is not a single finite-band object. The
threshold-local collar `(S_{N+M}-S_{N-4})u^{(n)}` is isolated as
`\mathcal{T}^{spill}_{j,n}`, while the lifted remainder
`(S_{j-4}-S_{N+M})u^{(n)}` is merged into the high-side packet handled together
with the genuine high-high interaction.
