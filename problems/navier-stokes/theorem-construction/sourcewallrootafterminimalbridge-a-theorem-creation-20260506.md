# SourceWallRootAfterMinimalBridge.A theorem creation

## Target

- target obligation: source-wall-root-after-reconcile
- target label: source-wall-root-after-reconcile
...
SourceWallRootAfterMinimalBridge.A. Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. This supplies the exact noncircular bridge needed for source-wall-root-after-reconcile.
...
- selection_reason: This theorem is the right next move because it changes the proof economy: Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. It is source-backed by 8 local ingredient(s). It deliberately reduces the remaining work to SourceWallRootAfterMinimalBridge.A.CoreSubprimitive rather than pretending the frontier is closed.

### Candidate Theorems

- SourceWallRootAfterMinimalBridge.A (minimal_bridge; score=74)
- SourceWallRootAfterContrapositiveExit.A (contrapositive_exit; score=68)
- SourceWallRootAfterCompactnessDefectRigidity.A (compactness_defect_rigidity; score=67)
- SourceWallRootAfterMonotoneBudget.A (monotone_budget; score=70)
...
- reason: No internal solver mechanism is installed for SourceWallRootAfterMinimalBridge.A.CoreSubprimitive.
- depth 0: SourceWallRootAfterMinimalBridge.A via minimal_bridge -> SourceWallRootAfterMinimalBridge.A.CoreSubprimitive
...
- next subprimitive: SourceWallRootAfterMinimalBridge.A.CoreSubprimitive -- core noncircular subprimitive for source-wall-root-after-reconcile
