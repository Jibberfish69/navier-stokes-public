# WindowPackingLeakage.A theorem creation

## Target

- target obligation: WindowPackingLeakage.A
- target label: window leakage is summable across the terminal scale tree
...
WindowPackingLeakage.A. Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. This supplies the exact noncircular bridge needed for window leakage is summable across the terminal scale tree.
...
- selection_reason: This theorem is the right next move because it changes the proof economy: Find the missing local bridge that is strictly weaker than the target but strong enough to feed dependency-discharge. It is source-backed by 8 local ingredient(s). It deliberately reduces the remaining work to WindowPackingLeakage.A.CoreSubprimitive rather than pretending the frontier is closed.

### Candidate Theorems

- WindowPackingLeakage.A (minimal_bridge; score=70)
- WindowPackingLeakage.A (contrapositive_exit; score=64)
- WindowPackingLeakage.A (compactness_defect_rigidity; score=63)
- WindowPackingLeakage.A (monotone_budget; score=66)
...
- reason: No internal solver mechanism is installed for WindowPackingLeakage.A.CoreSubprimitive.
- depth 0: WindowPackingLeakage.A via minimal_bridge -> WindowPackingLeakage.A.CoreSubprimitive
...
- window leakage is summable across the terminal scale tree
- full-MPP closure
...
- next subprimitive: WindowPackingLeakage.A.CoreSubprimitive -- core noncircular subprimitive for window leakage is summable across the terminal scale tree
