...
def certificate_for(entry)
...

  {
    ...
    "proof_explanation" => if status == "face_landing" && breaks.include?("Pack_Q")
      "SelectedFailure(surface) negates Pack_Q only through the row's selected Pack mechanism: #{diagnostic.fetch("selected_failure_type")}. This is a carrier/window/readout/admission failure, not an outside-CM participation-field/window disposal of the original NS participation law; any singular or source-residue branch with a same-solution pointwise record must be audited under Part first."
    else
