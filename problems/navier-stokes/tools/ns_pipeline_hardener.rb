...
CURRENT_SOURCE_WALL_ROOT_SUMMARY = [
  ...
  "The equality route under SourceWeightedTerminalAngularDepletion.A now has ASACDefectMeasure.A and TerminalNoFreeSinkEqualityClassProduction.A installed. The May 21 terminal Zeno correction classifies any B_ASAC^closed terminal atom as endpoint Pack failure: finite Zeno ancestry packets may be legal, but the terminal zero-radius residue is not a retained positive-radius packet. BASACTerminalAtomCMFaceLabeling.A, RetainedPackFieldNoIncomingParticipationFlux.A, and BASACLiouvilleCMParticipationBridge.A remain upstream finite-ancestry accounting or downstream post-exit readout, not a retained Field-packet conversion of the terminal residue. This closes terminal CM-legality while leaving the stronger forward positive supplier open.",
