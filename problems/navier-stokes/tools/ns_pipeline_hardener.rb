...
CURRENT_SOURCE_WALL_ROOT_SUMMARY = [
  ...
  "The equality route under SourceWeightedTerminalAngularDepletion.A now has ASACDefectMeasure.A and TerminalNoFreeSinkEqualityClassProduction.A installed. The May 21 terminal Zeno correction classifies any B_ASAC^closed terminal atom as endpoint Pack failure: finite Zeno ancestry packets may be legal, but the terminal zero-radius residue is not a retained positive-radius packet. NoPersistentUnpaidPositiveRadiusAncestry.A closes the positive-radius retained tail by the shared-participation strip balance: a positive terminal carrier must pay, receive incoming participation flux, leave through a legal ledger, or hit a finite CM face. BASACTerminalAtomCMFaceLabeling.A, RetainedPackFieldNoIncomingParticipationFlux.A, and BASACLiouvilleCMParticipationBridge.A remain the finite-ancestry accounting and post-exit readout support. This closes terminal CM-legality while leaving the stronger forward positive supplier open.",
