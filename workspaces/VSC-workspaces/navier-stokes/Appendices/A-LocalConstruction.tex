...
frequency cutoff therefore gives a unique representative in \(C(J;H^3)\), so
...
\)
and Cauchy--Schwarz therefore gives
\begin{equation}
...
\(L^2\) function with zero gradient and therefore coincide.  The uniqueness
...
this same interval.  Gronwall therefore yields
\begin{equation}
  \norm{w(t)}_{H^m}^2
  \leq
  \norm{a}_{H^m}^2
