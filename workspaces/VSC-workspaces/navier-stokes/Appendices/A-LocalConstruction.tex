...
The same local theorem first constructs the maximal history and later extends it from a smooth terminal value.
The existence time depends only on the fixed viscosity and the \(H^3\) size of the datum, while higher Sobolev orders persist on that base interval.
...
\]
Its three components are the middle trace, the parabolic gain, and the
equation's time row of one closed estimate.
...
\[
  ...
  \right).
\]
Since \(z_t=\nu\Delta z+F\), the equation supplies the remaining row:
\begin{equation}
  \norm{z}_{C(J_\delta;H^3)}
  +\norm{z}_{L^2(J_\delta;H^4)}
