...
Real motion can break that exact geometry through wobble, bending, misalignment, or disturbances that give viscosity more variation to spend as heat.
The case we're dealing with is the one in which none of those exits occurs: the vortex remains perfectly aligned for the next stretch, and every stretch leaves it thinner.

Nature does not supply that motion for free.
To persist, the fluid must arrive again and again at the balance that preserves the stretch while viscosity acts on the sharpening it creates.
...
Pressure is absent from the curl equation, but not from the motion.
It still determines the divergence-free velocity and strain that produce the stretch.
...
The scale of the equation separates energy control from critical concentration.
For \(\lambda>0\), set
\[
