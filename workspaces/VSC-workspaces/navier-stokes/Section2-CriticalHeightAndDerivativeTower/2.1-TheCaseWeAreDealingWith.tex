...
The word \emph{core} in the picture can be read in two ways.
It may follow a fixed set of fluid labels as they move apart, or it may mark the visible high-vorticity region while particles enter and leave it.
The drawing supports either physical reading.
The argument below uses only statements about the complete velocity field, so it does not ask the drawing to decide between them.
...
Moving to a smaller Navier--Stokes scale does not make diffusion disappear or make transport disappear; it preserves their relative strength.
At the same time,
\[
...
Choose \(H_0>H(0)\), set \(L_k:=2^kH_0\), and let \(t_k\) be the first time after \(t_{k-1}\) at which \(H\) reaches \(L_k\), beginning with
\[
...
The perfectly stretching picture has now become an exact demand on the full field: the same fluid would have to reach infinitely many new critical-height levels before one finite time.
The vortex does not only have to become thinner.
Its velocity has to change quickly enough to keep meeting the next level, its acceleration has to change quickly enough to produce that response, and its jerk has to change quickly enough to produce the next one.
