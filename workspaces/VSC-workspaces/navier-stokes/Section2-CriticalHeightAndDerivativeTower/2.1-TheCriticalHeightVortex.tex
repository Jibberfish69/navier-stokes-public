...
Now return to the kinetic-energy identity:
\begin{equation}\label{eq:BodyEnergyIdentity}
  \frac12\norm{u(t)}_2^2
  +\nu\int_0^t\norm{\nabla u(s)}_2^2\,ds
  =\frac12\norm{u_0}_2^2.
\end{equation}
The first term measures velocity over volume, one full scaling level below
\(H\).  It does not retain the width across which that velocity changes.  A
narrowing region can lose kinetic energy while its gradients rise and its
critical height remains visible.
...
The critical height tells us what a narrowing region can preserve while its
energy falls.  The linear heat flow supplies the other part of the picture: it
assigns an exact time scale to a width.  If
\(v_t=\nu\Delta v\), then
\[
