...
It remains a scalar quadratic functional of those rows, not another name for \(V_n\).
...
\end{equation}
Because \(\nu>0\), the coefficient of the top spatial energy never vanishes.
...
Once the finite response and transfer terms are controlled, moving deeper in time produces control farther across the Sobolev scale: more spatial regularity, not less.
The singularity pressure and the viscous ascent are balanced and opposing rather than complementary and self-feeding.
...
No single row has to carry an all-orders norm by itself.
