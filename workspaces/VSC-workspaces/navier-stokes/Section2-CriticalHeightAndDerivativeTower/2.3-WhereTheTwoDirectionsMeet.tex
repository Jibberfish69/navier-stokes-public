...
\[
  ...
  \ldots.
\]
If that row can be placed at every finite Sobolev level, then the spaces meet as an intersection:
\[
  V_r
  \in
...
The rows \(0,\ldots,N\) and columns \(0,\ldots,M\) cut out a finite rectangle.
Choose larger depths and the old entries remain the same entries inside a larger rectangle.
This is why compatibility matters: every overlap has to return the same derivative, not a new representative chosen for the larger block.

The right statement is finite at every stage.
For each chosen \(N\) and \(M\), control that rectangle through the entire approach to \(T_*\); allow its constant to depend on \(N\) and \(M\); then let the compatible family range over all finite choices.
Section~3 proves that whole-terminal rectangle estimate, completes the spatial ascent inside every temporal row, and takes the resulting intersection.
