...
Every finite choice of temporal depth and spatial height cuts one rectangle from this grid.
For fixed \(N\) and \(M\), the rows through \(N\) and columns through \(M\) form that rectangle, and every coordinate belongs to some finite choice.
The infinite intersection can be recovered only if those rectangles stay bounded through the terminal interval and agree on every overlap.
