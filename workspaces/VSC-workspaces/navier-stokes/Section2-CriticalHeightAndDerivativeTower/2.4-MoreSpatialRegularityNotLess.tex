...
The infinite grid is reached through finite rectangles.
For fixed \(N\) and \(M\), the rows through \(N\) and columns through \(M\) form one finite block, and every coordinate lies in some such block.
Classicality makes each finite rectangle finite at every strict cutoff \(T<T_*\).
That gives a bound after \(T\) is fixed, not one bound valid for all \(T<T_*\).
The remaining burden is to control each finite rectangle uniformly as \(T\uparrow T_*\), with bounds that agree wherever the rectangles overlap.
Section~\ref{sec:TheIntersectionInsideTheTower} takes up that finite-rectangle problem.
