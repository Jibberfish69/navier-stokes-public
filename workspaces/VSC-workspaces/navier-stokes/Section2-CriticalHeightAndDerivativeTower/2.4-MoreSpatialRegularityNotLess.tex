...
The matched spatial column moves the other way through the Sobolev scale:
\[
  E_{1/2},\ E_{3/2},\ E_{5/2},\ E_{7/2},\ldots
\]
A singularity makes us expect less spatial regularity.
The equation is exposing more, not less.
...
For fixed temporal depth \(N\) and spatial depth \(M\), the rows through \(N\) and the columns through \(M\) form one finite rectangle.
Every finite coordinate in the intersection belongs to one of them.

That is the object the next section has to control.
The vortex turned concentration into the critical-height problem; the Tower has turned that problem into one concrete demand: every finite rectangle, for every finite \(N\) and \(M\).
