...
The Laplacian in \eqref{eq:BodyTemporalVelocityRecurrence} already puts spatial derivatives inside every temporal row.
Differentiate that same row in space.
...
Thus \(\mathcal J[u_0]\) is one family of finite derivatives of one pair \((u,p)\), joined by the recurrences above.
...
The indices locate every finite derivative, but the critical-height balance tells us which spatial Sobolev energies viscosity supplies as \(t\uparrow T_*\).
Return to that balance and differentiate it in time.
