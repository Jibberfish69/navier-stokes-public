...
The lemma identifies every finite coordinate before the terminal time, while its bound may depend on the chosen \(T\).
In the rectangle coordinates of this appendix, the whole-terminal estimate from Section~\ref{sec:TheMissingEstimateAndGlobalSmoothness} reads as follows.
For \(N'\leq N\) and \(M'\leq M\), define the terminal bonding map
\[
  \rho_{N',M';I_*}^{N,M;I_*}:
  \mathscr X_{N,M}(I_*)
  \longrightarrow
...
These maps have the same identity and composition laws as their strict-cutoff counterparts.
...
\end{aligned}
\end{equation}
The constant may depend on the finite coordinate \((N,M)\).
The bound supplies the whole-terminal record
\[
  \cR_{N,M}[u_0]
  \in
  \mathscr X_{N,M}(I_*),
...
For every finite pair \((n,m)\), Theorem~\ref{thm:BodyWholeTerminalRectangleTheorem} gives \(V_n\in L^2(I_*;H^{m+1})\) and \(V_{n+1}\in L^2(I_*;H^{m-1})\), with every row belonging to the same pressure-complete history.
Fix \(N,M\), take \(m=M\), and add the finitely many squared norms for \(0\leq n\leq N\).
...
\[
  ...
  <\infty.
\]
The resulting whole-terminal sum is exactly \eqref{eq:WholeTerminalRectangleBound} in the present coordinates.
Forgetting higher temporal rows or lowering the spatial index preserves every shared distributional derivative and every pressure row, which yields \eqref{eq:WholeTerminalRectangleCompatibility}.
\end{remark}
...
The derivative identity and compatibility are part of the same-history record in \eqref{eq:WholeTerminalRectangleCompatibility}.
\end{proof}

The strict-subinterval lemma identifies the same coordinates before terminal time, and the whole-terminal estimate places each fixed rectangle across \(I_*\).
Compatibility makes their complete family one projective system.
