...
For finite \(N,M\in\Nzero\) and \(0<T<T_*\), define the squared adjacent-row norm
...
The gap between a strict cutoff and the whole terminal interval is already visible in one scalar function.
If \(f(t)=(T_*-t)^{-1/2}\), then
\[
  f\in L^2(0,T)\quad\text{for every }T<T_*,
  \qquad
  f\notin L^2(I_*).
\]
This function models only the quantifier and is not a Navier--Stokes history.
It shows why smoothness on every strict subinterval cannot supply a whole-terminal bound.
...
Assume the whole-terminal estimate in Theorem~\ref{thm:BodyWholeTerminalRectangleTheorem}.
...
\end{aligned}
\end{equation}
These finite coordinate bounds place the same mixed-jet record on the whole terminal interval:
\[
  \cR_{N,M}[u_0]
  \in
  \mathscr X_{N,M}(I_*),
...
\begin{proof}
Fix \(N,M\).
For \(0\leq n\leq N\), set
\[
  f_n^+(t):=\norm{V_n(t)}_{H^{M+1}}^2,
  \qquad
  f_n^-(t):=\norm{V_{n+1}(t)}_{H^{M-1}}^2.
\]
These functions are nonnegative, and the intervals \((0,T)\) increase to \(I_*\).
For \(\sigma\in\{+,-\}\),
\[
...
\]
Each integral on the right is a nonnegative term in the finite sum bounded by Theorem~\ref{thm:BodyWholeTerminalRectangleTheorem}, so
\[
  V_n\in L^2(I_*;H^{M+1}),
  \qquad
  V_{n+1}\in L^2(I_*;H^{M-1}).
\]
Adding these finitely many squared norms gives the velocity part of \eqref{eq:WholeTerminalRectangleBound}.
...
\[
  ...
  <\infty.
\]
This completes the pressure component of the record.
Forgetting higher temporal rows or lowering the spatial index leaves every shared distributional derivative and normalized pressure row unchanged, which gives \eqref{eq:WholeTerminalRectangleCompatibility}.
\end{proof}

Take the rectangle \((N,M)=(2,3)\).
Its velocity coordinates are \(V_0,V_1,V_2\) in \(L^2(I_*;H^4)\) and \(V_1,V_2,V_3\) in \(L^2(I_*;H^2)\), together with the pressure rows \(Q_0,Q_1,Q_2\) in \(L^1(I_*;H^3)\).
Restricting this rectangle to \((1,2)\) drops \(V_3\) and \(Q_2\), views \(V_0,V_1\) in \(L^2(I_*;H^3)\) and \(V_1,V_2\) in \(L^2(I_*;H^1)\), and views \(Q_0,Q_1\) in \(L^1(I_*;H^2)\).
Every retained coordinate is the same distributional derivative, now read in a weaker Sobolev space.
This is compatibility; the scalar example above concerns the separate question of whether any one coordinate stays finite on the whole terminal interval.

The finite-rectangle form also recovers the adjacent rows coordinate by coordinate.
