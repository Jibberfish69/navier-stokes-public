...
A larger rectangle already contains every coordinate of a smaller one.
All rectangles are made from distributional derivatives of the same \(u\), so their shared coordinates agree before any limit is taken.
This compatibility is the formal passage from the tower to the intersection.
Order the finite pairs \((N,M)\) coordinatewise and use the whole-terminal bonding maps from \eqref{eq:WholeTerminalRectangleCompatibility}.
Their inverse-limit record set is
\[
  \mathscr X_\infty(I_*)
  :=
...
The rectangle \(\cR_{N,M}[u_0]\) supplies
\[
...
\begin{equation}\label{eq:SectionThreeAllSpatialEnergies}
  ...
  <\infty.
\end{equation}
Nothing here sums the Sobolev norms over all orders.
The construction asks for a finite constant \(C_{N,M}\) only after a finite coordinate has been selected.
