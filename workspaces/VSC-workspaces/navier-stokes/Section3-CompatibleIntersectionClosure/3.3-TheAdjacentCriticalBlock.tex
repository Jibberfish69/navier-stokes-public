...
A finite singular endpoint would have to break the joined derivative structure of this same field.
The first concrete terminal readout is the critical height carried by the adjacent spatial rows.
The adjacent trace mechanism carries that readout to \(T_*\).
...
\]
Thus the critical height and the \(E_{3/2}\) energy both reach the terminal face with continuous representatives.
The pressure-explicit production current remains an integrable time-current.
No endpoint value is assigned to that current.
This single rectangle settles the critical readout.
The whole tower supplies the further endpoint needed for restart.
