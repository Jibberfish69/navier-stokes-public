...
\end{equation}
That statement holds for every strict cutoff and still does not give a bound that survives the approach to \(T_*\):
\[
  \forall T<T_*,\quad \mathfrak R_{N,M}(u;T)<\infty
  \quad\not\Longrightarrow\quad
  \sup_{0<T<T_*}\mathfrak R_{N,M}(u;T)<\infty.
\]
Reaching \(T_*\) requires the quantity on the right to be finite after \(N\) and \(M\) have been chosen.
That cutoff-independent bound is what carries the rectangle to the endpoint.
