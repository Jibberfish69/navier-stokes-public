...
and first stop at a strict cutoff \(T<T_*\).  A spatial row by itself has no
reason to arrive continuously at \(t=T\).  It needs its time derivative on
the other side of the desired trace space.
...
\end{equation}
Cauchy--Schwarz places the middle trace between the two adjacent rows
...
At \(M=2\), the first concrete pair is
\begin{equation}\label{eq:FirstConcreteAdjacentPair}
  ...
  u\in C([0,T];H^2).
\end{equation}
One row sits above the desired trace and the next temporal response sits below
it.  Their pairing reaches the middle.
...
Write \(\cR_{N,M}[u_0;T]\) for this velocity block together with the pressure
rows reconstructed by \eqref{eq:BodyTemporalPressureRecurrence} and the
differentiated equations that identify every entry.  The rectangle is a
finite piece of the derivative grid; every entry still belongs to the same
...
\]
After \((N,M)\) is fixed, the proof needs one bound that survives every cutoff
