...
Begin with the lowest rectangle.  It is the first place where the spatial row
and the response beneath it can reach one time slice together.  At
\((N,M)=(0,1)\), it asks for
...
The critical balance does close this interface for one genuine class of data,
and the calculation shows exactly where that class ends.  Put
\[
...
\begin{assumption}[Whole-terminal rectangle condition]\label{ass:BodyWholeTerminalRectangleCondition}
Let \(u_0\in\Ssigma\), let \(\nu>0\), and let \((u,p)\) be the maximal classical
solution generated by \((u_0,\nu)\) on \([0,T_*)\), with \(T_*<\infty\).  For
...
Given any finite temporal order and spatial height, choose one rectangle large enough to contain that derivative; every larger rectangle returns the same derivative of the same \(u\).
