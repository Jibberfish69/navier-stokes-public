...
Fixed material labels retain positive separation at \(T_*\).
Changing membership cannot preserve a nonzero whole-field critical tail beyond every frequency.
The narrowing-core picture therefore reaches zero scale in neither of the two senses tested here.
...
The first consequence is that one finite Sobolev row can control an infinite sum of spatial scales.
Fix any finite \(T>0\).
The global solution is classical on \([0,T]\), and the block \(\cR_{0,2}[u_0;T]\) controls the vorticity strongly enough to sum its dyadic pieces.
