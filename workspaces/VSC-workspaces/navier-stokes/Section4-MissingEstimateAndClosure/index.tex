...
\end{equation}
Each requested norm uses one finite rectangle and has its own finite constant.  No sum
over all derivative orders or common analytic radius is used here.
...
retain positive separation, and a core with changing membership sits inside a
whole-field critical tail that vanishes uniformly at high frequency.  Neither estimate
uses wobble, fragmentation, buckling, or lost symmetry.  The perfectly stretching
vortex cannot carry singular critical content into zero scale at \(T_*\).
...
The compatible traces also give \(u_*\in H^\infty\); the differentiated recurrence
determines its normalized pressure-complete endpoint jet.  Appendix~\ref{app:CompatibleTerminalAssembly}
proves that these traces agree and that local uniqueness glues the restarted solution
to the original classical history.
