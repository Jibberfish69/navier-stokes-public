...
Those relations keep the velocity intersection pressure-complete without confusing a slaved pressure jet with membership in the velocity space itself.

The target is now clear, but a target containing every finite pair is not yet a finite estimate that can be proved in one step.
A proof has to take finitely many temporal rows and finitely many spatial heights, join adjacent energy levels around common time traces, and then let the finite choices range.
That demand determines the shape of the finite records.
